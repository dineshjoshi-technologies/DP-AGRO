# DPA-149 Permit Application: State Agriculture Department

**Application ID:** DPA-149-STATE-001
**Submission Date:** 2026-09-05
**Applicant:** Botanical Research Agent (DPA)
**Research Workstream:** DPA-75 Phase 1 — Medicinal Plant Genomics
**Applicable To:** State Government Research Permit
**Target State:** Karnataka (confirmed; other states provisional)

## 1. Project Overview
- **Project:** Medicinal Plant Genomics Research for AI-Driven Cultivar Improvement
- **Workstream:** DPA-75 Phase 1
- **Principal Investigator:** Botanical Research Agent (interim Plant Genomics lead)
- **Coordinator:** DPA-76 Sustainable Manufacturing (bi-weekly sync)
- **Status:** DRAFTED / pending review & submission

## 2. States Covered
| State | Agriculture/Horticulture Dept | Research Purpose | Estimated Timeline |
|---|---|---|---|
| Karnataka (confirmed) | Department of Horticulture | Genomic sample collection from pilot farms; agro-climatic zone mapping | 3-4 weeks |

Total states: **1 confirmed (Karnataka)** — all 10 pilot farms (FARM-001…FARM-010) are located in Karnataka. Additional states (Maharashtra, Tamil Nadu, Gujarat, Uttar Pradesh, Rajasthan) remain provisional expansion candidates and are excluded from this filing until pilot farms are established there. Confirmed per [pilot-farm-confirmation](pilot-farm-confirmation.md).

## 3. Research Activities Permitted
The following activities are requested under this permit:
- **Non-destructive sampling:** Leaf and seed collection from cultivated plants (max 20% per plant per season)
- **Genomic DNA extraction:** Laboratory processing of collected samples
- **Field observations:** Phenotyping, growth measurements, harvest timing
- **Soil and water sampling:** Physico-chemical analysis from farm plots
- **Pest and disease monitoring:** Visual assessment and sample collection
- **Harvest data collection:** Yield per plant, biomass measurements
- **NO destructive sampling:** Roots not excavated; NO plant uprooting permitted
- **NO field experiments:** No planting of new genotypes; no fertilizer/ pesticide trials

## 4. Plant Material and Quantities
- **Per Farm:** 50 plants per species × 6 species = 300 plants maximum per DPA pilot farm
- **Per State:** 10 DPA pilot farms confirmed in Karnataka
- **Total:** ~3,000 plants (cultivated, marked for observation only)
- **Collection Window:** Post-monsoon (Oct-Nov 2026) through summer (Mar-May 2027)
- **Sampling Frequency:** Once per growth phase (vegetative, flowering, fruiting)

## 5. Reporting Requirements
- **Monthly Progress Report:** Activities, any issues, sample counts
- **Quarterly Summary:** Aggregated data across all permitted farms
- **Final Report:** Complete dataset, genomic insights, cultivation recommendations
- **Data Sharing:** Summary statistics shared with state agriculture dept; raw data per collaboration agreement

## 6. Compliance and Conditions
- **Permit Valid Until:** 31 December 2027 (or project completion, whichever earlier)
- **Prior Notice:** 72 hours before any field activity in new locations
- **Labeling:** All sample bags tagged with DPA permit number and farm ID
- **Destruction of Samples:** All genomic DNA extracts to be stored per SOP; no disposal without dept notification
- **Confidentiality:** Farm-specific data confidential; aggregate data shared with permission
- **Inspection Rights:** State agriculture dept reserves right to inspect farms during permit period

## 7. Application Declaration
I hereby declare that:
1. All activities will comply with state agriculture department regulations
2. Only cultivated plants will be sampled; no wild collection involved
3. All samples will be properly tagged and tracked per permit requirements
4. Monthly and quarterly reports will be submitted as specified
5. No activities will disturb normal crop growth or farming operations
6. I will coordinate with DPA-76 for bi-weekly sync and reporting

**Applicant Signature:** ___________________________
**Date:** ___________________________

## 8. Attachments
- [x] DPA pilot farm registration records (Karnataka) — [pilot-farm-registration-karnataka.md](../pilot-farms/pilot-farm-registration-karnataka.md)
- [ ] List of partnered farmer cooperatives — engagement record on file ([partner-cooperatives-karnataka.md](../pilot-farms/partner-cooperatives-karnataka.md)); pending real cooperative engagement + RCS verification before filing cites registration numbers
- [ ] Sample collection SOP and tracking sheet
- [ ] Monthly reporting template (DPA-149-Report-Template)
- [x] Farm location coordinates and land records — [farm_registry.json](../../ops/sensor_network/farm_registry.json) + registration pack (survey land records pending DPA-76)

---

**Next:** Submit to respective state agriculture departments; DPA-76 to coordinate state-specific follow-up; track approval timelines in DPA-149 permit tracker.