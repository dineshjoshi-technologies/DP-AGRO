...
import { codexHomeHasUsableAuth, isManagedCodexHomePath, pathExists, prepareManagedCodexHome, resolveManagedCodexHomeDir, resolveSharedCodexHomeDir, seedManagedCodexHome, } from "./codex-home.js";
import { prepareCodexRuntimeConfig } from "./runtime-config.js";
import { resolveCodexDesiredSkillNames } from "./skills.js";
import { buildCodexExecArgs } from "./codex-args.js";
import { SANDBOX_INSTALL_COMMAND } from "../index.js";
import { CODEX_OUTPUT_INACTIVITY_MONITOR_SIGTERM_GRACE_MS, createCodexOutputInactivityMonitor, formatOutputInactivityMonitorErrorMessage, resolveCodexInactivityTimeout, } from "./output-inactivity-monitor.js";
...
function signalCodexChild(target, signal) {
...
export async function execute(ctx) {
    ...
    // and left untouched.
    const configuredHomeIsManaged = configuredCodexHome != null &&
    ...
    if (effectiveHomeIsManaged &&
        ...
        throw new Error(`no Codex credentials provisioned for managed home "${effectiveCodexHome}" ` +
    ...
    const envConfigStrings = Object.fromEntries(Object.entries(envConfig).filter((entry) => typeof entry[1] === "string"));
    ...
    try {
        ...
        const preparedExecutionTargetRuntime = executionTargetIsRemote
            ? await (async () => {
                await onLog("stdout", `[paperclip] Syncing workspace and CODEX_HOME to ${describeAdapterExecutionTarget(executionTarget)}.\n`);
        ...
        const executionTargetIsSandbox = runtimeExecutionTarget?.kind === "remote" && runtimeExecutionTarget.transport === "sandbox";
        ...
        if (monitorResolution.mode === "disabled") {
            await onLog("stdout", `[paperclip] Codex output inactivity monitor is DISABLED via adapterConfig.outputInactivityTimeoutMs=null. Hung codex runs will only be detected by the platform-level silent-run safety net.\n`);
        }
        else if (monitorResolution.mode === "default" && "reason" in monitorResolution) {
            await onLog("stdout", `[paperclip] Ignoring non-positive adapterConfig.outputInactivityTimeoutMs; falling back to default ${monitorResolution.timeoutMs}ms.\n`);
        }
        ...
        const runAttempt = async (resumeSessionId) => {
            ...
            let monitorTerminationSignal = null;
            let monitorElapsedMs = 0;
            let monitorTimeoutMs = 0;
            let killTarget = null;
            let sigkillTimer = null;
            let monitorLogPromise = null;
            const monitor = monitorResolution.mode === "disabled"
                ...
                : createCodexOutputInactivityMonitor({
                    ...
                    onFire: (state) => {
                        ...
                        const logLine = `[paperclip] adapter.invoke ${message}; ` +
