...
export function isCodexUnknownSessionError(stdout, stderr) {
    const haystack = `${stdout}\n${stderr}`
        ...
        .filter(Boolean)
        .join("\n");
    return /unknown (session|thread)|session .* not found|thread .* not found|conversation .* not found|missing rollout path for thread|state db missing rollout path|state db returned stale rollout path|no rollout found for thread id/i.test(haystack);
}
