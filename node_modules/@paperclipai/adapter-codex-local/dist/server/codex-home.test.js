...
import { codexHomeHasUsableAuth, ensureSymlink, isManagedCodexHomePath, prepareManagedCodexHome, reconcileManagedCodexHome, seedManagedCodexHome, } from "./codex-home.js";
    ...
    it("still throws on EEXIST when a raced-in auth symlink points elsewhere", async () => {
        ...
        try {
            await expect(prepareManagedCodexHome({
                ...
                PAPERCLIP_HOME: paperclipHome,
                PAPERCLIP_INSTANCE_ID: "default",
            }, async () => { }, "company-1")).rejects.toMatchObject({ code: "EEXIST" });
            expect((await fs.lstat(managedAuth)).isSymbolicLink()).toBe(true);
...
describe("isManagedCodexHomePath", () => {
    ...
    const companyRoot = path.resolve("/srv/paperclip/instances/default/companies/company-1");
    it("treats the per-agent managed home as managed", () => {
        expect(isManagedCodexHomePath(env, "company-1", path.join(companyRoot, "agents", "agent-7", "codex-home"))).toBe(true);
    });
    it("treats the shared company home as managed", () => {
        expect(isManagedCodexHomePath(env, "company-1", path.join(companyRoot, "codex-home"))).toBe(true);
    });
    it("treats a path outside the company tree as an external override", () => {
        ...
        expect(isManagedCodexHomePath(env, "company-1", path.resolve("/srv/paperclip/instances/default/companies/company-2/codex-home"))).toBe(false);
    });
    it("returns false without a companyId", () => {
        expect(isManagedCodexHomePath(env, undefined, path.join(companyRoot, "codex-home"))).toBe(false);
    });
});
...
describe("seedManagedCodexHome", () => {
    it("symlinks auth.json from the shared source into an explicit per-agent home", async () => {
        ...
        try {
            const sharedCodexHome = path.join(root, "shared-codex-home");
            const agentHome = path.join(root, "instances", "default", "companies", "company-1", "agents", "agent-7", "codex-home");
            const sharedAuth = path.join(sharedCodexHome, "auth.json");
...
describe("reconcileManagedCodexHome", () => {
    async function makeFixture() {
        ...
        const sharedCodexHome = path.join(root, "shared-codex-home");
        const paperclipHome = path.join(root, "paperclip-home");
        const agentHome = path.join(paperclipHome, "instances", "default", "companies", "company-1", "agents", "agent-7", "codex-home");
        const sharedAuth = path.join(sharedCodexHome, "auth.json");
        const agentAuth = path.join(agentHome, "auth.json");
        await fs.mkdir(sharedCodexHome, { recursive: true });
        await fs.writeFile(sharedAuth, '{"token":"shared"}', "utf8");
    ...
    it("preserves an existing API-key auth.json when the key is secret-bound", async () => {
        ...
        try {
            ...
            await fs.writeFile(fx.agentAuth, JSON.stringify({ OPENAI_API_KEY: "sk-secret-resolved" }), { mode: 0o600 });
            const result = await reconcileManagedCodexHome({
                companyId: "company-1",
                configuredCodexHome: fx.agentHome,
