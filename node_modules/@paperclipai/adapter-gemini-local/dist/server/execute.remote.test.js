...
const { runChildProcess, ensureCommandResolvable, resolveCommandForLogs, prepareWorkspaceForSshExecution, restoreWorkspaceFromSshExecution, runSshCommand, syncDirectoryToSsh, startAdapterExecutionTargetPaperclipBridge, } = vi.hoisted(() => ({
...
vi.mock("@paperclipai/adapter-utils/ssh", async () => {
    const actual = await vi.importActual("@paperclipai/adapter-utils/ssh");
    return {
        ...actual,
...
describe("gemini remote execution", () => {
    ...
    it("prepares the workspace, syncs Gemini skills, and restores workspace changes for remote SSH execution", async () => {
        ...
        expect(runSshCommand).toHaveBeenCalledWith(expect.anything(), expect.stringContaining(".gemini/skills"), expect.anything());
        ...
        expect(runSshCommand).not.toHaveBeenCalledWith(expect.anything(), expect.stringContaining(".gemini/settings.json"), expect.anything());
        expect(runSshCommand).not.toHaveBeenCalledWith(expect.anything(), expect.stringContaining("gemini-api-key"), expect.anything());
        const call = runChildProcess.mock.calls[0];
    ...
    it("pre-selects gemini-api-key auth in the managed HOME for sandbox execution", async () => {
        ...
        const runnerScripts = runnerExecute.mock.calls.map((call) => `${call[0].command} ${(call[0].args ?? []).join(" ")}`);
...
//# sourceMappingURL=execute.remote.test.js.map