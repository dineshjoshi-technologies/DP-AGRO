...
import { asBoolean, asNumber, asString, asStringArray, buildPaperclipEnv, buildInvocationEnvForLogs, ensureAbsoluteDirectory, ensurePaperclipSkillSymlink, joinPromptSections, ensurePathInEnv, refreshPaperclipWorkspaceEnvForExecution, readPaperclipRuntimeSkillEntries, readPaperclipIssueWorkModeFromContext, resolvePaperclipDesiredSkillNames, removeMaintainerOnlySkillSymlinks, parseObject, renderTemplate, renderPaperclipWakePrompt, stringifyPaperclipWakePayload, DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE, } from "@paperclipai/adapter-utils/server-utils";
import { DEFAULT_GEMINI_LOCAL_MODEL, SANDBOX_INSTALL_COMMAND } from "../index.js";
import { describeGeminiFailure, detectGeminiAuthRequired, isGeminiTransientNetworkError, isGeminiTurnLimitResult, isGeminiSessionUnrecoverableError, parseGeminiJsonl, } from "./parse.js";
...
function buildGeminiHeadlessEnv(env) {
    const next = { ...env };
...
function buildGeminiRuntimeEnv(env) {
    return Object.fromEntries(Object.entries(ensurePathInEnv({ ...process.env, ...buildGeminiHeadlessEnv(env) })).filter((entry) => typeof entry[1] === "string"));
}
...
export async function execute(ctx) {
    ...
    if (executionTargetIsRemote) {
        try {
            ...
            await onLog("stdout", `[paperclip] Syncing workspace and Gemini runtime assets to ${describeAdapterExecutionTarget(executionTarget)}.\n`);
            ...
            const managedRemoteHomeDir = managedHome && preparedExecutionTargetRuntime.runtimeRootDir
            ...
            if (managedRemoteHomeDir && hasGeminiApiKey) {
                ...
                await runAdapterExecutionTargetShellCommand(runId, executionTarget, `mkdir -p ${JSON.stringify(path.posix.dirname(remoteSettingsPath))} && { [ -f ${JSON.stringify(remoteSettingsPath)} ] || printf '%s' ${JSON.stringify(authSettingsJson)} > ${JSON.stringify(remoteSettingsPath)}; }`, { cwd, env, timeoutSec, graceSec, onLog });
            }
        }
        catch (error) {
    ...
    if (executionTargetIsRemote && runtimeSessionId && !canResumeSession) {
        await onLog("stdout", `[paperclip] Gemini session "${runtimeSessionId}" does not match the current remote execution identity and will not be resumed in "${effectiveExecutionCwd}". Starting a fresh remote session.\n`);
    }
    else if (runtimeSessionId && !canResumeSession) {
        await onLog("stdout", `[paperclip] Gemini session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${effectiveExecutionCwd}".\n`);
    }
