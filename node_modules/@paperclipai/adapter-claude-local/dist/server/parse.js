...
const CLAUDE_EXTRA_USAGE_RESET_RE = /(?:out\s+of\s+extra\s+usage|extra\s+usage|usage\s+limit\s+reached|usage\s+cap\s+reached|5[-\s]?hour\s+limit\s+reached|weekly\s+limit\s+reached|claude\s+usage\s+limit\s+reached)[\s\S]{0,80}?\bresets?\s+(?:at\s+)?([^\n()]+?)(?:\s*\(([^)]+)\))?(?:[.!]|\n|$)/i;
...
export function isClaudeMaxTurnsResult(parsed) {
    ...
    return structuredStopReasons.some((reason) => reason === "max_turns" ||
        reason === "max_turns_exhausted" ||
        reason === "turn_limit" ||
        reason === "turn_limit_exhausted");
}
export function isClaudeRefusalResult(parsed) {
    if (!parsed)
        return false;
    ...
    const subtype = asString(parsed.subtype, "").trim().toLowerCase();
    if (subtype === "model_refusal" || subtype === "refusal")
        return true;
    const structuredStopReasons = [
        parsed.stop_reason,
        parsed.stopReason,
...
export function isClaudeUnknownSessionError(parsed) {
    ...
    const allMessages = [resultText, ...extractClaudeErrorMessages(parsed)]
        .map((msg) => msg.trim())
        .filter(Boolean);
    return allMessages.some((msg) => /no conversation found with session id|unknown session|session .* not found|not a valid UUID|--resume requires a valid session|is not a UUID|does not match any session title/i.test(msg));
}
export function isClaudePoisonedPreviousMessageIdError(parsed) {
    const resultText = asString(parsed.result, "").trim();
    const allMessages = [resultText, ...extractClaudeErrorMessages(parsed)]
        .map((msg) => msg.trim())
        .filter(Boolean);
    return allMessages.some((msg) => /diagnostics\.previous_message_id.*starts with `msg_`/i.test(msg));
}
export function isClaudeImageProcessingError(parsed) {
    const resultText = asString(parsed.result, "").trim();
    const allMessages = [resultText, ...extractClaudeErrorMessages(parsed)]
        .map((msg) => msg.trim())
        .filter(Boolean);
...
function parseClaudeResetClockTime(clockText, now, timeZoneHint) {
...
export function extractClaudeRetryNotBefore(input, now = new Date()) {
