...
import { asString, asNumber, asBoolean, asStringArray, parseObject, parseJson, applyPaperclipWorkspaceEnv, buildPaperclipEnv, readPaperclipRuntimeSkillEntries, readPaperclipIssueWorkModeFromContext, joinPromptSections, buildInvocationEnvForLogs, ensureAbsoluteDirectory, ensurePathInEnv, refreshPaperclipWorkspaceEnvForExecution, renderTemplate, renderPaperclipWakePrompt, rewriteWorkspaceCwdEnvVarsForExecution, shapePaperclipWorkspaceEnvForExecution, stringifyPaperclipWakePayload, DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE, } from "@paperclipai/adapter-utils/server-utils";
import { shellQuote } from "@paperclipai/adapter-utils/ssh";
import { parseClaudeStreamJson, describeClaudeFailure, detectClaudeLoginRequired, extractClaudeRetryNotBefore, isClaudeMaxTurnsResult, isClaudeRefusalResult, isClaudeTransientUpstreamError, isClaudeUnknownSessionError, isClaudePoisonedPreviousMessageIdError, isClaudeImageProcessingError, } from "./parse.js";
...
export async function execute(ctx) {
    ...
    const preparedExecutionTargetRuntime = executionTargetIsRemote
        ? await (async () => {
            await onLog("stdout", `[paperclip] Syncing workspace and Claude runtime assets to ${describeAdapterExecutionTarget(executionTarget)}.\n`);
    ...
    if (executionTargetIsSandbox && effort) {
        ...
        if (supportsEffort === false) {
            effectiveEffort = "";
            await onLog("stderr", `[paperclip] Claude CLI in the sandbox does not advertise --effort; omitting configured effort "${effort}". Upgrade the sandbox CLI/image to restore reasoning-effort control.\n`);
        }
    }
    ...
    if (runtimeSessionId && !isValidUuid) {
        await onLog("stdout", `[paperclip] Claude session "${runtimeSessionId}" is not a valid UUID and will not be passed to --resume.\n`);
    }
    ...
    if (runtimeSessionId && runtimePromptBundleKey.length > 0 && runtimePromptBundleKey !== promptBundle.bundleKey) {
        await onLog("stdout", `[paperclip] Claude session "${runtimeSessionId}" was saved for prompt bundle "${runtimePromptBundleKey}" and will not be resumed with "${promptBundle.bundleKey}".\n`);
    }
    ...
    const runAttempt = async (resumeSessionId) => {
        ...
        if (dangerouslySkipPermissions && executionTargetIsRemote) {
            commandNotes.push("Using a broad --allowedTools whitelist for remote execution so hosted targets do not inherit local Claude bypass permissions.");
        }
        if (attemptInstructionsFilePath && !resumeSessionId) {
    ...
    const toAdapterResult = (attempt, opts) => {
        ...
        const rawResolvedSessionId = parsedStream.sessionId ??
    ...
    try {
        ...
        const sessionErrorKind = sessionId &&
        ...
        if (sessionErrorKind !== null) {
            const reason = sessionErrorKind === "poisoned"
