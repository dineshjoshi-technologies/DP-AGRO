...
import { buildCatalogManifest, formatCatalogManifest, validateCatalog, } from "./catalog-builder.js";
const tempDirs = [];
describe("skills catalog manifest", () => {
    ...
    it("reuses the existing manifest entry when a pinned GitHub reference is temporarily unavailable", async () => {
        ...
        await fs.writeFile(path.join(packageDir, "generated", "catalog.json"), formatCatalogManifest({
            schemaVersion: 1,
    ...
    it("detects stale generated manifests", async () => {
        ...
        expect(result.errors).toContain("generated/catalog.json is stale. Run pnpm --filter @paperclipai/skills-catalog build:manifest.");
    });
});
async function createCatalogPackage() {
    ...
    await fs.writeFile(path.join(packageDir, "package.json"), JSON.stringify({ version: "0.3.1" }), "utf8");
    return packageDir;
}
async function writeSkill(packageDir, kind, category, slug, options) {
...
async function writeReference(packageDir, kind, category, slug, descriptor) {
    const skillDir = path.join(packageDir, "catalog", kind, category, slug);
    await fs.mkdir(skillDir, { recursive: true });
    await fs.writeFile(path.join(skillDir, "catalog-ref.json"), `${JSON.stringify(descriptor, null, 2)}\n`, "utf8");
}
//# sourceMappingURL=catalog-builder.test.js.map