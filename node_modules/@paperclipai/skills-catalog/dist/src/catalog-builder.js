...
import { asBoolean, isPlainRecord, asString, asStringArray, parseFrontmatterMarkdown, } from "./frontmatter.js";
...
export async function buildCatalogManifest(options) {
    ...
    for (const candidate of candidates) {
        const skill = await buildCatalogSkill(packageDir, candidate, errors, existingSkillsById.get(skillIdForCandidate(candidate)) ?? null);
        if (skill)
            skills.push(skill);
    }
...
async function discoverSkillCandidates(packageDir, errors) {
...
async function collectMisplacedSkillFiles(catalogDir, errors) {
    async function visit(dir) {
        for (const entry of await sortedDirEntries(dir)) {
            ...
            if (entry.name !== SKILL_ENTRYPOINT && entry.name !== CATALOG_REFERENCE_FILE)
                continue;
...
async function buildCatalogSkill(packageDir, candidate, errors, existingSkill) {
    ...
    if (!name)
        errors.push(`${prefix}/SKILL.md frontmatter must include name.`);
    const description = asString(parsed.frontmatter.description);
    if (!description)
        errors.push(`${prefix}/SKILL.md frontmatter must include description.`);
...
function skillIdForCandidate(candidate) {
    return `paperclipai:${candidate.kind}:${candidate.category}:${candidate.slug}`;
}
async function buildReferencedCatalogSkill(packageDir, candidate, errors, existingSkill) {
    const prefix = relativePackagePath(packageDir, candidate.absolutePath);
    validateSlug("category", candidate.category, prefix, errors);
    validateSlug("slug", candidate.slug, prefix, errors);
    ...
    if (!source)
        return null;
    const fallbackSkill = canReuseExistingReferencedSkill(existingSkill, candidate, source, toPosixPath(path.relative(packageDir, candidate.absolutePath)))
        ? existingSkill
        : null;
    ...
    const name = asString(parsed.frontmatter.name);
    if (!name)
        errors.push(`${source.url}/${SKILL_ENTRYPOINT} frontmatter must include name.`);
    const description = asString(parsed.frontmatter.description);
    if (!description)
        errors.push(`${source.url}/${SKILL_ENTRYPOINT} frontmatter must include description.`);
    const explicitKey = asString(parsed.frontmatter.key);
    if (explicitKey && explicitKey !== key) {
...
function canReuseExistingReferencedSkill(existingSkill, candidate, source, expectedPath) {
    if (!existingSkill || existingSkill.source?.type !== "github")
        return false;
    const existingSource = existingSkill.source;
    return (existingSkill.id === skillIdForCandidate(candidate) &&
...
async function readReferencedSkillDescriptor(descriptorPath, prefix, errors) {
    let raw;
    try {
    ...
    if (raw.files !== undefined && !descriptor.files)
        errors.push(`${prefix}/${CATALOG_REFERENCE_FILE} files must be an array of strings.`);
    if (raw.recommendedForRoles !== undefined && !descriptor.recommendedForRoles)
        errors.push(`${prefix}/${CATALOG_REFERENCE_FILE} recommendedForRoles must be an array of strings.`);
    if (raw.requires !== undefined && !descriptor.requires)
        errors.push(`${prefix}/${CATALOG_REFERENCE_FILE} requires must be an array of strings.`);
    if (raw.tags !== undefined && !descriptor.tags)
        errors.push(`${prefix}/${CATALOG_REFERENCE_FILE} tags must be an array of strings.`);
    return descriptor;
}
function buildCatalogSkillSource(descriptor, errors, prefix) {
    ...
    if (!/^[0-9a-f]{40}$/i.test(descriptor.commit) || sourcePath === null)
        return null;
    return {
        type: "github",
        hostname,
...
async function collectReferencedSkillFiles(source, includePatterns, prefix, errors) {
    ...
    for (const entry of tree) {
        ...
        if (!normalizedPatterns.some((pattern) => referencedPathMatches(relativePath, pattern)))
            continue;
    ...
    files.sort((a, b) => {
        if (a.path === SKILL_ENTRYPOINT)
            return -1;
        if (b.path === SKILL_ENTRYPOINT)
            return 1;
        return a.path.localeCompare(b.path);
    });
    return files;
}
async function fetchGitHubTree(source, prefix, errors) {
...
async function readReferencedFileText(source, relativePath, prefix, errors) {
    const bytes = await fetchReferencedFileBytes(source, relativePath, prefix, errors);
    return bytes ? bytes.toString("utf8") : null;
}
async function fetchReferencedFileBytes(source, relativePath, prefix, errors) {
...
async function collectSkillFiles(packageDir, skillDir, prefix, errors) {
    const files = [];
...
function deriveTrustLevel(files) {
    if (files.some((file) => file.kind === "script"))
        return "scripts_executables";
    if (files.some((file) => file.kind === "asset" || file.kind === "other"))
        return "assets";
...
function collectUniquenessErrors(skills, errors) {
...
function collectDuplicateErrors(fieldSkills, field, errors) {
    const seen = new Map();
...
function normalizeReferencedPath(input) {
...
function referencedPathMatches(relativePath, pattern) {
    if (pattern.endsWith("/**")) {
...
function githubApiBase(hostname) {
    const normalized = hostname.toLowerCase();
    return normalized === "github.com" || normalized === "www.github.com"
        ? "https://api.github.com"
        : `https://${hostname}/api/v3`;
}
function rawGitHubUrl(source, relativePath) {
