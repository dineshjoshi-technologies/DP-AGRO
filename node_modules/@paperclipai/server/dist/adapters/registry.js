...
import { execute as acpxExecute, testEnvironment as acpxTestEnvironment, sessionCodec as acpxSessionCodec, getConfigSchema as getAcpxConfigSchema, listAcpxSkills, syncAcpxSkills, } from "@paperclipai/adapter-acpx-local/server";
import { agentConfigurationDoc as acpxAgentConfigurationDoc, models as acpxModels, } from "@paperclipai/adapter-acpx-local";
import { execute as claudeExecute, listClaudeSkills, syncClaudeSkills, listClaudeModels, refreshClaudeModels, testEnvironment as claudeTestEnvironment, sessionCodec as claudeSessionCodec, getQuotaWindows as claudeGetQuotaWindows, } from "@paperclipai/adapter-claude-local/server";
import { agentConfigurationDoc as claudeAgentConfigurationDoc, models as claudeModels, modelProfiles as claudeModelProfiles, } from "@paperclipai/adapter-claude-local";
...
import { agentConfigurationDoc as codexAgentConfigurationDoc, models as codexModels, modelProfiles as codexModelProfiles, } from "@paperclipai/adapter-codex-local";
...
import { agentConfigurationDoc as cursorAgentConfigurationDoc, models as cursorModels, modelProfiles as cursorModelProfiles, } from "@paperclipai/adapter-cursor-local";
import { execute as cursorCloudExecute, getConfigSchema as getCursorCloudConfigSchema, sessionCodec as cursorCloudSessionCodec, testEnvironment as cursorCloudTestEnvironment, } from "@paperclipai/adapter-cursor-cloud/server";
...
import { agentConfigurationDoc as geminiAgentConfigurationDoc, models as geminiModels, modelProfiles as geminiModelProfiles, } from "@paperclipai/adapter-gemini-local";
import { execute as grokExecute, listGrokSkills, syncGrokSkills, testEnvironment as grokTestEnvironment, sessionCodec as grokSessionCodec, } from "@paperclipai/adapter-grok-local/server";
...
import { agentConfigurationDoc as openCodeAgentConfigurationDoc, models as openCodeModels, modelProfiles as openCodeModelProfiles, } from "@paperclipai/adapter-opencode-local";
...
import { agentConfigurationDoc as piAgentConfigurationDoc, modelProfiles as piModelProfiles, } from "@paperclipai/adapter-pi-local";
import { BUILTIN_ADAPTER_TYPES } from "./builtin-adapter-types.js";
import { buildExternalAdapters } from "./plugin-loader.js";
import { getDisabledAdapterTypes } from "../services/adapter-plugin-store.js";
import { processAdapter } from "./process/index.js";
import { httpAdapter } from "./http/index.js";
function readConfiguredCommand(config, fallback) {
...
function buildNpmRuntimeCommandSpec(config, fallbackCommand, packageName) {
...
function buildCursorRuntimeCommandSpec(config) {
...
function dedupeAdapterModels(models) {
    const seen = new Set();
    const result = [];
...
function prefixAdapterModelLabels(models, provider) {
 ...
 */
export function resolveExternalAdapterRegistration(externalAdapter) {
    return {
        ...externalAdapter,
        sessionManagement: externalAdapter.sessionManagement
 ...
 */
let adapterModelsEnvCache = null;
function getDeclaredAdapterModels() {
    ...
    try {
        ...
        console.error("[paperclip] Invalid PAPERCLIP_ADAPTER_MODELS; ignoring declared model lists:", err);
    }
...
export async function refreshAdapterModels(type) {
    const adapter = findActiveServerAdapter(type);
    if (!adapter)
        return [];
...
export async function listAdapterModelProfiles(type) {
    const adapter = findActiveServerAdapter(type);
    if (!adapter)
        return [];
