...
import { instanceDatabaseBackupRoutes, } from "./routes/instance-database-backups.js";
...
export function isDatabaseConnectionUnavailableError(err) {
    const error = err;
...
export function resolveViteHmrHost(bindHost) {
    const normalized = bindHost.trim().toLowerCase();
    if (normalized === "0.0.0.0" || normalized === "::")
        return undefined;
    return bindHost;
}
export function shouldServeViteDevHtml(req) {
    const pathname = req.path;
    if (VITE_DEV_STATIC_PATHS.has(pathname))
        return false;
...
export function shouldEnablePrivateHostnameGuard(opts) {
    return (opts.deploymentExposure === "private" &&
...
export async function createApp(db, opts) {
    ...
    const devWatcher = createPluginDevWatcher(lifecycle, async (pluginId) => (await pluginRegistry.getById(pluginId))?.packagePath ?? null);
    ...
    const ensureBundledKubernetesPlugin = async () => {
        const KUBERNETES_PLUGIN_KEY = "paperclip.kubernetes-sandbox-provider";
        const pluginPath = process.env["PAPERCLIP_KUBERNETES_PLUGIN_PATH"] ??
        ...
        try {
            ...
            if (existing) {
                logger.info({ pluginKey: KUBERNETES_PLUGIN_KEY, status: existing.status }, "kubernetes sandbox plugin already installed; skipping auto-install");
                return;
            }
            ...
            if (!fs.existsSync(path.join(pluginPath, "dist", "manifest.js"))) {
                logger.info({ pluginPath }, "kubernetes sandbox plugin bundle not present; skipping auto-install");
                return;
            }
            ...
            if (installed) {
                ...
                logger.info({ pluginId: installed.id, pluginKey: installed.pluginKey }, "kubernetes sandbox plugin auto-installed and loaded");
            }
            else {
            ...
            logger.error({ err }, "Failed to auto-install the kubernetes sandbox plugin; continuing boot (degraded: kubernetes provider unavailable)");
        }
    };
