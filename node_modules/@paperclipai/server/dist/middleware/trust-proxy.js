...
function isValidSubnetToken(token) {
    if (NAMED_SUBNETS.has(token))
        return true;
    ...
    if (token.includes(":") && IPV6_RE.test(token))
        return true;
    return false;
}
/**
 ...
 */
export function parseTrustProxyEnv(raw) {
    if (raw === undefined)
        return undefined;
    ...
    if (value === "true")
        return true;
    if (STRICT_POS_INT_RE.test(value))
        return Number(value);
    ...
    if (/^\s*\d+\s*$/.test(raw)) {
        throw new Error(`TRUST_PROXY: invalid integer value ${JSON.stringify(raw)} — use a positive integer with no leading zeros or whitespace`);
    }
    const tokens = value
        .split(",")
        .map((t) => t.trim())
    ...
    for (const token of tokens) {
        if (!isValidSubnetToken(token)) {
            throw new Error(`TRUST_PROXY: unrecognized token ${JSON.stringify(token)} — expected one of {loopback, linklocal, uniquelocal} or a CIDR like 10.0.0.0/8 or fd00::/8`);
        }
    }
    return tokens;
}
 ...
 */
export function applyTrustProxy(app, value) {
    if (value === undefined)
        return;
