...
function resolveWorktreeRuntimeContext(env, overrideConfigPath) {
    if (env.PAPERCLIP_IN_WORKTREE !== "true")
        return null;
    ...
    const persistedConfigLooksStale = persistedConfigPath !== null &&
    ...
    const worktreeName = nonEmpty(stablePersistedEnv.PAPERCLIP_WORKTREE_NAME) ??
        nonEmpty(env.PAPERCLIP_WORKTREE_NAME) ??
        path.basename(worktreeRoot);
    const instanceId = nonEmpty(stablePersistedEnv.PAPERCLIP_INSTANCE_ID) ??
        nonEmpty(env.PAPERCLIP_INSTANCE_ID) ??
        sanitizeWorktreeInstanceId(worktreeName);
    const homeDir = resolveHomeAwarePath(nonEmpty(stablePersistedEnv.PAPERCLIP_HOME) ??
...
function resolveRepoManagedWorktreesRoot(worktreeRoot) {
...
function collectSiblingWorktreePorts(context) {
    ...
    if (fs.existsSync(instancesDir)) {
        for (const entry of fs.readdirSync(instancesDir, { withFileTypes: true })) {
            if (!entry.isDirectory() || entry.name === context.instanceId)
                continue;
    ...
    if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
        for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
            ...
            if (path.resolve(siblingConfigPath) === path.resolve(context.configPath))
                continue;
            if (fs.existsSync(siblingConfigPath)) {
                siblingConfigPaths.add(siblingConfigPath);
            }
    ...
    for (const siblingConfigPath of siblingConfigPaths) {
        try {
            const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8"));
            if (Number.isInteger(siblingConfig.server.port) && siblingConfig.server.port > 0) {
                serverPorts.add(siblingConfig.server.port);
            }
            if (siblingConfig.database.mode === "embedded-postgres" &&
...
function findNextUnclaimedPort(preferredPort, claimedPorts) {
...
function buildIsolatedWorktreeConfig(config, context, portOverrides) {
    const serverPort = portOverrides?.serverPort ?? config.server.port;
    const databasePort = config.database.mode === "embedded-postgres"
...
function needsWorktreeConfigRepair(config, context) {
...
export function applyRuntimePortSelectionToConfig(config, input) {
    let changed = false;
    ...
    if (input.allowDatabasePortWrite !== false &&
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles() {
    ...
    if (fs.existsSync(context.configPath)) {
        try {
            const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8"));
            const siblingPorts = collectSiblingWorktreePorts(context);
            const hasSiblingPortCollision = siblingPorts.serverPorts.has(parsed.server.port) ||
            ...
            if (needsWorktreeConfigRepair(parsed, context) || hasSiblingPortCollision) {
                const selectedServerPort = findNextUnclaimedPort(parsed.server.port === 3100 ? 3101 : parsed.server.port, siblingPorts.serverPorts);
                const selectedDatabasePort = parsed.database.mode === "embedded-postgres"
                    ? findNextUnclaimedPort(parsed.database.embeddedPostgresPort === 54329
                        ? 54330
                        : parsed.database.embeddedPostgresPort, new Set([...siblingPorts.databasePorts, selectedServerPort]))
                    : undefined;
...
export function maybePersistWorktreeRuntimePorts(input) {
    const context = resolveWorktreeRuntimeContext(process.env);
    if (!context || !fs.existsSync(context.configPath))
        return;
    let fileConfig;
    try {
        fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8"));
    }
    catch {
        return;
    }
