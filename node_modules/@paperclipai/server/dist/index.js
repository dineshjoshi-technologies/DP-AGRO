...
import { createDb, ensurePostgresDatabase, formatEmbeddedPostgresError, getPostgresDataDirectory, inspectMigrations, applyPendingMigrations, createEmbeddedPostgresLogBuffer, prepareEmbeddedPostgresNativeRuntime, reconcilePendingMigrationHistory, formatDatabaseBackupResult, runDatabaseBackup, authUsers, companies, companyMemberships, instanceUserRoles, } from "@paperclipai/db";
import detectPort from "detect-port";
import { createApp } from "./app.js";
import { loadConfig } from "./config.js";
import { logger } from "./middleware/logger.js";
import { setupLiveEventsWebSocketServer } from "./realtime/live-events-ws.js";
import { feedbackService, backfillPrincipalAccessCompatibility, bootstrapExecutionPolicyFromEnv, heartbeatService, instanceSettingsService, reconcileCloudUpstreamRunsOnStartup, reconcileCodexLocalManagedHomesOnStartup, reconcilePersistedRuntimeServicesOnStartup, routineService, } from "./services/index.js";
import { parseAdapterRegistryEnv, reconcileAdapterAvailability, } from "./services/adapter-registry-bootstrap.js";
...
export async function startServer() {
    ...
    function assertCloudDatabaseContract() {
        if (config.deploymentMode !== "authenticated" || config.deploymentExposure !== "public") {
            return;
        }
        if (!config.databaseUrl) {
            throw new Error("authenticated public deployments require DATABASE_URL or config.database.connectionString; refusing embedded PostgreSQL fallback");
        }
        if (!isPostgresConnectionString(config.databaseUrl)) {
            throw new Error("authenticated public deployments require DATABASE_URL to be a postgres/postgresql connection string");
        }
    }
    ...
    const accessBackfill = await backfillPrincipalAccessCompatibility(db);
    ...
    void reconcileCloudUpstreamRunsOnStartup(db)
        .then((result) => {
        if (result.reconciled > 0) {
            logger.warn({ reconciled: result.reconciled }, "reconciled cloud upstream runs from a previous server process");
        }
    })
        .catch((err) => {
    ...
    void reconcileCodexLocalManagedHomesOnStartup(db)
        ...
        if (result.seeded > 0 || result.failed > 0) {
            logger.warn({ seeded: result.seeded, failed: result.failed, scanned: result.scanned }, "reconciled codex_local managed homes (backfilled missing auth)");
        }
        if (result.sourceAuthMissing > 0) {
            logger.warn({ sourceAuthMissing: result.sourceAuthMissing, scanned: result.scanned }, "could not backfill codex_local managed homes because shared Codex auth is missing");
        }
    })
        .catch((err) => {
    ...
    try {
        const policyResult = await bootstrapExecutionPolicyFromEnv(db);
        if (policyResult) {
            logger.warn({
    ...
    if (config.heartbeatSchedulerEnabled) {
        ...
        const routines = routineService(db, { pluginWorkerManager });
        ...
        await (async () => {
            for (let attempt = 1; attempt <= 2; attempt++) {
                try {
                    ...
                    logger.info({ reaped: result.reaped, runIds: result.runIds }, "startup reap of orphaned heartbeat runs complete");
                    break;
                }
                catch (err) {
                    if (attempt < 2) {
                        ...
                        logger.error({ err }, "startup reap of orphaned heartbeat runs failed after retry — periodic reaper will serve as degraded backstop");
                    }
                }
            }
        ...
        setInterval(() => {
            ...
            if (sweptRuntimeStatuses > 0) {
                logger.info({ swept: sweptRuntimeStatuses }, "heartbeat runtime-status sweeper cleared expired entries");
            }
            void heartbeat
                .tickTimers(new Date())
                .then((result) => {
    ...
    });
    {
        const shutdown = async (signal) => {
            ...
            const appShutdown = app.locals?.paperclipShutdown;
            appShutdown?.();
