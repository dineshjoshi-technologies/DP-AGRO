...
import { createIssueTreeHoldSchema, isUuidLike, previewIssueTreeControlSchema, releaseIssueTreeHoldSchema, } from "@paperclipai/shared";
...
async function waitForRunCancellationTasks(tasks) {
    let timeout = null;
    try {
        await Promise.race([
            Promise.all(tasks),
...
export function issueTreeControlRoutes(db) {
    const router = Router();
    const issuesSvc = issueService(db);
    const treeControlSvc = issueTreeControlService(db);
    const heartbeat = heartbeatService(db);
    async function resolveRootIssue(req) {
        const rootIssueId = req.params.id;
    ...
    router.post("/issues/:id/tree-holds", validate(createIssueTreeHoldSchema), async (req, res) => {
        ...
        if (result.hold.mode === "pause" || result.hold.mode === "cancel") {
            ...
            const cancelledWakeups = await treeControlSvc.cancelUnclaimedWakeupsForTree(root.companyId, root.id, result.hold.mode === "pause"
        ...
        if (result.hold.mode === "restore") {
            ...
            const wakeAgents = typeof req.body.metadata === "object"
                ...
                && req.body.metadata.wakeAgents === true;
            if (wakeAgents) {
                for (const restoredIssue of statusUpdate.updatedIssues) {
                    if (!restoredIssue.assigneeAgentId)
                        continue;
    ...
    router.get("/issues/:id/tree-holds", async (req, res) => {
        ...
        const holds = await treeControlSvc.listHolds(root.companyId, root.id, {
            ...
            mode: modeParam === "pause" || modeParam === "resume" || modeParam === "cancel" || modeParam === "restore"
                ? modeParam
                : undefined,
            includeMembers,
        });
