...
function requireBoardUserId(req, res) {
    assertBoard(req);
    if (!req.actor.userId) {
...
export function sidebarPreferenceRoutes(db) {
    const router = Router();
    ...
    router.put("/companies/:companyId/sidebar-preferences/me", validate(upsertSidebarOrderPreferenceSchema), async (req, res) => {
        const companyId = req.params.companyId;
        assertCompanyAccess(req, companyId);
        const userId = requireBoardUserId(req, res);
        if (!userId)
