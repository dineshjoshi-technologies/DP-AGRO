...
import { buildPromotedSourceTrust, isLowTrustQuarantined, redactQuarantinedBodyForHigherTrust, resolveActorSourceTrustForIssue, sanitizeQuarantinedCommentForHigherTrust, } from "../services/source-trust.js";
import { LOW_TRUST_ISSUE_ANCESTRY_MAX_DEPTH, resolveCoreTrustPreset, } from "../services/trust-preset-resolver.js";
...
async function listIssueLinkedCases(db, companyId, issueId) {
    const rows = await db
        .select({
        ...
        .where(and(eq(pipelineCaseIssueLinks.companyId, companyId), eq(pipelineCaseIssueLinks.issueId, issueId), eq(pipelineCases.companyId, companyId), eq(pipelines.companyId, companyId)));
    return rows.map((row) => ({
        id: row.case.id,
...
function noopTaskWatchdogService() {
    return {
...
function actorMatchesExecutionParticipant(actor, participant) {
    if (!participant)
        return false;
    ...
    if (participant.type !== actor.actorType)
        return false;
...
const APPROVAL_NEGATION_REGEX = /\b(?:NOT|REJECT(?:ED|ING|S)?|DENY|DENIED|DENYING|BLOCK(?:ED|ING|S)?|CHANGES?\s+REQUESTED)\b/i;
function isApprovalReviewComment(body) {
...
async function assertCanManageIssueMonitor(accessSvc, req, companyId, assigneeAgentId, monitorChanged) {
    if (!monitorChanged)
        return;
...
function shouldImplicitlyMoveCommentedIssueToTodo(input) {
    ...
    if (typeof input.actorRunId === "string"
...
function isAssigneeSelfCommentOnTerminalIssue(input) {
    if (!input.hasCommentBody)
        return false;
    if (input.resumeRequested)
        return false;
    if (!isClosedIssueStatus(input.issueStatus))
        return false;
    if (typeof input.assigneeAgentId !== "string" || input.assigneeAgentId.length === 0)
        return false;
...
export function issueRoutes(db, storage, opts = {}) {
    ...
    const taskWatchdogFactory = Object.prototype.hasOwnProperty.call(serviceIndex, "taskWatchdogService")
    ...
    async function queueTaskWatchdogEvaluation(issue, runId) {
    ...
    async function sourceTrustForActorWrite(issue, actor) {
        return resolveActorSourceTrustForIssue({ db, issue, actor });
    }
    function hasExplicitIssueWorkspaceCreateSelection(input) {
    ...
    async function resolveRunIssueWorkspaceInheritanceSource(companyId, actor) {
        ...
        const run = await db
            ...
            .from(heartbeatRuns)
            .where(and(eq(heartbeatRuns.id, actor.runId), eq(heartbeatRuns.companyId, companyId)))
            .then((rows) => rows[0] ?? null);
        if (!run || run.agentId !== actor.agentId)
            return null;
    ...
    async function resolveAgentTrustForIssue(input, companyId, issue) {
        if (!input.agentId)
            return null;
    ...
    async function actorIsLowTrustReview(req, companyId, issue) {
        if (req.actor.type !== "agent")
            return false;
    ...
    async function assertLowTrustControlPlaneDenied(req, res, companyId, issue) {
        if (!(await actorIsLowTrustReview(req, companyId, issue)))
            return false;
        res.status(403).json({ error: "Low-trust actors cannot use this control-plane surface" });
        return true;
    }
    async function shouldRedactLowTrustForHeartbeatContext(issue, actor) {
    ...
    async function lookupLowTrustSourceArtifact(input) {
        if (input.artifactKind === "issue") {
            ...
            const sourceIssue = await db
                ...
                .then((rows) => rows[0] ?? null);
            if (!sourceIssue || row.companyId !== sourceIssue.companyId)
                return null;
            if (row.id !== input.issueId) {
                ...
                for (let depth = 0; cursor && depth < LOW_TRUST_ISSUE_ANCESTRY_MAX_DEPTH; depth += 1) {
                    ...
                    const parent = await db
                        ...
                        .then((rows) => rows[0] ?? null);
                    if (!parent || parent.companyId !== row.companyId)
                        return null;
    ...
    async function cancelScheduledRetrySupersededByComment(input) {
        ...
        try {
            ...
            logger.error({ err, issueId: input.issue.id, runId: scheduledRetryRunId }, "failed to cancel scheduled retry superseded by issue comment");
            throw err;
        }
    }
    ...
    function shouldIncludeDocumentAnnotations(req) {
        if (req.query.includeAnnotations === "false" || req.query.includeAnnotations === "0")
            return false;
        return req.actor.type === "agent" || parseBooleanQuery(req.query.includeAnnotations);
    }
    function shouldIncludeDocumentAnnotationComments(req) {
        return parseBooleanQuery(req.query.includeAnnotationComments);
    }
    function annotationActorInput(req) {
    ...
    async function assertCanAssignTasks(req, companyId, assignmentScope) {
        assertCompanyAccess(req, companyId);
    ...
    async function assertTaskBridgeCreateAllowed(req, companyId, assignmentScope) {
        if (!isTaskBridgeKeyActor(req))
            return;
        await assertCanAssignTasks(req, companyId, assignmentScope);
    }
    async function decideIssueAccess(req, issue, action) {
    ...
    async function assertIssueReadAllowed(req, res, issue) {
    ...
    async function assertAgentIssueCommentAllowed(req, res, issue) {
        if (req.actor.type !== "agent")
            return true;
    ...
    async function rejectTaskWatchdogConfigMutation(req, res) {
        if (req.actor.type !== "agent")
            return false;
    ...
    async function assertTaskWatchdogIssueMutationAllowed(req, res, issue, opts = {}) {
        if (req.actor.type !== "agent")
            return true;
        const scope = await resolveTaskWatchdogMutationScope(db, req.actor);
        if (scope.kind === "none")
    ...
    async function rejectAgentIssueThreadInteractionResolution(req, res, issue) {
        if (req.actor.type !== "agent")
            return false;
        if (req.actor.runId &&
    ...
    async function assertTaskWatchdogCreateIssueAllowed(req, res, companyId, parent) {
        if (req.actor.type !== "agent")
            return true;
        const scope = await resolveTaskWatchdogMutationScope(db, req.actor);
        if (scope.kind === "none")
    ...
    async function resolveWatchdogFollowUpSerializationContext(req, parent) {
    ...
    function mergeIssueBlockerIds(existing, blockerIssueId) {
    ...
    async function findCurrentSerializedWatchdogChild(parent) {
        const children = await db
            ...
            .from(issueRows)
            .where(and(eq(issueRows.companyId, parent.companyId), eq(issueRows.parentId, parent.id), inArray(issueRows.status, ["todo", "in_progress", "in_review", "blocked"]), isNull(issueRows.hiddenAt)))
    ...
    async function blockWatchdogParentOnCurrentChild(input) {
        if (!input.watchdogParentIssueId || !input.currentChildIssueId)
            return;
        const watchdogParent = await svc.getById(input.watchdogParentIssueId);
        if (!watchdogParent || watchdogParent.originKind !== TASK_WATCHDOG_ORIGIN_KIND)
            return;
        if (watchdogParent.status !== "in_progress" && watchdogParent.status !== "blocked")
            return;
        const relations = await svc.getRelationSummaries(watchdogParent.id);
        const nextBlockedByIssueIds = mergeIssueBlockerIds(relations.blockedBy?.map((relation) => relation.id) ?? [], input.currentChildIssueId);
    ...
    function normalizeWatchdogDiscovery(input) {
        if (!input || typeof input !== "object" || Array.isArray(input))
            return null;
        const record = input;
        const kind = typeof record.kind === "string" &&
            ISSUE_WATCHDOG_DISCOVERY_KINDS.includes(record.kind)
            ? record.kind
            : null;
        if (!kind)
            return null;
        const evidenceMarkdown = typeof record.evidenceMarkdown === "string" && record.evidenceMarkdown.trim().length > 0
    ...
    async function resolveTaskWatchdogProductBugFollowUp(req, res, companyId, discovery) {
        if (!discovery)
            return null;
        if (req.actor.type !== "agent") {
            res.status(403).json({
    ...
    function isStatusOnlyCheapRecoveryContext(contextSnapshot) {
    ...
    function requestsCheapIssueAssigneeModelProfile(input) {
        const overrides = input.assigneeAdapterOverrides;
        return !!overrides &&
            typeof overrides === "object" &&
            !Array.isArray(overrides) &&
            overrides.modelProfile === "cheap";
    }
    async function loadActorRunContext(req, companyId) {
        if (req.actor.type !== "agent")
            return null;
        const runId = req.actor.runId?.trim();
        if (!runId)
    ...
    async function assertCheapRecoveryIssueAssigneeProfileAllowed(req, res, issue, input) {
        if (!requestsCheapIssueAssigneeModelProfile(input))
            return true;
    ...
    async function assertDeliverableMutationAllowedByRunContext(req, res, issue) {
        const run = await loadActorRunContext(req, issue.companyId);
        if (!run)
            return true;
        if (!isStatusOnlyCheapRecoveryContext(run.contextSnapshot))
            return true;
        res.status(403).json({
    ...
    async function assertApprovalMutationAllowedByRunContext(req, res, issue) {
        const run = await loadActorRunContext(req, issue.companyId);
        if (!run)
            return true;
        if (!isStatusOnlyCheapRecoveryContext(run.contextSnapshot))
    ...
    async function loadWorkProductRunAttribution(runId) {
        return await db
            .select({
            id: heartbeatRuns.id,
            companyId: heartbeatRuns.companyId,
    ...
    async function resolveWorkProductCreatedByRunId(req, res, companyId, input, mode) {
    ...
    async function normalizeIssueAssigneeAgentReference(companyId, rawAssigneeAgentId) {
        ...
        if (resolved.agent.orgChainHealth?.status === "invalid_org_chain") {
            throw conflict(resolved.agent.orgChainHealth?.repairGuidance ??
    ...
    async function destroyReusableSandboxLeasesForTerminalIssue(issue) {
        try {
            ...
            logger.warn({ err, issueId: issue.id, executionWorkspaceId: issue.executionWorkspaceId ?? null }, "failed to destroy reusable sandbox leases for terminal issue");
        }
    }
    ...
    router.get("/companies/:companyId/issues/count", async (req, res) => {
        ...
        if (!(await actorCanReadCompanyScope(req, companyId))) {
            ...
            while (true) {
                ...
                if (rows.length < ISSUE_LIST_MAX_LIMIT)
                    break;
                offset += rows.length;
            }
    ...
    router.get("/issues/:id/heartbeat-context", async (req, res) => {
        ...
        const safeWakeComment = wakeComment && wakeComment.issueId === issue.id
            ? redactLowTrust
                ? sanitizeQuarantinedCommentForHigherTrust(wakeComment)
                : wakeComment
            : null;
        const safeContinuationSummary = continuationSummary && redactLowTrust
    ...
    router.get("/issues/:id", async (req, res) => {
        ...
        if (!(await assertIssueReadAllowed(req, res, issue)))
            return;
        const [{ project, goal }, ancestors, mentionedProjectIds, documentPayload, relations, blockerAttention, productivityReview, referenceSummary, successfulRunHandoffStates, scheduledRetry, activeRecoveryAction, linkedCases,] = await Promise.all([
            resolveIssueProjectAndGoal(issue),
            svc.getAncestors(issue.id),
    ...
    router.put("/issues/:id/watchdog", validate(upsertIssueWatchdogSchema), async (req, res) => {
        ...
        if (!(await assertAgentIssueMutationAllowed(req, res, issue)))
            return;
        if (await rejectTaskWatchdogConfigMutation(req, res))
            return;
        if (await assertLowTrustControlPlaneDenied(req, res, issue.companyId, issue))
            return;
        const actor = getActorInfo(req);
    ...
    router.post("/issues/:id/documents/:key/annotations", validate(createDocumentAnnotationThreadSchema), async (req, res) => {
        const id = req.params.id;
        const issue = await svc.getById(id);
        if (!issue) {
            res.status(404).json({ error: "Issue not found" });
    ...
    router.get("/issues/:id/documents/:key/annotations/:threadId", async (req, res) => {
        ...
        const thread = await documentAnnotationsSvc.getThreadForIssueDocument(issue.id, keyParsed.data, req.params.threadId);
        if (!thread) {
    ...
    router.post("/issues/:id/documents/:key/annotations/:threadId/comments", validate(createDocumentAnnotationCommentSchema), async (req, res) => {
        const id = req.params.id;
        const issue = await svc.getById(id);
        if (!issue) {
            res.status(404).json({ error: "Issue not found" });
        ...
        const { actor, annotationActor } = annotationActorInput(req);
        const referenceSummaryBefore = await issueReferencesSvc.listIssueReferenceSummary(issue.id);
        const comment = await documentAnnotationsSvc.addComment(issue.id, keyParsed.data, req.params.threadId, req.body, annotationActor);
    ...
    router.patch("/issues/:id/documents/:key/annotations/:threadId", validate(updateDocumentAnnotationThreadSchema), async (req, res) => {
        const id = req.params.id;
        const issue = await svc.getById(id);
        if (!issue) {
            res.status(404).json({ error: "Issue not found" });
        ...
        const { actor, annotationActor } = annotationActorInput(req);
        const thread = await documentAnnotationsSvc.updateThread(issue.id, keyParsed.data, req.params.threadId, req.body, annotationActor);
        await logActivity(db, {
            companyId: issue.companyId,
            actorType: actor.actorType,
            actorId: actor.actorId,
    ...
    router.put("/issues/:id/documents/:key", validate(upsertIssueDocumentSchema), async (req, res) => {
        ...
        const redirectedFromLockedDocument = "redirectedFromLockedDocument" in result ? result.redirectedFromLockedDocument : null;
    ...
    router.delete("/issues/:id/documents/:key", async (req, res) => {
        ...
        const referenceSummaryAfter = await issueReferencesSvc.listIssueReferenceSummary(issue.id);
        if (removed)
            await externalObjectsSvc.syncDocumentSafely(removed.id);
        const referenceDiff = issueReferencesSvc.diffIssueReferenceSummary(referenceSummaryBefore, referenceSummaryAfter);
        const actor = getActorInfo(req);
        await logActivity(db, {
            companyId: issue.companyId,
            ...
            const updatedSource = await (async () => {
                if (req.body.sourceArtifactKind === "issue") {
                    return tx
                        .update(issueRows)
                        .set(markPromoted)
                        .where(and(eq(issueRows.id, req.body.sourceArtifactId), eq(issueRows.sourceTrust, sourceTrust)))
                ...
                if (req.body.sourceArtifactKind === "comment") {
                    return tx
                        ...
                        .set(markPromoted)
                        .where(and(eq(issueComments.id, req.body.sourceArtifactId), eq(issueComments.issueId, issue.id), eq(issueComments.sourceTrust, sourceTrust)))
                ...
                return tx
                    ...
                    .set(markPromoted)
                    .where(and(eq(issueWorkProducts.id, req.body.sourceArtifactId), eq(issueWorkProducts.issueId, issue.id), eq(issueWorkProducts.sourceTrust, sourceTrust)))
    ...
    router.patch("/work-products/:id", validate(updateIssueWorkProductSchema), async (req, res) => {
        ...
        if (createdByRunId === undefined && Object.prototype.hasOwnProperty.call(req.body, "createdByRunId"))
            return;
        if (createdByRunId !== undefined)
            patch.createdByRunId = createdByRunId;
    ...
    router.post("/companies/:companyId/issues", applyCreateIssueStatusDefault, validate(createIssueSchema), async (req, res) => {
        ...
        if (req.actor.type === "agent" && effectiveParentId) {
            ...
            if (!isTaskBridgeKeyActor(req) && !(await assertIssueReadAllowed(req, res, createParent)))
                return;
        }
        if (!watchdogProductBugFollowUp &&
            !(await assertTaskWatchdogCreateIssueAllowed(req, res, companyId, createParent)))
            return;
        const normalizedAssigneeAgentId = await normalizeIssueAssigneeAgentReference(companyId, rawCreateBody.assigneeAgentId);
        const actor = getActorInfo(req);
        ...
        if (!(await assertCheapRecoveryIssueAssigneeProfileAllowed(req, res, { companyId }, createBody)))
            return;
        ...
        const executionPolicy = applyActorMonitorScheduledBy(normalizeIssueExecutionPolicy(createBody.executionPolicy), actor.actorType);
    ...
    router.post("/issues/:id/children", applyCreateIssueStatusDefault, validate(createChildIssueSchema), async (req, res) => {
        ...
        if (!isTaskBridgeKeyActor(req) && !(await assertIssueReadAllowed(req, res, parent)))
            return;
        if (!(await assertTaskWatchdogCreateIssueAllowed(req, res, parent.companyId, parent)))
            return;
        if (await assertLowTrustControlPlaneDenied(req, res, parent.companyId, parent))
            return;
        assertNoAgentHostWorkspaceCommandMutation(req, collectIssueWorkspaceCommandPaths(req.body));
        const normalizedAssigneeAgentId = await normalizeIssueAssigneeAgentReference(parent.companyId, req.body.assigneeAgentId);
        const createBody = {
            ...req.body,
            ...(normalizedAssigneeAgentId !== undefined ? { assigneeAgentId: normalizedAssigneeAgentId } : {}),
        };
    ...
    router.post("/issues/:id/accepted-plan-decompositions", validate(createAcceptedPlanDecompositionSchema), async (req, res) => {
        ...
        for (const child of req.body.children) {
            const normalizedAssigneeAgentId = await normalizeIssueAssigneeAgentReference(sourceIssue.companyId, child.assigneeAgentId);
            const childBody = {
                ...child,
                ...(normalizedAssigneeAgentId !== undefined ? { assigneeAgentId: normalizedAssigneeAgentId } : {}),
            };
            ...
            if (!(await assertCheapRecoveryIssueAssigneeProfileAllowed(req, res, sourceIssue, childBody)))
                return;
        ...
        for (const child of requestedChildren) {
            const executionPolicy = applyActorMonitorScheduledBy(normalizeIssueExecutionPolicy(child.executionPolicy), actor.actorType);
        ...
        const existingSerializedChild = serializationContext
            ...
            : null;
        const serializedBlockedChildIds = new Set();
        if (serializationContext) {
            for (let index = 0; index < normalizedChildren.length; index += 1) {
                const blockerIssueId = index === 0
    ...
    router.patch("/issues/:id", validate(updateIssueRouteSchema), async (req, res) => {
        ...
        if (!(await assertAgentIssueMutationAllowed(req, res, existing)))
            return;
        if (!(await assertCheapRecoveryIssueAssigneeProfileAllowed(req, res, existing, req.body)))
            return;
        const actor = getActorInfo(req);
        ...
        const scheduledRetryForHumanComment = shouldHumanCommentResumeInProgressScheduledRetry({
            ...
            : null;
        const shouldResumeInProgressScheduledRetry = !!scheduledRetryForHumanComment &&
        ...
        const effectiveMoveToTodoRequested = !assigneeSelfCommentOnTerminal &&
            (explicitMoveToTodoRequested ||
                (!!commentBody &&
        ...
        if (interruptRequested) {
            ...
            if (runToInterrupt) {
                const cancelled = await heartbeat.cancelRun(runToInterrupt.id, "Interrupted by board comment", operatorInterruptCancelOptions({ issueId: existing.id, actor }));
                if (cancelled) {
        ...
        let cancelledScheduledRetryRunId = null;
        if (commentBody &&
            shouldResumeInProgressScheduledRetry &&
            updateFields.status === "todo") {
        ...
        const scheduledRetrySupersededByComment = shouldResumeInProgressScheduledRetry &&
            previous.status !== undefined &&
    ...
    router.delete("/issues/:id/comments/:commentId", async (req, res) => {
        ...
        let annotationCleanup = { deletedCommentIds: [], resolvedThreadIds: [] };
        const deleted = await svc.tombstoneComment(commentId, {
            ...
            afterTombstone: async (deletedComment, tx) => {
                ...
                await Promise.all(annotationCleanup.deletedCommentIds.map((annotationCommentId) => Promise.all([
    ...
    router.post("/issues/:id/comments", validate(addIssueCommentSchema), async (req, res) => {
        ...
        const mentionGrantedPeerAgentCommentOnly = isClosed &&
        ...
        if (effectiveResumeRequested === true && !(await assertExplicitResumeIntentAllowed(req, res, issue)))
            return;
        ...
        let commentDecisionStageWakeup = null;
        ...
        if (interruptRequested) {
            ...
            if (runToInterrupt) {
                const cancelled = await heartbeat.cancelRun(runToInterrupt.id, "Interrupted by board comment", operatorInterruptCancelOptions({ issueId: currentIssue.id, actor }));
                if (cancelled) {
                    interruptedRunId = cancelled.id;
                    await logActivity(db, {
                        companyId: cancelled.companyId,
        ...
        const shouldAutoApproveReviewComment = currentIssue.status === "in_review" &&
        ...
        if (shouldAutoApproveReviewComment) {
            ...
            try {
                txResult = await db.transaction(async (tx) => {
                    ...
                    if (!updated)
                        throw new AutoApprovalIssueMissingError();
                    if (transition.decision && decisionId) {
                        await tx.insert(issueExecutionDecisions).values({
                            id: decisionId,
                            companyId: updated.companyId,
        ...
        void (async () => {
            ...
            const becameTerminal = !["done", "cancelled"].includes(issueBeforeCommentDecision.status) &&
