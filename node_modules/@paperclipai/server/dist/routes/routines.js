...
import { createRoutineSchema, createDocumentAnnotationCommentSchema, createDocumentAnnotationThreadSchema, createRoutineTriggerSchema, rotateRoutineTriggerSecretSchema, runRoutineSchema, updateDocumentAnnotationThreadSchema, updateRoutineSchema, updateRoutineTriggerSchema, } from "@paperclipai/shared";
...
export function routineRoutes(db, options = {}) {
    const router = Router();
    ...
    async function remapRoutineDescriptionAnnotations(req, routineId) {
    ...
    async function assertBoardCanAssignTasks(req, companyId) {
    ...
    function assertCanManageCompanyRoutine(req, companyId, assigneeAgentId) {
        assertCompanyAccess(req, companyId);
        if (req.actor.type === "board")
            return;
    ...
    async function assertCanManageExistingRoutine(req, routineId) {
    ...
    async function logRoutineRevisionCreated(req, input) {
        if (!input.revisionId)
            return;
        const actor = getActorInfo(req);
        await logActivity(db, {
    ...
    router.get("/routines/:id", async (req, res) => {
        const detail = await svc.getDetail(req.params.id);
        if (!detail) {
    ...
    router.get("/routines/:id/description/annotations/:threadId", async (req, res) => {
        ...
        const thread = await documentAnnotationsSvc.getThreadForRoutineDocument(routine.id, routineDocumentKey, req.params.threadId);
        if (!thread) {
    ...
    router.post("/routines/:id/description/annotations", validate(createDocumentAnnotationThreadSchema), async (req, res) => {
        const routine = await assertCanManageExistingRoutine(req, req.params.id);
        if (!routine) {
            res.status(404).json({ error: "Routine not found" });
            return;
        ...
        const thread = await documentAnnotationsSvc.createRoutineThread(routine.id, routineDocumentKey, req.body, annotationActor);
    ...
    router.post("/routines/:id/description/annotations/:threadId/comments", validate(createDocumentAnnotationCommentSchema), async (req, res) => {
        const routine = await assertCanManageExistingRoutine(req, req.params.id);
        if (!routine) {
            res.status(404).json({ error: "Routine not found" });
            return;
        ...
        const { actor, annotationActor } = annotationActorInput(req);
        const comment = await documentAnnotationsSvc.addRoutineComment(routine.id, routineDocumentKey, req.params.threadId, req.body, annotationActor);
        await logActivity(db, {
            companyId: routine.companyId,
            actorType: actor.actorType,
            actorId: actor.actorId,
    ...
    router.patch("/routines/:id/description/annotations/:threadId", validate(updateDocumentAnnotationThreadSchema), async (req, res) => {
        const routine = await assertCanManageExistingRoutine(req, req.params.id);
        if (!routine) {
            res.status(404).json({ error: "Routine not found" });
            return;
        ...
        const { actor, annotationActor } = annotationActorInput(req);
        const thread = await documentAnnotationsSvc.updateRoutineThread(routine.id, routineDocumentKey, req.params.threadId, req.body, annotationActor);
        await logActivity(db, {
            companyId: routine.companyId,
            actorType: actor.actorType,
            actorId: actor.actorId,
    ...
    router.patch("/routines/:id", validate(updateRoutineSchema), async (req, res) => {
        ...
        const assigneeWillChange = req.body.assigneeAgentId !== undefined &&
            req.body.assigneeAgentId !== routine.assigneeAgentId;
        if (assigneeWillChange) {
            await assertBoardCanAssignTasks(req, routine.companyId);
        }
        const statusWillActivate = req.body.status !== undefined &&
    ...
    router.post("/routines/:id/revisions/:revisionId/restore", async (req, res) => {
        ...
        await assertBoardCanAssignTasks(req, routine.companyId);
        const result = await svc.restoreRevision(routine.id, req.params.revisionId, {
            agentId: req.actor.type === "agent" ? req.actor.agentId : null,
            userId: req.actor.type === "board" ? req.actor.userId ?? "board" : null,
            runId: req.actor.runId ?? null,
        });
    ...
    router.get("/routines/:id/runs", async (req, res) => {
        const routine = await svc.get(req.params.id);
        if (!routine) {
            res.status(404).json({ error: "Routine not found" });
            return;
        }
    ...
    router.patch("/routine-triggers/:id", validate(updateRoutineTriggerSchema), async (req, res) => {
        const trigger = await svc.getTrigger(req.params.id);
        if (!trigger) {
    ...
    router.post("/routine-triggers/public/:publicId/fire", async (req, res) => {
        const result = await svc.firePublicTrigger(req.params.publicId, {
            ...
            payload: typeof req.body === "object" && req.body !== null ? req.body : null,
        });
