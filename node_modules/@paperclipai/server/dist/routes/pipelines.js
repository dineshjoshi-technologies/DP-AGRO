...
import { agents, documents, documentRevisions, heartbeatRuns, issueDocuments, issues as issueRows, issueRelations, pipelineAutomationExecutions, pipelineCaseBlockers, pipelineCaseDocuments, pipelineCaseEvents, pipelineCaseIssueLinks, pipelineCases, pipelineDocuments, pipelineStages, pipelineTransitions, pipelines, routines, } from "@paperclipai/db";
import { validate } from "../middleware/validate.js";
import { badRequest, conflict, forbidden, HttpError, notFound, unauthorized, unprocessable } from "../errors.js";
import { PIPELINE_CASE_EVENTS_DEFAULT_LIMIT, PIPELINE_CASE_EVENTS_MAX_LIMIT, PIPELINE_CONTEXT_PACK_EVENT_LIMIT, ensurePipelineCaseBodyDocumentFromSummary, pipelineService, resolvePipelineCaseConversationSource, } from "../services/pipelines.js";
import { COMPANY_CASE_EVENTS_DEFAULT_LIMIT, COMPANY_CASE_EVENTS_MAX_LIMIT, COMPANY_CASE_EVENTS_MAX_TYPES, getCaseChildrenTree, getDirectChildrenSummary, loadDescendantActiveWorkCountsForCases, listCompanyCaseEvents, listPipelineAttention, loadActiveWorkForCases, loadPipelineDescendantActiveWorkCounts, loadPipelineConnections, PIPELINE_ATTENTION_DEFAULT_LIMIT, PIPELINE_ATTENTION_MAX_LIMIT, } from "../services/pipelines-aggregation.js";
import { accessService } from "../services/access.js";
import { authorizationService } from "../services/authorization.js";
import { issueService } from "../services/issues.js";
import { assertCompanyAccess } from "./authz.js";
import { computePipelineHealth, deriveCaseType, envConfigSchema, issueDocumentKeySchema, PIPELINE_CASE_BODY_DOCUMENT_KEY, pipelineAutomationRetryRequestSchema, pipelineAutomationRetryScopeSchema, } from "@paperclipai/shared";
import { documentAnnotationService } from "../services/document-annotations.js";
import { logActivity } from "../services/activity-log.js";
import { formatPipelineConversationBodyDocumentContextMarkdown, loadPipelineConversationBodyDocumentContext, } from "../services/pipeline-conversation-context.js";
import { resolveActorSourceTrustForIssue } from "../services/source-trust.js";
import { formatPipelineCaseOutputContextMarkdown, pipelineCaseOutputsService, summarizePipelineCaseOutputsForContext, } from "../services/pipeline-case-outputs.js";
...
function stageAutomationRoutineId(config) {
    if (!config || typeof config !== "object" || Array.isArray(config))
        return null;
    const onEnter = config.onEnter;
    if (!onEnter || typeof onEnter !== "object" || Array.isArray(onEnter))
        return null;
    const record = onEnter;
    return record.type === "run_routine" && typeof record.routineId === "string" ? record.routineId : null;
}
function readAutomationContextValue(value) {
    return typeof value === "string" && value.trim() ? value.trim() : null;
}
function stageAutomationContext(config) {
    const onEnter = config.onEnter;
    ...
    return {
        ...
        executionWorkspaceSettings: record.executionWorkspaceSettings && typeof record.executionWorkspaceSettings === "object" && !Array.isArray(record.executionWorkspaceSettings)
            ? record.executionWorkspaceSettings
            : null,
    };
}
function withDerivedStageAutomation(stage, routineById) {
...
function extractIntakeFormFields(stage) {
    const baseFields = [{ key: "title", label: "Name", type: "text", required: true, options: [] }];
    ...
    return [
        ...
        ...variables.flatMap((raw) => {
            ...
            const options = Array.isArray(variable.options)
                ? variable.options.filter((option) => typeof option === "string" && option.trim().length > 0)
                : [];
            ...
            if (!legacyKey)
                return [];
            if (variable.showInAddForm !== true)
                return [];
            if (typeof variable.label !== "string" || variable.label.trim().length === 0)
                return [];
...
function codedConflictForUnique(error) {
    if (isPgUniqueViolation(error)) {
        throw conflict("Duplicate pipeline resource key", { code: "duplicate_key" });
    }
    throw error;
}
function assertPipelineCompanyAccess(req, companyId) {
    try {
...
function attentionCallerFor(req) {
    if (req.actor.type === "agent") {
        if (!req.actor.agentId)
            throw unauthorized();
...
function parseEventTypesQuery(value) {
    if (value === undefined)
        return undefined;
    ...
    for (const type of types) {
        if (!/^[a-z_]{1,64}$/.test(type))
            throw badRequest(`Invalid event type: ${type}`);
    }
...
function parseCaseEventsQuery(query) {
...
async function resolvePipelineCompanyId(db, pipelineId) {
    const row = await db
...
async function resolveCaseCompanyId(db, caseId) {
    const row = await db
...
async function assertPipelineAccess(db, req, pipelineId) {
    const companyId = await resolvePipelineCompanyId(db, pipelineId);
    assertPipelineCompanyAccess(req, companyId);
    return companyId;
}
async function assertPipelineWriteAccess(req, input) {
...
async function getPipelineDocumentRow(db, input) {
    return db
        ...
        .where(and(eq(pipelineDocuments.companyId, input.companyId), eq(pipelineDocuments.pipelineId, input.pipelineId), eq(pipelineDocuments.key, input.key)))
        .limit(1)
        .then((rows) => rows[0] ?? null);
}
async function listPipelineDocumentRevisions(db, input) {
    return db
        .select({
...
function parseDocumentKey(rawKey) {
...
async function getPipelineCaseDocumentRow(db, input) {
    return db
        ...
        .leftJoin(documentRevisions, eq(documents.latestRevisionId, documentRevisions.id))
        .where(and(eq(pipelineCaseDocuments.companyId, input.companyId), eq(pipelineCaseDocuments.caseId, input.caseId), eq(pipelineCaseDocuments.key, input.key)))
        .limit(1)
        .then((rows) => rows[0] ?? null);
}
async function listPipelineCaseDocumentRevisions(db, input) {
    return db
        .select({
        id: documentRevisions.id,
        companyId: documentRevisions.companyId,
...
async function resolveCasePipelineId(db, input) {
    const row = await db
...
function activityActorForPipelineRoute(actor) {
    if (actor.type === "agent") {
        return { actorType: "agent", actorId: actor.agentId, agentId: actor.agentId, runId: actor.runId };
    }
    if (actor.type === "user") {
        return { actorType: "user", actorId: actor.userId, agentId: null, runId: null };
    }
    return { actorType: "system", actorId: "pipeline", agentId: null, runId: null };
}
function issueIdFromPipelineRouteRunContext(contextSnapshot) {
...
async function sourceTrustForPipelineCaseDocumentWrite(dbOrTx, input) {
    ...
    if (!issue) {
        const runIssueId = await dbOrTx
            ...
            .from(heartbeatRuns)
            .where(and(eq(heartbeatRuns.companyId, input.companyId), eq(heartbeatRuns.id, input.actor.runId), eq(heartbeatRuns.agentId, input.actor.agentId)))
            .limit(1)
            .then((rows) => issueIdFromPipelineRouteRunContext(rows[0]?.contextSnapshot));
        issue = runIssueId
            ? await dbOrTx
                .select()
                .from(issueRows)
...
async function assertCaseAccess(db, req, caseId) {
    const companyId = await resolveCaseCompanyId(db, caseId);
    assertPipelineCompanyAccess(req, companyId);
    return companyId;
}
async function getStagesByKey(db, pipelineId) {
...
async function writeRouteEvent(db, input) {
...
async function getIssueMutationTarget(db, input) {
    return db
        .select({
        id: issueRows.id,
...
async function assertIssueLinkMutationAllowed(req, input) {
    const decision = await input.access.decide({
        actor: req.actor,
        action: "issue:mutate",
        resource: {
    ...
    if (input.issue.assigneeAgentId === null)
        return;
...
export function pipelineRoutes(db, options = {}) {
    const router = Router();
    ...
    router.get("/companies/:companyId/pipelines", async (req, res) => {
        ...
        const rows = await db
            .select({
            pipeline: pipelines,
            stageCount: sql `count(distinct ${pipelineStages.id})::int`,
            openCaseCount: sql `count(distinct ${pipelineCases.id}) filter (where ${pipelineCases.terminalKind} is null)::int`,
            attentionCount: sql `count(distinct ${pipelineCases.id}) filter (where ${pipelineCases.terminalKind} is null and (${pipelineCases.pendingSuggestion} is not null or (${pipelineCases.stageId} = ${pipelineStages.id} and ${pipelineStages.kind} = 'review')))::int`,
            inMotionCount: sql `count(distinct ${pipelineCases.id}) filter (where ${pipelineCases.terminalKind} is null and ${pipelineCases.stageId} = ${pipelineStages.id} and ${pipelineStages.kind} = 'working')::int`,
            lastActivityAt: sql `max(${pipelineCases.updatedAt})`,
        })
            .from(pipelines)
    ...
    router.get("/pipelines/:pipelineId/health", async (req, res) => {
        ...
        const [pipeline, stages, instructionDocs, companyAgents, companyPipelines, companyStages, failedAutomationRows] = await Promise.all([
            ...
            db.select({ key: pipelineDocuments.key, body: documentRevisions.body })
                ...
                .innerJoin(documents, eq(pipelineDocuments.documentId, documents.id))
                .leftJoin(documentRevisions, eq(documents.latestRevisionId, documentRevisions.id))
                .where(and(eq(pipelineDocuments.companyId, companyId), eq(pipelineDocuments.pipelineId, pipelineId), ilike(pipelineDocuments.key, `${STAGE_INSTRUCTIONS_PREFIX}%`))),
            ...
            db.select({
                ...
                .where(and(eq(pipelineAutomationExecutions.companyId, companyId), eq(pipelineCases.pipelineId, pipelineId), eq(pipelineAutomationExecutions.status, "failed"), isNull(pipelineCases.terminalKind)))
        ...
        const bodyByStageId = new Map();
        for (const doc of instructionDocs) {
            if (!doc.key.startsWith(STAGE_INSTRUCTIONS_PREFIX))
                continue;
        ...
        const healthStages = stages.map((stage) => {
            const stageWithAutomation = withDerivedStageAutomation(stage, routineById);
            const automation = stageWithAutomation.config.automation;
            return {
                id: stage.id,
                key: stage.key,
                name: stage.name,
                kind: stage.kind,
                config: (stageWithAutomation.config ?? null),
                instructionsBody: automation?.instructionsBody ?? bodyByStageId.get(stage.id) ?? "",
            };
        });
        const failedAutomations = failedAutomationRows.map((row) => ({
            stageId: row.stageId,
    ...
    router.patch("/pipelines/:pipelineId", validate(updatePipelineSchema), async (req, res) => {
        ...
        if (req.body.enforceTransitions !== undefined)
            patch.enforceTransitions = req.body.enforceTransitions;
        if (req.body.archived !== undefined)
            patch.archivedAt = req.body.archived ? new Date() : null;
    ...
    router.put("/pipelines/:pipelineId/transitions", validate(replaceTransitionsSchema), async (req, res) => {
        ...
        const transitions = req.body.transitions.map((edge) => {
            const from = byKey.get(edge.fromStageKey);
            const to = byKey.get(edge.toStageKey);
            if (!from || !to)
                throw unprocessable("Transition references unknown stage", { code: "validation" });
    ...
    router.get("/pipelines/:pipelineId/documents/:key", async (req, res) => {
        ...
        if (!row)
            throw notFound("Pipeline document not found");
        res.json(row);
    });
    ...
    router.post("/pipelines/:pipelineId/documents/:key/revisions/:revisionId/restore", async (req, res) => {
        ...
        const result = await db.transaction(async (tx) => {
            ...
            const sourceRevision = await tx
                ...
                .then((rows) => rows[0] ?? null);
            if (!sourceRevision)
                throw notFound("Pipeline document revision not found");
    ...
    router.get("/pipelines/:pipelineId/cases", async (req, res) => {
        ...
        const rows = await db
            ...
            .from(pipelineCases)
            .innerJoin(pipelineStages, eq(pipelineCases.stageId, pipelineStages.id))
            .leftJoin(parentCase, and(eq(parentCase.companyId, companyId), eq(parentCase.id, pipelineCases.parentCaseId)))
            .leftJoin(parentPipeline, and(eq(parentPipeline.companyId, companyId), eq(parentPipeline.id, parentCase.pipelineId)))
            .where(and(eq(pipelineCases.companyId, companyId), eq(pipelineCases.pipelineId, pipelineId), stageKey ? eq(pipelineStages.key, stageKey) : undefined, parentCaseId ? eq(pipelineCases.parentCaseId, parentCaseId) : undefined, includeRetired ? undefined : isNull(pipelineCases.hiddenFromBoardAt), terminal === true ? isNotNull(pipelineCases.terminalKind) : terminal === false ? isNull(pipelineCases.terminalKind) : undefined, q ? or(ilike(pipelineCases.title, `%${q}%`), ilike(pipelineCases.summary, `%${q}%`)) : undefined))
    ...
    router.get("/cases/:caseId/documents/:key", async (req, res) => {
        ...
        if (!row)
            throw notFound("Pipeline case document not found");
        res.json(row);
    });
    router.put("/cases/:caseId/documents/:key", validate(upsertPipelineCaseDocumentSchema), async (req, res) => {
        ...
        if (!result.created) {
            await Promise.all(result.linkedIssueDocuments.map((link) => documentAnnotationsSvc.remapOpenThreadsForDocument({
    ...
    router.post("/cases/:caseId/documents/:key/revisions/:revisionId/restore", async (req, res) => {
        ...
        const result = await db.transaction(async (tx) => {
            ...
            const sourceRevision = await tx
                ...
                .then((rows) => rows[0] ?? null);
            if (!sourceRevision)
                throw notFound("Pipeline case document revision not found");
            if (existing.document.latestRevisionId === sourceRevision.id) {
                throw conflict("Selected revision is already the latest revision", {
                    currentRevisionId: existing.document.latestRevisionId,
                });
    ...
    router.get("/cases/:caseId/children", async (req, res) => {
        ...
        const rows = await db
            ...
            .from(pipelineCases)
            .innerJoin(pipelineStages, eq(pipelineCases.stageId, pipelineStages.id))
            .where(and(eq(pipelineCases.companyId, companyId), eq(pipelineCases.parentCaseId, caseId), isNull(pipelineCases.hiddenFromBoardAt)))
            .orderBy(asc(pipelineCases.createdAt));
        const caseIds = rows.map((row) => row.case.id);
        const [activeWork, descendantActiveWorkCounts] = await Promise.all([
            loadActiveWorkForCases(db, companyId, caseIds),
    ...
    router.post("/cases/:caseId/release", validate(releaseCaseSchema), async (req, res) => {
        ...
        if (req.body.force && actor.type === "agent")
            throw new HttpError(403, "Agents cannot force-release pipeline leases", { code: "forbidden" });
    ...
    router.get("/cases/:caseId/issue-links", async (req, res) => {
        ...
        const links = await db
            ...
            .where(and(eq(pipelineCaseIssueLinks.companyId, companyId), eq(pipelineCaseIssueLinks.caseId, caseId), eq(issueRows.companyId, companyId)))
    ...
    router.delete("/cases/:caseId/issue-links/:linkId", async (req, res) => {
        ...
        const existingLink = await db
            ...
            .from(pipelineCaseIssueLinks)
            .where(and(eq(pipelineCaseIssueLinks.id, linkId), eq(pipelineCaseIssueLinks.companyId, companyId), eq(pipelineCaseIssueLinks.caseId, caseId)))
            .limit(1)
            .then((rows) => rows[0] ?? null);
        if (!existingLink)
            throw notFound("Pipeline case issue link not found");
...
async function getCaseDetail(db, companyId, caseId) {
    const row = await db
    ...
    const parentCasePromise = row.case.parentCaseId
        ? db
            ...
            .innerJoin(pipelineStages, eq(pipelineCases.stageId, pipelineStages.id))
            .innerJoin(pipelines, eq(pipelineCases.pipelineId, pipelines.id))
            .where(and(eq(pipelineCases.companyId, companyId), eq(pipelineCases.id, row.case.parentCaseId), eq(pipelines.companyId, companyId)))
            .limit(1)
            .then((rows) => rows[0] ?? null)
        : Promise.resolve(null);
    const [allowedNextStages, links, blockers, blocks, childrenCounts, activeWorkByCase, descendantActiveWorkCounts, parentCase, conversationSource, liveness, builtFromAutomation,] = await Promise.all([
...
function stageAutomationId(stage) {
    const config = stage.config && typeof stage.config === "object" && !Array.isArray(stage.config)
        ? stage.config
        : null;
    const onEnter = config?.onEnter;
...
async function loadBuiltFromAutomation(db, companyId, caseRow) {
    if (!caseRow.automationAttemptId)
        return null;
    const row = await db
        .select({
        ...
        .from(pipelineAutomationExecutions)
        .innerJoin(pipelineCases, and(eq(pipelineCases.companyId, companyId), eq(pipelineCases.id, pipelineAutomationExecutions.caseId)))
        .innerJoin(pipelines, and(eq(pipelines.companyId, companyId), eq(pipelines.id, pipelineCases.pipelineId)))
        .innerJoin(routines, and(eq(routines.companyId, companyId), eq(routines.id, pipelineAutomationExecutions.routineId)))
        .where(and(eq(pipelineAutomationExecutions.companyId, companyId), eq(pipelineAutomationExecutions.id, caseRow.automationAttemptId)))
        .limit(1)
        .then((rows) => rows[0] ?? null);
    if (!row)
        return null;
...
function isLiveIssueStatus(status) {
    return status === "todo" || status === "in_progress" || status === "in_review";
}
function isWaitingIssueStatus(status) {
    return status === "backlog" || status === "todo" || status === "in_review";
}
function summarizeLinkedIssue(issue) {
    return {
        id: issue.id,
...
function readBreakdownRequestKeys(payload) {
    if (!payload || typeof payload !== "object" || Array.isArray(payload))
        return [];
    const keys = payload.requestKeys;
    if (!Array.isArray(keys))
        return [];
    return [...new Set(keys.filter((key) => typeof key === "string" && key.trim().length > 0))];
}
function readStageBreakdownConfig(config) {
    if (!config || typeof config !== "object" || Array.isArray(config))
        return null;
...
function stageHasChildrenTerminalGate(config) {
    if (!config || typeof config !== "object" || Array.isArray(config))
        return false;
    const record = config;
...
function readStageAutomationId(stage) {
    if (!stage.config || typeof stage.config !== "object" || Array.isArray(stage.config))
        return null;
    const onEnterValue = stage.config.onEnter;
    if (!onEnterValue || typeof onEnterValue !== "object" || Array.isArray(onEnterValue))
        return null;
    const onEnter = onEnterValue;
    ...
    if (onEnter.type !== "run_routine" || routineId.length === 0)
        return null;
    return rawId.length > 0 ? rawId : `${stage.id}:on_enter`;
}
function readStageAutomationTargetPipelineId(stage) {
    if (!readStageAutomationId(stage))
        return null;
...
async function assertStageAutomationTargetWriteAccess(db, req, input) {
...
async function assertCurrentStageAutomationTargetWriteAccess(db, req, input) {
    const row = await db
    ...
    if (input.automationId && currentAutomationId !== input.automationId)
        return;
...
function parsePermissionPreflightFingerprint(fingerprint) {
    if (!fingerprint)
        return null;
    ...
    if (!caseId || !stageId || !automationId || !targetPipelineId || !principalId || !permissionKey)
        return null;
    return { caseId, stageId, automationId, targetPipelineId, principalId, permissionKey };
}
async function latestBreakdownCreatedEvent(db, companyId, caseId) {
    return db
        .select()
        .from(pipelineCaseEvents)
        .where(and(eq(pipelineCaseEvents.companyId, companyId), eq(pipelineCaseEvents.caseId, caseId), eq(pipelineCaseEvents.type, "updated"), sql `${pipelineCaseEvents.payload}->>'kind' = 'breakdown_created'`))
        .orderBy(desc(pipelineCaseEvents.createdAt), desc(pipelineCaseEvents.id))
        .limit(1)
        .then((rows) => rows[0] ?? null);
}
async function derivePipelineCaseLiveness(db, companyId, row) {
    ...
    const blockerCase = await db
        ...
        .where(and(eq(pipelineCaseBlockers.companyId, companyId), eq(pipelineCaseBlockers.caseId, row.case.id), or(isNull(pipelineCases.terminalKind), ne(pipelineCases.terminalKind, "done"))))
        .orderBy(asc(pipelineCases.createdAt))
        .limit(1)
        .then((rows) => rows[0] ?? null);
    if (blockerCase) {
    ...
    const linkedIssues = await db
        ...
        .from(pipelineCaseIssueLinks)
        .innerJoin(issueRows, eq(pipelineCaseIssueLinks.issueId, issueRows.id))
        .where(and(eq(pipelineCaseIssueLinks.companyId, companyId), eq(pipelineCaseIssueLinks.caseId, row.case.id), inArray(pipelineCaseIssueLinks.role, ["automation", "work"]), eq(issueRows.companyId, companyId), isNull(issueRows.hiddenAt)))
    ...
    if (blockedIssue) {
        const blocker = await db
            ...
            .where(and(eq(issueRelations.companyId, companyId), eq(issueRelations.type, "blocks"), eq(issueRelations.relatedIssueId, blockedIssue.issue.id)))
    ...
    const latestAutomation = await db
        .select()
        .from(pipelineAutomationExecutions)
        .where(and(eq(pipelineAutomationExecutions.companyId, companyId), eq(pipelineAutomationExecutions.caseId, row.case.id)))
    ...
    if (breakdownConfig) {
        ...
        const createdRows = expectedRequestKeys.length > 0
            ? await db
                ...
                .from(pipelineCases)
                .where(and(eq(pipelineCases.companyId, companyId), eq(pipelineCases.parentCaseId, row.case.id), inArray(pipelineCases.requestKey, expectedRequestKeys), isNull(pipelineCases.hiddenFromBoardAt)))
            : [];
...
function buildCaseContextMarkdown(detail, bodyDocumentContext, outputSummaries) {
...
async function getChildOutcomeSummaries(db, companyId, caseId) {
    ...
    const reviewEvents = await db
        .select()
        .from(pipelineCaseEvents)
        .where(and(eq(pipelineCaseEvents.companyId, companyId), inArray(pipelineCaseEvents.caseId, childIds), eq(pipelineCaseEvents.type, "review_decided")))
        .orderBy(desc(pipelineCaseEvents.createdAt), desc(pipelineCaseEvents.id));
    const latestReviewByCaseId = new Map();
    for (const event of reviewEvents) {
        if (!latestReviewByCaseId.has(event.caseId))
