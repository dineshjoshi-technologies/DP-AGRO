...
import { AGENT_ADAPTER_TYPES, createEnvironmentSchema, getEnvironmentCapabilities, probeEnvironmentConfigSchema, updateEnvironmentSchema, } from "@paperclipai/shared";
import { conflict, forbidden, unprocessable } from "../errors.js";
import { validate } from "../middleware/validate.js";
import { issueService, instanceSettingsService, logActivity, projectService, } from "../services/index.js";
import { collectEnvironmentSecretRefs, normalizeEnvironmentConfigForPersistence, normalizeEnvironmentConfigForProbe, readSshEnvironmentPrivateKeySecretId, } from "../services/environment-config.js";
...
export function environmentRoutes(db, options = {}) {
    const router = Router();
    ...
    function assertCanAccessInstanceEnvironments(req) {
    ...
    function assertCanReadInstanceEnvironments(req) {
        assertBoardOrgAccess(req);
    }
    function canReadFullInstanceEnvironment(req) {
        return req.actor.type === "board"
            && (req.actor.source === "local_implicit" || req.actor.isInstanceAdmin);
    }
    function redactEnvironmentForRestrictedView(environment) {
        return {
            ...environment,
            config: {},
    ...
    function presentEnvironmentForRead(req, environment) {
        return canReadFullInstanceEnvironment(req)
            ? environment
            : redactEnvironmentForRestrictedView(environment);
    }
    async function assertCanReadSecretsForDraftProbe(req, companyId) {
    ...
    async function resolveEnvironmentSecretContextCompanyId(req, environmentId, options) {
        const routeCompanyId = typeof req.params.companyId === "string" && req.params.companyId.trim().length > 0
        ...
        if (!options.required)
            return null;
        throw unprocessable("Environment secret management requires a companyId context during the instance-scoped transition.");
    }
    function summarizeEnvironmentUpdate(patch, environment) {
        const details = {
        ...
        if (patch.driver !== undefined)
            details.driver = environment.driver;
        ...
        if (patch.config !== undefined) {
            ...
            details.configTopLevelKeyCount =
                patch.config && typeof patch.config === "object" && !Array.isArray(patch.config)
                    ? Object.keys(patch.config).length
                    : 0;
        }
    ...
    router.post("/companies/:companyId/environments", validate(createEnvironmentSchema), async (req, res) => {
        ...
        const input = {
            ...req.body,
            envVars: await secrets.normalizeEnvBindingsForPersistence(companyId, req.body.envVars, { strictMode: strictSecretsMode, fieldPath: "envVars" }),
        ...
        await secrets.syncSecretRefsForTarget(companyId, { targetType: "environment", targetId: environment.id }, await collectEnvironmentSecretRefs({ db, environment }));
        await secrets.syncEnvBindingsForTarget(companyId, { targetType: "environment", targetId: environment.id }, environment.envVars);
    ...
    router.get("/environments/:id", async (req, res) => {
        const environment = await svc.getById(req.params.id);
        if (!environment) {
    ...
    router.get("/environment-leases/:leaseId", async (req, res) => {
        const lease = await svc.getLeaseById(req.params.leaseId);
        if (!lease) {
    ...
    router.patch("/environments/:id", validate(updateEnvironmentSchema), async (req, res) => {
        ...
        const companyIdForSecrets = req.body.config !== undefined || req.body.driver !== undefined || req.body.envVars !== undefined
            ? await resolveEnvironmentSecretContextCompanyId(req, existing.id, { required: true })
            : null;
        const configSource = req.body.config !== undefined
        ...
        const patch = {
            ...
            ...(req.body.envVars !== undefined
                ? {
                    envVars: await secrets.normalizeEnvBindingsForPersistence(companyIdForSecrets, req.body.envVars, { strictMode: strictSecretsMode, fieldPath: "envVars" }),
                }
                : {}),
        ...
        if (patch.config !== undefined || patch.driver !== undefined) {
            await secrets.syncSecretRefsForTarget(companyIdForSecrets, { targetType: "environment", targetId: environment.id }, await collectEnvironmentSecretRefs({ db, environment }));
        }
        if (patch.envVars !== undefined) {
            await secrets.syncEnvBindingsForTarget(companyIdForSecrets, { targetType: "environment", targetId: environment.id }, environment.envVars);
        }
        await logInstanceEnvironmentActivity({
            actor,
            action: "environment.updated",
            entityId: environment.id,
            details: summarizeEnvironmentUpdate(patch, environment),
        });
    ...
    router.delete("/environments/:id", async (req, res) => {
        ...
        assertCanAccessInstanceEnvironments(req);
        const companyIds = await instanceSettings.listCompanyIds();
        await Promise.all(companyIds.flatMap((companyId) => [
    ...
    router.post("/environments/:id/probe", async (req, res) => {
        ...
        if (!companyIdForSecrets) {
            ...
            if (secretRefs.length > 0) {
                throw unprocessable("Environment probe requires an explicit companyId to resolve secret-backed config for this environment.");
            }
        }
    ...
    router.post("/companies/:companyId/environments/probe-config", validate(probeEnvironmentConfigSchema), async (req, res) => {
        const companyId = req.params.companyId;
        assertCanAccessInstanceEnvironments(req);
