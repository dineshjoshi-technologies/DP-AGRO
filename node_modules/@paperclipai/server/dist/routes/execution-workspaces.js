...
import { findWorkspaceCommandDefinition, matchWorkspaceRuntimeServiceToCommand, updateExecutionWorkspaceSchema, workspaceOverviewQuerySchema, workspaceRuntimeControlTargetSchema, } from "@paperclipai/shared";
...
export function executionWorkspaceRoutes(db, opts = {}) {
    const router = Router();
    ...
    async function assertExecutionWorkspaceReadAllowed(req, res, companyId) {
    ...
    async function assertRuntimeManageAllowed(req, res, companyId) {
        const decision = await access.decide({
            actor: req.actor,
    ...
    router.get("/companies/:companyId/execution-workspaces", async (req, res) => {
        ...
        if (!(await assertExecutionWorkspaceReadAllowed(req, res, companyId)))
            return;
        const filters = {
    ...
    router.get("/execution-workspaces/:id", async (req, res) => {
        ...
        if (!(await assertExecutionWorkspaceReadAllowed(req, res, workspace.companyId)))
            return;
        res.json(workspace);
    });
    ...
    async function handleExecutionWorkspaceRuntimeCommand(req, res) {
        ...
        if (!(await assertRuntimeManageAllowed(req, res, existing.companyId)))
            return;
