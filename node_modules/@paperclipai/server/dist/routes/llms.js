...
const pluginOnlyAdapterDocs = new Map([
...
export function llmRoutes(db) {
    ...
    router.get("/llms/agent-configuration.txt", async (req, res) => {
        ...
        const lines = [
            ...
            ...Array.from(pluginOnlyAdapterDocs.keys())
                .filter((adapterType) => !adapters.some((adapter) => adapter.type === adapterType))
                .map((adapterType) => `- ${adapterType}: /llms/agent-configuration/${adapterType}.txt`),
            "",
    ...
    router.get("/llms/agent-configuration/:adapterType.txt", async (req, res) => {
        await assertCanRead(req);
        const adapterType = req.params.adapterType;
        ...
        res
            .type("text/plain")
            .send(adapter.agentConfigurationDoc ??
