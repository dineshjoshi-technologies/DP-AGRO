 ...
 */
function stripActionSignals(response) {
 ...
 */
function serializeTurn(role, body) {
...
export function boardChatRoutes(db, opts) {
    const router = Router();
    ...
    let _boardSkillCache = null;
    function loadBoardSkill() {
        if (_boardSkillCache)
        ...
        try {
            ...
            return ("You are a board-level assistant helping a human manage their AI-agent " +
    ...
    router.post("/board/chat/stream", async (req, res) => {
        ...
        const { companyId, message, taskId } = req.body;
        ...
        if (!issueId) {
            ...
            const boardIssue = companyIssues.find((i) => i.title === "Board Operations" &&
                i.status !== "done" &&
                i.status !== "cancelled");
            if (boardIssue) {
        ...
        const serverAddr = localAddress === "::" || localAddress === "::1" ? "127.0.0.1" : localAddress;
        ...
        const writeToolStatus = (toolName) => {
            if (!res.writable)
                return;
            let statusText;
        ...
        proc.on("error", (err) => {
            ...
            if (res.writable) {
                res.write(`data: ${JSON.stringify({
                    type: "error",
                    message: "Could not start the board assistant. Is the `claude` CLI installed and on PATH?",
                })}\n\n`);
                res.end();
            }
        });
...
//# sourceMappingURL=board-chat.js.map