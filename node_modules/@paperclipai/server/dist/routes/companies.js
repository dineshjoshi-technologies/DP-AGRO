...
import { DEFAULT_FEEDBACK_DATA_SHARING_TERMS_VERSION, companyArtifactsQuerySchema, companyPortabilityExportSchema, companyPortabilityImportSchema, companyPortabilityPreviewSchema, createCompanySchema, feedbackTargetTypeSchema, feedbackTraceStatusSchema, feedbackVoteValueSchema, updateCompanyBrandingSchema, updateCompanySchema, } from "@paperclipai/shared";
import { badRequest, forbidden } from "../errors.js";
import { validate } from "../middleware/validate.js";
import { accessService, agentService, budgetService, companyArtifactsService, companyPortabilityService, companyService, feedbackService, logActivity, } from "../services/index.js";
...
export function companyRoutes(db, storage) {
    ...
    async function assertSameCompanyCeoAgentOrBoard(req, companyId, capability) {
    ...
    router.post("/", validate(createCompanySchema), async (req, res) => {
        ...
        await access.ensureRoleDefaultGrants(company.id, ownerPrincipalId, "owner", req.actor.userId ?? null);
        await logActivity(db, {
    ...
    router.patch("/:companyId", async (req, res) => {
        ...
        if (body.status === "active" && existingCompany.status === "paused") {
            const [archivedPausedCount] = await db
                ...
                .from(agentsTable)
                .where(and(eq(agentsTable.companyId, companyId), eq(agentsTable.status, "paused"), eq(agentsTable.pauseReason, "company_archived")));
            transitionsPausedToActiveWithArchivePausedAgents =
                Number(archivedPausedCount?.value ?? 0) > 0;
        }
        const lifecycleEventEmittedByService = transitionsToArchived ||
...
function createImportJob(cloudTenantKey) {
...
async function runImportJob(job, operation) {
    try {
...
function importedCompanyActivityContext(actor, include) {
    return {
        actorType: actor.actorType,
        actorId: actor.actorId,
        agentId: actor.agentId,
...
async function logImportedCompanyActivity(db, activity, result) {
    await logActivity(db, {
        companyId: result.company.id,
...
function importJobAcceptedResponse(job) {
    return {
        job: {
            id: job.id,
            status: job.status,
...
function importJobResponse(job) {
...
function cleanupTerminalImportJobs(importJobs, terminalRetentionMs) {
    const now = Date.now();
