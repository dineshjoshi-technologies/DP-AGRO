...
async function assertAgentCanManageRuntimeServicesForWorkspace(db, req, input) {
    ...
    const actorRun = req.actor.runId
        ? await db
            ...
            .from(heartbeatRuns)
            .where(and(eq(heartbeatRuns.id, req.actor.runId), eq(heartbeatRuns.companyId, input.companyId), eq(heartbeatRuns.agentId, actorAgent.id)))
            .then((rows) => rows[0] ?? null)
        : null;
    ...
    const linkedScopeIssues = await db
        ...
        .from(issues)
        .leftJoin(projects, and(eq(projects.id, issues.projectId), eq(projects.companyId, issues.companyId)))
        .where(and(eq(issues.companyId, input.companyId), isNull(issues.hiddenAt), inArray(issues.status, WORKSPACE_RUNTIME_ELIGIBLE_ISSUE_STATUSES), workspaceScopeCondition));
    ...
    const linkedIssue = await db
        ...
        .from(issues)
        .leftJoin(projects, and(eq(projects.id, issues.projectId), eq(projects.companyId, issues.companyId)))
        .where(and(eq(issues.companyId, input.companyId), isNull(issues.hiddenAt), inArray(issues.status, WORKSPACE_RUNTIME_ELIGIBLE_ISSUE_STATUSES), inArray(issues.assigneeAgentId, eligibleAgentIds), workspaceScopeCondition))
        .then((rows) => rows[0] ?? null);
    if (linkedIssue) {
        assertLowTrustCanManageRuntimeForIssue({
            actorAgent,
...
function assertLowTrustCanManageRuntimeForActor(input) {
    ...
    if (resolution.kind !== "low_trust_review")
        return resolution;
    if (isLowTrustRuntimeManagementAllowed(resolution))
        return resolution;
