...
export function companySkillRoutes(db) {
    ...
    function canCreateSkills(agent) {
    ...
    function skillActor(req) {
        if (req.actor.type === "agent") {
            return { type: "agent", agentId: req.actor.agentId ?? null };
        }
        if (req.actor.type === "board") {
            return { type: "user", userId: req.actor.userId ?? null };
        }
        return { type: "system" };
    }
    ...
    router.get("/skills/catalog/:catalogId/files", async (req, res) => {
        assertAuthenticated(req);
        const catalogRef = firstQueryString(req.query.ref) ?? req.params.catalogId;
    ...
    router.post("/companies/:companyId/skills/:skillId/comments", validate(companySkillCommentCreateSchema), async (req, res) => {
        const companyId = req.params.companyId;
        const skillId = req.params.skillId;
        assertCompanyAccess(req, companyId);
    ...
    router.patch("/companies/:companyId/skills/:skillId/comments/:commentId", validate(companySkillCommentUpdateSchema), async (req, res) => {
        const companyId = req.params.companyId;
        const skillId = req.params.skillId;
