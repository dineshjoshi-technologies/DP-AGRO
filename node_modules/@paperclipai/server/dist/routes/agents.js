...
import { DEFAULT_ACPX_LOCAL_AGENT, DEFAULT_ACPX_LOCAL_MODE, DEFAULT_ACPX_LOCAL_NON_INTERACTIVE_PERMISSIONS, DEFAULT_ACPX_LOCAL_PERMISSION_MODE, } from "@paperclipai/adapter-acpx-local";
...
function readRunIssueId(context) {
...
export function agentRoutes(db, options = {}) {
    ...
    async function decideAgentRead(req, agent) {
    ...
    async function assertAgentReadAllowed(req, res, agent) {
    ...
    async function filterAgentsForActor(req, rows, fallbackCompanyId) {
    ...
    async function resolveAgentSelfTrustPreset(req, agent) {
    ...
    function buildLowTrustSelfView(agent) {
        return {
            id: agent.id,
            companyId: agent.companyId,
            name: agent.name,
    ...
    function asEnvBindingString(value) {
    ...
    async function normalizeMediatedAdapterConfigForPersistence(input) {
        const normalizedAdapterConfig = await secretsSvc.normalizeAdapterConfigForPersistence(input.companyId, input.adapterConfig, {
    ...
    function codexLocalAgentHome(companyId, agentId) {
    ...
    function codexLocalEnvKeyConfigured(value) {
        if (asEnvBindingString(value))
            return true;
        const record = asRecord(value);
    ...
    function applyCodexLocalKeyIsolation(companyId, agentId, adapterType, adapterConfig) {
    ...
    function buildUnsupportedSkillSnapshot(adapterType, desiredSkillEntries = []) {
    ...
    function normalizeDesiredSkillSelections(requestedDesiredSkills) {
        ...
        for (const value of requestedDesiredSkills) {
            ...
            if (!entry.key || out.has(entry.key))
                continue;
    ...
    async function buildRuntimeSkillConfig(companyId, adapterType, config, options = {}) {
    ...
    async function resolveDesiredSkillAssignment(companyId, adapterType, adapterConfig, requestedDesiredSkills) {
        ...
        const desiredSkillEntries = resolvedRequestedSkillEntries.filter((entry, index, entries) => entries.findIndex((candidate) => candidate.key === entry.key) === index);
        const desiredSkills = desiredSkillEntries.map((entry) => entry.key);
        return {
    ...
    router.get("/agents/:id/skills", async (req, res) => {
        ...
        if (!adapter?.listSkills) {
            ...
            const desiredSkillEntries = preference.desiredSkillEntries.filter((entry, index, entries) => entries.findIndex((candidate) => candidate.key === entry.key) === index);
    ...
    router.get("/agents/:id", async (req, res) => {
        ...
        assertCompanyAccess(req, agent.companyId);
        if (!(await assertAgentReadAllowed(req, res, agent)))
            return;
    ...
    router.post("/companies/:companyId/agent-hires", validate(createAgentHireSchema), async (req, res) => {
        ...
        const requestedAdapterConfig = applyCodexLocalKeyIsolation(companyId, hiredAgentId, hireInput.adapterType, applyCreateDefaultsByAdapterType(hireInput.adapterType, rawHireAdapterConfig));
        const desiredSkillAssignment = await resolveDesiredSkillAssignment(companyId, hireInput.adapterType, requestedAdapterConfig, normalizeDesiredSkillSelections(Array.isArray(requestedDesiredSkills) ? requestedDesiredSkills : undefined));
        const normalizedAdapterConfig = await normalizeMediatedAdapterConfigForPersistence({
            companyId,
    ...
    router.post("/companies/:companyId/agents", validate(createAgentSchema), async (req, res) => {
        ...
        const requestedAdapterConfig = applyCodexLocalKeyIsolation(companyId, agentId, createInput.adapterType, applyCreateDefaultsByAdapterType(createInput.adapterType, rawCreateAdapterConfig));
        const desiredSkillAssignment = await resolveDesiredSkillAssignment(companyId, createInput.adapterType, requestedAdapterConfig, normalizeDesiredSkillSelections(Array.isArray(requestedDesiredSkills) ? requestedDesiredSkills : undefined));
        const normalizedAdapterConfig = await normalizeMediatedAdapterConfigForPersistence({
            companyId,
    ...
    router.patch("/agents/:id/instructions-path", validate(updateAgentInstructionsPathSchema), async (req, res) => {
        ...
        const normalizedAdapterConfig = await secretsSvc.normalizeAdapterConfigForPersistence(existing.companyId, syncedAdapterConfig, { strictMode: strictSecretsMode, adapterType: existing.adapterType });
        const actor = getActorInfo(req);
    ...
    router.patch("/agents/:id/instructions-bundle", validate(updateAgentInstructionsBundleSchema), async (req, res) => {
        ...
        const normalizedAdapterConfig = await secretsSvc.normalizeAdapterConfigForPersistence(existing.companyId, adapterConfig, { strictMode: strictSecretsMode, adapterType: existing.adapterType });
    ...
    router.put("/agents/:id/instructions-bundle/file", validate(upsertAgentInstructionsFileSchema), async (req, res) => {
        ...
        const normalizedAdapterConfig = await secretsSvc.normalizeAdapterConfigForPersistence(existing.companyId, result.adapterConfig, { strictMode: strictSecretsMode, adapterType: existing.adapterType });
        await svc.update(id, { adapterConfig: normalizedAdapterConfig }, {
            recordRevision: {
                createdByAgentId: actor.agentId,
                createdByUserId: actor.actorType === "user" ? actor.actorId : null,
    ...
    router.patch("/agents/:id", validate(updateAgentSchema), async (req, res) => {
        ...
        if (touchesAdapterConfiguration) {
            ...
            const effectiveAdapterConfig = applyCodexLocalKeyIsolation(existing.companyId, existing.id, requestedAdapterType, applyCreateDefaultsByAdapterType(requestedAdapterType, rawEffectiveAdapterConfig));
    ...
    router.get("/heartbeat-runs/:runId", async (req, res) => {
        ...
        res.json(redactCurrentUserValue({ ...decoratedRun, retryExhaustedReason, outputSilence: await heartbeat.buildRunOutputSilence(run) }, await getCurrentUserRedactionOptions()));
    });
