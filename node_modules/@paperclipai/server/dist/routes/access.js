...
import { assets, agentApiKeys, authUsers, companies, companyLogos, companyMemberships, instanceUserRoles, invites, joinRequests, principalPermissionGrants, } from "@paperclipai/db";
import { acceptInviteSchema, createCliAuthChallengeSchema, claimJoinRequestApiKeySchema, createBoardApiKeySchema, createCompanyInviteSchema, createOpenClawInvitePromptSchema, listCompanyInvitesQuerySchema, listJoinRequestsQuerySchema, resolveCliAuthChallengeSchema, searchAdminUsersQuerySchema, updateCompanyMemberWithPermissionsSchema, updateCompanyMemberSchema, archiveCompanyMemberSchema, updateMemberPermissionsSchema, updateUserCompanyAccessSchema, PERMISSION_KEYS, isUuidLike, } from "@paperclipai/shared";
...
function normalizeHermesGatewayApiBaseUrl(url) {
    const normalized = new URL(url.toString());
    const normalizedPath = normalized.pathname.replace(/\/+$/, "") || "/";
    if (normalized.port === DEFAULT_HERMES_DASHBOARD_PORT &&
        HERMES_DASHBOARD_API_PATHS.has(normalizedPath)) {
        normalized.pathname = "/api";
    }
    return normalized;
}
function isDefaultHermesDashboardRoot(url) {
    const normalizedPath = url.pathname.replace(/\/+$/, "") || "/";
    return (url.port === DEFAULT_HERMES_DASHBOARD_PORT &&
        HERMES_DASHBOARD_API_PATHS.has(normalizedPath));
}
    ...
    if (input.adapterType === "hermes_gateway") {
        ...
        if (!rawApiBaseUrl) {
            ...
            try {
                ...
                if (apiBaseUrl.protocol !== "http:" && apiBaseUrl.protocol !== "https:") {
                ...
                else if (apiBaseUrl.protocol === "http:" &&
                    ...
                    fatalErrors.push("agentDefaultsPayload.apiBaseUrl uses remote plain HTTP; use HTTPS or set dangerouslyAllowInsecureRemoteHttp=true for unsafe local development");
                }
                else {
...
export async function prepareAgentDefaultsPayloadForJoinPersistence(input) {
    ...
    return secretService(input.db).normalizeAdapterConfigForPersistence(input.companyId, input.normalized, { adapterType: input.adapterType, actor: input.actor });
}
...
export function accessRoutes(db, opts) {
    ...
    async function approveHumanJoinRequestFromInvite(input) {
        ...
        await access.ensureMembership(input.companyId, "user", input.joinRequest.requestingUserId, membershipRole, "active");
        const grants = humanJoinGrantsFromDefaults(input.invite.defaultsPayload, membershipRole);
        await access.setPrincipalGrants(input.companyId, "user", input.joinRequest.requestingUserId, grants, input.invite.invitedByUserId ?? null);
        if (input.joinRequest.status === "approved") {
            return input.joinRequest;
        }
        const approvedAt = new Date();
        const approvedByUserId = input.invite.invitedByUserId ?? (isLocalImplicit(input.req) ? "local-board" : null);
    ...
    router.post("/invites/:token/accept", validate(acceptInviteSchema), async (req, res) => {
        ...
        const persistedJoinDefaultsPayload = requestType === "agent"
                ...
                : await db.transaction(async (tx) => {
                    ...
                    const row = await tx
                        ...
                        capabilities: requestType === "agent"
                            ...
                            : null,
                        agentDefaultsPayload: requestType === "agent" ? persistedJoinDefaultsPayload : null,
                        claimSecretHash,
