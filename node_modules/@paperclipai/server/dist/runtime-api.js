...
function pushCandidate(candidates, seen, rawUrl) {
...
export function collectReachableInterfaceHosts(input = {}) {
    ...
    for (const entries of Object.values(interfaces)) {
        for (const entry of entries ?? []) {
            ...
            if (!host || isLoopbackHost(host) || isWildcardHost(host) || isLinkLocalHost(host))
                continue;
            if (seen.has(host))
                continue;
            seen.add(host);
...
export function buildRuntimeApiCandidateUrls(input) {
    ...
    if (candidates.length === 0) {
        pushCandidate(candidates, seen, choosePrimaryRuntimeApiUrl({
