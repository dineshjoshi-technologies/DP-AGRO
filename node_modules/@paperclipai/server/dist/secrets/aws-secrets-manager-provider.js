...
function commonValue(values) {
    const nonEmpty = values.filter((value) => Boolean(value?.trim()));
...
function pathSegments(name) {
    return name.split("/").map((segment) => segment.trim()).filter(Boolean);
}
function inferPathSignals(entry, tags) {
...
function discoveryDisplayName(input) {
    const qualifier = input.environmentTag ??
        input.namespace ??
...
function discoverAwsProviderConfigCandidates(input) {
    ...
    if (skippedForeignPaperclipSampleCount > 0) {
        skippedWarnings.push(`Skipped ${skippedForeignPaperclipSampleCount} Paperclip-managed AWS secret sample(s) that were not tagged for this company.`);
    }
...
export function createAwsSecretsManagerProvider(options) {
    ...
    return {
        ...
        async discoverProviderConfigs(input) {
            const config = resolveConfig(input.providerConfig);
            const gateway = resolveGateway(config);
            const query = input.query?.trim();
            const pageSize = input.pageSize && Number.isFinite(input.pageSize)
