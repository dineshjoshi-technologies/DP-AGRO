...
export function pluginDriverProviderKey(config) {
...
export async function resolvePluginEnvironmentDriver(input) {
    ...
    const driver = plugin.manifestJson.environmentDrivers?.find((candidate) => candidate.driverKey === input.config.driverKey);
    if (!driver) {
...
export async function resolvePluginSandboxProviderDriverByKey(input) {
    ...
    for (const plugin of plugins) {
        const driver = plugin.manifestJson.environmentDrivers?.find((candidate) => candidate.driverKey === input.driverKey && candidate.kind === "sandbox_provider");
        if (!driver)
            continue;
...
export async function listReadyPluginEnvironmentDrivers(input) {
    ...
    return plugins.flatMap((plugin) => {
        if (plugin.status !== "ready" || !input.workerManager?.isRunning(plugin.id))
            return [];
...
export async function validatePluginSandboxProviderConfig(input) {
    ...
    if (!result.ok) {
        throw unprocessable(result.errors?.[0] ?? `Sandbox provider "${input.provider}" rejected its config.`, {
...
export async function validatePluginEnvironmentDriverConfig(input) {
    ...
    if (!result.ok) {
        throw unprocessable(result.errors?.[0] ?? `Plugin environment driver "${pluginDriverProviderKey(input.config)}" rejected its config.`, {
            errors: result.errors ?? [],
            warnings: result.warnings ?? [],
        });
    }
...
export async function executePluginEnvironmentCommand(input) {
    ...
    return await input.workerManager.call(plugin.id, "environmentExecute", input.params, resolvePluginExecuteRpcTimeoutMs({
...
//# sourceMappingURL=plugin-environment-driver.js.map