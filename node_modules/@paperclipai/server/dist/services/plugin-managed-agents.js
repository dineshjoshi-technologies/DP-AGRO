...
import { agents, companies, pluginEntities, pluginManagedResources, } from "@paperclipai/db";
...
function bindingExternalId(companyId, agentKey) {
    return `managed:agent:${companyId}:${agentKey}`;
}
function managedMetadata(pluginId, pluginKey, declaration, existing) {
    return {
        ...(existing ?? {}),
...
function selectPreferredAdapterType(declaration, usage) {
...
function declarationPatch(declaration, input = {}) {
    return {
...
function applyInstructionTemplateVariables(content, variables) {
    let next = content;
...
function declaredInstructionFiles(declaration, variables) {
    const instructionDeclaration = declaration.instructions;
    if (!instructionDeclaration?.content && !instructionDeclaration?.files)
        return null;
    ...
    return {
        entryFile,
        files: Object.fromEntries(Object.entries(files).map(([filePath, content]) => [
            filePath,
            applyInstructionTemplateVariables(content, variables),
        ])),
    };
}
function rowIsManagedAgent(row, pluginKey, agentKey) {
    const metadata = row.metadata;
    if (!metadata || typeof metadata !== "object" || Array.isArray(metadata))
        return false;
    const marker = metadata.paperclipManagedResource;
    if (!marker || typeof marker !== "object" || Array.isArray(marker))
        return false;
    const record = marker;
    return (record.pluginKey === pluginKey
        && record.resourceKind === "agent"
        && record.resourceKey === agentKey);
}
export function pluginManagedAgentService(db, options) {
    const agentSvc = agentService(db);
    const approvalSvc = approvalService(db);
    const instructions = agentInstructionsService();
    function declarationFor(agentKey) {
    ...
    async function getBinding(companyId, agentKey) {
        return db
            .select()
            .from(pluginEntities)
            .where(and(eq(pluginEntities.pluginId, options.pluginId), eq(pluginEntities.entityType, MANAGED_AGENT_ENTITY_TYPE), eq(pluginEntities.externalId, bindingExternalId(companyId, agentKey))))
            .then((rows) => rows[0] ?? null);
    }
    async function upsertBinding(companyId, declaration, agentId, extraData = {}, effectiveAdapterType) {
        ...
        const managedResource = await db
            ...
            .where(and(eq(pluginManagedResources.companyId, companyId), eq(pluginManagedResources.pluginId, options.pluginId), eq(pluginManagedResources.resourceKind, "agent"), eq(pluginManagedResources.resourceKey, declaration.agentKey)))
            .then((rows) => rows[0] ?? null);
        if (managedResource) {
            await db
    ...
    async function findRelinkCandidate(companyId, declaration) {
        const rows = await db
            .select()
            .from(agents)
    ...
    async function companyAdapterUsage(companyId) {
        const rows = await db
    ...
    async function resolveManagedAdapterType(companyId, declaration) {
        return selectPreferredAdapterType(declaration, await companyAdapterUsage(companyId));
    }
    async function materializeDeclaredInstructions(companyId, agent, declaration, materializeOptions) {
    ...
    async function managedInstructionDefaultDrift(companyId, agent, declaration) {
        if (!agent)
            return null;
        const variables = await optionsForInstructionVariables(companyId);
        const declared = declaredInstructionFiles(declaration, variables);
    ...
    async function optionsForInstructionVariables(companyId) {
    ...
    async function resolution(companyId, declaration, agent, status, approvalId) {
        return {
            pluginKey: options.pluginKey,
            resourceKind: "agent",
            resourceKey: declaration.agentKey,
    ...
    async function createManagedAgent(companyId, declaration) {
        ...
        return resolution(companyId, declaration, created, "created", approvalId);
    }
    async function get(agentKey, companyId) {
        ...
        return resolution(companyId, declaration, agent, "resolved");
    }
    async function reconcile(agentKey, companyId) {
        const declaration = declarationFor(agentKey);
    ...
    async function reset(agentKey, companyId) {
        const declaration = declarationFor(agentKey);
        ...
        if (!updated)
            throw notFound("Managed agent not found");
        const updatedAgent = await materializeDeclaredInstructions(companyId, updated, declaration, { replaceExisting: true });
...
//# sourceMappingURL=plugin-managed-agents.js.map