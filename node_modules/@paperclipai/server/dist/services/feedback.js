...
import { agents, companies, companySkills, costEvents, documentRevisions, documents, feedbackExports, feedbackVotes, heartbeatRunEvents, heartbeatRuns, instanceSettings, issueComments, issueDocuments, issues, } from "@paperclipai/db";
...
import { DEFAULT_FEEDBACK_DATA_SHARING_PREFERENCE, DEFAULT_FEEDBACK_DATA_SHARING_TERMS_VERSION, instanceGeneralSettingsSchema, } from "@paperclipai/shared";
...
function captureStatusFromFiles(files) {
    ...
    const hasAdapterFiles = files.some((file) => file.source !== "paperclip_run" &&
...
async function buildCodexTraceFiles(input) {
    ...
    const managedRoot = path.join(resolvePaperclipInstanceRoot(), "companies", input.companyId, "codex-home", "sessions");
...
async function buildOpenCodeTraceFiles(input) {
    ...
    const opencodeRoot = resolveHomeAwarePath(process.env.PAPERCLIP_OPENCODE_STORAGE_DIR ?? "~/.local/share/opencode");
...
async function listIssueContextItems(db, issue) {
    const [commentRows, revisionRows] = await Promise.all([
        db
            ...
            .from(issueComments)
            .where(and(eq(issueComments.companyId, issue.companyId), eq(issueComments.issueId, issue.id), isNull(issueComments.deletedAt))),
        db
            .select({
...
async function buildFeedbackTraceBundleFromRow(db, row) {
    ...
    if (!sourceRunId) {
        ...
        if (!run || run.companyId !== row.companyId) {
            ...
            }, state, "bundle.paperclipRun", MAX_TRACE_FILE_CHARS);
            files.push(makeBundleFile({
            ...
            const sanitizedEvents = sanitizeFeedbackValue(events, state, "bundle.paperclipRun.events", MAX_TRACE_FILE_CHARS);
            files.push(makeBundleFile({
