...
function parsePositiveIntMs(value) {
    if (value === undefined)
        return undefined;
    ...
    if (!Number.isFinite(parsed) || !Number.isInteger(parsed) || parsed <= 0) {
        throw new Error(`PAPERCLIP_K8S_RPC_TIMEOUT_MS must be a positive integer of milliseconds (got "${value}").`);
    }
    return parsed;
}
 ...
 */
export function parseExecutionPolicyBootstrapEnv(env) {
    const raw = env.PAPERCLIP_EXECUTION_MODE?.trim();
    if (!raw || raw === "any")
        return null;
    if (raw !== "kubernetes") {
        throw new Error(`PAPERCLIP_EXECUTION_MODE must be "kubernetes" or "any" (got "${raw}").`);
    }
    ...
    if (backend) {
        if (backend !== "job" && backend !== "sandbox-cr") {
            throw new Error(`PAPERCLIP_K8S_BACKEND must be "job" or "sandbox-cr" (got "${backend}").`);
        }
    ...
    if (egressAllowFqdns)
        kubernetesConfig.egressAllowFqdns = egressAllowFqdns;
    const egressAllowCidrs = parseList(env.PAPERCLIP_K8S_EGRESS_ALLOW_CIDRS);
    if (egressAllowCidrs)
        kubernetesConfig.egressAllowCidrs = egressAllowCidrs;
    const adapters = parseAdapterRegistryEnv(env);
    if (adapters)
        kubernetesConfig.adapters = adapters;
 ...
 */
export async function applyExecutionPolicyBootstrap(db, bootstrap) {
    ...
    for (const companyId of companyIds) {
        try {
            ...
            logger.error({ err, companyId }, "failed to ensure managed Kubernetes environment during execution-policy bootstrap");
    ...
    if (failedCompanyIds.length > 0) {
        throw new Error(`execution-policy bootstrap: ${failedCompanyIds.length} of ${companyIds.length} companies failed to get a managed Kubernetes environment under executionMode=${bootstrap.executionMode}; refusing to start (companies: ${failedCompanyIds.join(", ")})`);
    }
 ...
 */
export async function bootstrapExecutionPolicyFromEnv(db, env = process.env) {
    const bootstrap = parseExecutionPolicyBootstrapEnv(env);
    if (!bootstrap)
        return null;
    return applyExecutionPolicyBootstrap(db, bootstrap);
}
//# sourceMappingURL=execution-policy-bootstrap.js.map