...
import { agents, pluginManagedResources, plugins, projects, routines, routineTriggers, } from "@paperclipai/db";
import { ROUTINE_STATUSES } from "@paperclipai/shared";
import { notFound, unprocessable } from "../errors.js";
import { logActivity } from "./activity-log.js";
import { routineService } from "./routines.js";
const MANAGED_ROUTINE_RESOURCE_KIND = "routine";
function buildRoutineDefaults(declaration) {
    return {
...
function normalizeRef(pluginKey, ref, resourceKind) {
    if (!ref)
        return null;
...
export function pluginManagedRoutineService(db, options) {
    const routinesSvc = routineService(db, {
        pluginWorkerManager: options.pluginWorkerManager,
    });
    function declarationFor(routineKey) {
    ...
    async function getBinding(companyId, routineKey) {
        return db
            .select({
            ...
            .where(and(eq(pluginManagedResources.companyId, companyId), eq(pluginManagedResources.pluginId, options.pluginId), eq(pluginManagedResources.resourceKind, MANAGED_ROUTINE_RESOURCE_KIND), eq(pluginManagedResources.resourceKey, routineKey)))
            .then((rows) => rows[0] ?? null);
    }
    async function upsertBinding(companyId, declaration, routineId) {
    ...
    async function getRoutineWithManagedBy(companyId, declaration) {
    ...
    async function resolveAgentId(companyId, declaration, overrides) {
        ...
        const binding = await db
            ...
            .from(pluginManagedResources)
            .where(and(eq(pluginManagedResources.companyId, companyId), eq(pluginManagedResources.pluginId, options.pluginId), eq(pluginManagedResources.resourceKind, "agent"), eq(pluginManagedResources.resourceKey, ref.resourceKey)))
            .then((rows) => rows[0] ?? null);
        if (!binding)
            return { agentId: null, missingRef: ref };
        const row = await db
            .select({ id: agents.id })
            .from(agents)
    ...
    async function resolveProjectId(companyId, declaration, overrides) {
        if (overrides?.projectId !== undefined) {
            if (!overrides.projectId)
                return { projectId: null, missingRef: null };
            const row = await db
        ...
        const binding = await db
            .select({ resourceId: pluginManagedResources.resourceId })
            .from(pluginManagedResources)
            .where(and(eq(pluginManagedResources.companyId, companyId), eq(pluginManagedResources.pluginId, options.pluginId), eq(pluginManagedResources.resourceKind, "project"), eq(pluginManagedResources.resourceKey, ref.resourceKey)))
            .then((rows) => rows[0] ?? null);
        if (!binding)
            return { projectId: null, missingRef: ref };
        const row = await db
            .select({ id: projects.id })
            .from(projects)
    ...
    async function resolveRefs(companyId, declaration, overrides) {
        ...
        const missingRefs = [];
        if (agent.missingRef)
            missingRefs.push(agent.missingRef);
        if (project.missingRef)
            missingRefs.push(project.missingRef);
        return {
    ...
    function resolution(companyId, declaration, routine, status, missingRefs = []) {
        return {
            pluginKey: options.pluginKey,
            resourceKind: "routine",
            resourceKey: declaration.routineKey,
    ...
    async function ensureDefaultTriggers(routineId, declaration) {
    ...
    async function createManagedRoutine(companyId, declaration, overrides) {
    ...
    async function reset(routineKey, companyId, overrides) {
        const declaration = declarationFor(routineKey);
        const current = await get(routineKey, companyId);
        if (!current.routine) {
            return createManagedRoutine(companyId, declaration, overrides);
        ...
        if (!updated)
            throw notFound("Managed routine not found");
    ...
    async function update(routineKey, companyId, patch) {
        const declaration = declarationFor(routineKey);
        const current = await get(routineKey, companyId);
        if (!current.routine)
            throw notFound("Managed routine not found");
    ...
    async function run(routineKey, companyId, overrides) {
        const declaration = declarationFor(routineKey);
        const current = await get(routineKey, companyId);
        if (!current.routine)
            throw notFound("Managed routine not found");
...
//# sourceMappingURL=plugin-managed-routines.js.map