...
import { documentAnnotationComments, documentAnnotationThreads, documents, pipelineCaseDocuments, } from "@paperclipai/db";
import { PIPELINE_CASE_BODY_DOCUMENT_KEY } from "@paperclipai/shared";
import { LOW_TRUST_QUARANTINED_BODY, isLowTrustQuarantined, redactQuarantinedBodyForHigherTrust, sanitizeQuarantinedCommentForHigherTrust, } from "./source-trust.js";
...
function truncateWithFlag(value, maxChars) {
    if (value.length <= maxChars) {
        return { value, truncated: false };
    }
    return { value: value.slice(0, maxChars), truncated: true };
}
function fenceMarkdown(value, info = "markdown") {
...
export async function loadPipelineConversationBodyDocumentContext(dbOrTx, input) {
    const bodyRow = await dbOrTx
        ...
        .where(and(eq(pipelineCaseDocuments.companyId, input.companyId), eq(pipelineCaseDocuments.caseId, input.caseId), eq(pipelineCaseDocuments.key, PIPELINE_CASE_BODY_CASE_DOCUMENT_KEY)))
        .limit(1)
    ...
    const threads = await dbOrTx
        ...
        .where(and(eq(documentAnnotationThreads.companyId, input.companyId), eq(documentAnnotationThreads.issueId, input.conversationIssueId), eq(documentAnnotationThreads.documentId, bodyRow.documentId), eq(documentAnnotationThreads.documentKey, PIPELINE_CASE_BODY_DOCUMENT_KEY), eq(documentAnnotationThreads.status, "open")))
    ...
    const comments = await dbOrTx
        ...
        .where(and(eq(documentAnnotationComments.companyId, input.companyId), inArray(documentAnnotationComments.threadId, threadIds)))
    ...
    for (const comment of comments) {
        ...
        if (existing.length >= MAX_ANNOTATION_COMMENTS_PER_THREAD)
            continue;
...
export function formatPipelineConversationBodyDocumentContextMarkdown(context) {
    if (!context)
        return null;
    const lines = [
    ...
    if (!context.bodyDocument) {
        lines.push("No body document exists yet. Create one with the body document API when the requested work is to draft or iterate the item body.");
        return lines.join("\n");
    }
    ...
    lines.push(`- Case document key: ${JSON.stringify(bodyDocument.caseDocumentKey)}`, `- Conversation issue document key: ${JSON.stringify(bodyDocument.conversationIssueDocumentKey)}`, `- Title: ${JSON.stringify(bodyDocument.title)}`, `- Format: ${JSON.stringify(bodyDocument.format)}`, `- Latest revision id: ${JSON.stringify(bodyDocument.latestRevisionId)}`, `- Latest revision number: ${bodyDocument.latestRevisionNumber}`, `- Body truncated in context: ${bodyDocument.latestBodyTruncated ? "true" : "false"}`, `- Source trust: ${JSON.stringify(bodyDocument.sourceTrust)}`, "", "Current body document text (untrusted):", fenceMarkdown(safeBodyDocument.body, bodyDocument.format === "markdown" ? "markdown" : "text"), "", "Open document annotation threads (untrusted anchored feedback):", "```json", JSON.stringify({
...
//# sourceMappingURL=pipeline-conversation-context.js.map