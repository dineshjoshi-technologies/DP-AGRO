...
import { projects, projectGoals, goals, issues, budgetPolicies, pluginManagedResources, plugins, projectWorkspaces, } from "@paperclipai/db";
import { deriveProjectUrlKey, hasNonAsciiContent, isUuidLike, normalizeProjectUrlKey, } from "@paperclipai/shared";
...
async function attachWorkspaces(db, rows) {
    ...
    const runtimeServicesByWorkspaceId = await listCurrentRuntimeServicesForProjectWorkspaces(db, rows[0].companyId, workspaceRows.map((workspace) => workspace.id));
    ...
    const managedRows = await db
        ...
        .where(and(eq(pluginManagedResources.resourceKind, "project"), inArray(pluginManagedResources.resourceId, projectIds)));
    const managedByProjectId = new Map();
 ...
 */
export function buildProjectListMetricMaps(taskCountRows, budgetRows) {
 ...
 */
async function attachListMetrics(db, companyId, rows) {
    if (rows.length === 0)
        return rows;
    const projectIds = rows.map((r) => r.id);
    const [taskCountRows, budgetRows] = await Promise.all([
        ...
        db
            ...
            .from(budgetPolicies)
            .where(and(eq(budgetPolicies.companyId, companyId), eq(budgetPolicies.scopeType, "project"), eq(budgetPolicies.metric, "billed_cents"), eq(budgetPolicies.isActive, true), inArray(budgetPolicies.scopeId, projectIds))),
    ]);
...
function buildManagedProjectDefaults(declaration) {
    return {
    ...
    return {
        ...
        resolveManagedProject: async (input) => {
            ...
            const existingBinding = await db
                .select()
                .from(pluginManagedResources)
                .where(and(eq(pluginManagedResources.companyId, input.companyId), eq(pluginManagedResources.pluginId, input.pluginId), eq(pluginManagedResources.resourceKind, "project"), eq(pluginManagedResources.resourceKey, input.projectKey)))
                .then((rows) => rows[0] ?? null);
            if (existingBinding) {
        ...
        clearExecutionWorkspaceEnvironmentSelection: async (companyId, environmentId) => {
            ...
            for (const row of rows) {
                ...
                if (policy?.environmentId !== environmentId)
                    continue;
                await db
                    .update(projects)
                    .set({
        ...
        listWorkspaces: async (projectId) => {
            ...
            if (rows.length === 0)
                return [];
            const runtimeServicesByWorkspaceId = await listCurrentRuntimeServicesForProjectWorkspaces(db, rows[0].companyId, rows.map((workspace) => workspace.id));
