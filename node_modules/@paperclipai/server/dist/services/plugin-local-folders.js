...
function problem(code, message, problemPath) {
    return { code, message, path: problemPath };
}
export function assertPluginLocalFolderKey(folderKey) {
...
export function findLocalFolderDeclaration(declarations, folderKey) {
    return declarations?.find((declaration) => declaration.folderKey === folderKey) ?? null;
}
export function requireLocalFolderDeclaration(declarations, folderKey) {
...
function validateRequiredPath(pathValue, label) {
    try {
...
function normalizeListRelativePath(relativePath) {
...
function mergeFolderConfig(declaration, stored, override) {
...
export function getStoredLocalFolders(settingsJson) {
    const folders = settingsJson?.localFolders;
    if (!folders || typeof folders !== "object")
        return {};
    return folders;
}
export function setStoredLocalFolder(settingsJson, folderKey, config) {
    return {
        ...(settingsJson ?? {}),
        localFolders: {
...
export async function inspectPluginLocalFolder(input) {
    ...
    const config = mergeFolderConfig(input.declaration ?? null, input.storedConfig ?? null, input.overrideConfig);
    const access = config?.access ?? input.declaration?.access ?? "readWrite";
    const requiredDirectories = (config?.requiredDirectories ?? []).map((item) => validateRequiredPath(item, "requiredDirectories"));
    const requiredFiles = (config?.requiredFiles ?? []).map((item) => validateRequiredPath(item, "requiredFiles"));
...
async function assertPathInsideRoot(rootRealPath, candidatePath) {
...
async function ensureDirectoryInsideRoot(rootRealPath, relativePath) {
...
export async function preparePluginLocalFolder(input) {
    ...
    const config = mergeFolderConfig(input.declaration ?? null, input.storedConfig ?? null, input.overrideConfig);
    const access = config?.access ?? input.declaration?.access ?? "readWrite";
    if (!config?.path || access !== "readWrite" || !path.isAbsolute(config.path))
        return;
    const configuredPath = path.resolve(config.path);
    try {
        const stat = await fs.stat(configuredPath);
...
async function inspectChildPath(rootRealPath, relativePath, kind) {
    let resolvedPath;
    try {
...
export async function resolvePluginLocalFolderPath(rootPath, relativePath, options) {
    const normalized = normalizeRelativePath(relativePath);
...
export async function readPluginLocalFolderText(rootPath, relativePath) {
...
export async function listPluginLocalFolderEntries(rootPath, options = {}) {
    const rootRealPath = await fs.realpath(rootPath);
    ...
    const entries = [];
    let truncated = false;
    const visit = async (directoryRealPath, directoryRelativePath) => {
        if (truncated)
            return;
...
export async function writePluginLocalFolderTextAtomic(rootPath, relativePath, contents) {
    const rootRealPath = await fs.realpath(rootPath);
    const normalized = normalizeRelativePath(relativePath);
    ...
    const tempPath = path.join(path.dirname(resolved.absolutePath), `.paperclip-${path.basename(resolved.absolutePath)}-${process.pid}-${randomUUID()}.tmp`);
...
export async function deletePluginLocalFolderFile(rootPath, relativePath, folderKey) {
    const rootRealPath = await fs.realpath(rootPath);
    let resolved;
    try {
...
export function defaultLocalFolderBasePath(pluginKey, companyId) {
    return path.join(os.homedir(), ".paperclip", "plugin-data", companyId, pluginKey);
}
export function assertConfiguredLocalFolder(status) {
...
export function assertWritableConfiguredLocalFolder(status) {
    if (!status.configured || !status.realPath || !status.readable) {
        throw notFound("Local folder is not configured or readable");
    }
    const onlyMissingRequiredPaths = status.problems.every((item) => item.code === "missing_directory" || item.code === "missing_file");
    if (!status.healthy && !onlyMissingRequiredPaths) {
        throw badRequest("Local folder is not healthy");
    }
}
//# sourceMappingURL=plugin-local-folders.js.map