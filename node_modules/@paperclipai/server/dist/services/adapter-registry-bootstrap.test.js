...
describe("parseAdapterRegistryEnv", () => {
    ...
    it("throws on malformed JSON (fail loud)", () => {
        expect(() => parseAdapterRegistryEnv({ PAPERCLIP_ADAPTERS: "{not json" })).toThrow(/PAPERCLIP_ADAPTERS/);
    });
    it("throws on schema-invalid content (fail loud)", () => {
        expect(() => parseAdapterRegistryEnv({ PAPERCLIP_ADAPTERS: JSON.stringify([{ enabled: true }]) })).toThrow(/PAPERCLIP_ADAPTERS/);
    });
});
//# sourceMappingURL=adapter-registry-bootstrap.test.js.map