...
export function createPluginDevWatcher(lifecycle, resolvePluginPackagePath, fsDeps) {
    ...
    const fileExists = fsDeps?.existsSync ?? existsSync;
    log.info({ resolvesInstalledPlugins: Boolean(resolvePluginPackagePath) }, "plugin-dev-watcher: initialized");
    ...
    async function watchLocalPluginById(pluginId) {
        if (!resolvePluginPackagePath) {
            log.debug({ pluginId }, "plugin-dev-watcher: no package path resolver configured, skipping lifecycle event");
            return;
        }
        try {
