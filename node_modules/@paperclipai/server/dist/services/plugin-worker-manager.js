...
export function createPluginWorkerHandle(pluginId, options) {
    ...
    function errorCodeForWorkerHostError(err) {
        const code = err?.code;
        const pluginErrorCodes = Object.values(PLUGIN_RPC_ERROR_CODES);
    ...
    function deriveInvocationScope(method, params) {
        if (!isRecord(params))
            return null;
        const directCompanyId = readNonEmptyString(params.companyId);
        if (directCompanyId)
            return { companyId: directCompanyId };
    ...
    function registerInvocation(scope, ttlMs) {
        const invocation = {
            id: randomUUID(),
            scope,
        };
    ...
    function clearInvocation(invocation) {
        if (!invocation)
            return;
    ...
    function contextForWorkerMessage(message) {
        const invocationId = readNonEmptyString(message.paperclipInvocationId);
        if (!invocationId) {
    ...
    function handleWorkerNotification(notification) {
        ...
        if (notification.method === "streams.open" ||
            ...
            if (context.invalidInvocationScope) {
                log.warn({ method: notification.method, companyId }, "dropping plugin stream notification with invalid invocation scope");
                return;
            }
            const allowedCompanyId = context.invocationScope?.companyId;
            if (allowedCompanyId && companyId !== allowedCompanyId) {
                log.warn({ method: notification.method, companyId, allowedCompanyId }, "dropping plugin stream notification outside invocation company scope");
                return;
            }
            ...
            if (notification.method === "streams.open") {
                ...
                if (ch)
                    openStreamChannels.set(ch, companyId);
            }
