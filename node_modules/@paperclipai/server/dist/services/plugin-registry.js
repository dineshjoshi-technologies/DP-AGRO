...
import { plugins, pluginConfig, pluginCompanySettings, pluginEntities, pluginJobs, pluginJobRuns, pluginWebhookDeliveries, } from "@paperclipai/db";
 ...
 */
function isPluginKeyConflict(error) {
 ...
 */
export function pluginRegistryService(db) {
    // -----------------------------------------------------------------------
    // Internal helpers
    // -----------------------------------------------------------------------
    ...
    async function getByKey(pluginKey) {
        return db
            .select()
            .from(plugins)
    ...
    async function nextInstallOrder() {
        const result = await db
            .select({ maxOrder: sql `coalesce(max(${plugins.installOrder}), 0)` })
            .from(plugins);
    ...
    return {
        ...
        update: async (id, data) => {
            ...
            if (data.version !== undefined)
                setClause.version = data.version;
         ...
         */
        uninstall: async (id, removeData = false) => {
            const plugin = await getById(id);
            if (!plugin)
                throw notFound("Plugin not found");
            if (removeData) {
         ...
         */
        upsertConfig: async (pluginId, input) => {
         ...
         */
        patchConfig: async (pluginId, input) => {
            const plugin = await getById(pluginId);
            if (!plugin)
                throw notFound("Plugin not found");
            const existing = await db
         ...
         */
        setConfigError: async (pluginId, lastError) => {
            const rows = await db
                .update(pluginConfig)
                ...
                .returning();
            if (rows.length === 0)
                throw notFound("Plugin config not found");
            return rows[0];
        },
        /** Delete a plugin's config row. */
        deleteConfig: async (pluginId) => {
            const rows = await db
                .delete(pluginConfig)
                .where(eq(pluginConfig.pluginId, pluginId))
                .returning();
        ...
        getCompanySettings: (pluginId, companyId) => db
            .select()
        ...
        upsertCompanySettings: async (pluginId, companyId, input) => {
            const plugin = await getById(pluginId);
            if (!plugin)
                throw notFound("Plugin not found");
            const existing = await db
         ...
         */
        getEntityByExternalId: (pluginId, entityType, externalId, companyId) => {
            const companyIdPredicate = companyId == null
                ? isNull(pluginEntities.companyId)
                : eq(pluginEntities.companyId, companyId);
            return db
                .select()
                .from(pluginEntities)
                .where(and(companyIdPredicate, eq(pluginEntities.pluginId, pluginId), eq(pluginEntities.entityType, entityType), eq(pluginEntities.externalId, externalId)))
                .then((rows) => rows[0] ?? null);
        },
        /**
         ...
         */
        upsertEntity: async (pluginId, input) => {
            ...
            const existing = await db
                .select()
                .from(pluginEntities)
                .where(and(companyIdPredicate, eq(pluginEntities.pluginId, pluginId), eq(pluginEntities.entityType, input.entityType), eq(pluginEntities.externalId, input.externalId ?? "")))
                .then((rows) => rows[0] ?? null);
            if (existing) {
                return db
         ...
         */
        getJobByKey: (pluginId, jobKey) => db
            .select()
            .from(pluginJobs)
         ...
         */
        upsertJob: async (pluginId, jobKey, input) => {
            const existing = await db
                .select()
                .from(pluginJobs)
                .where(and(eq(pluginJobs.pluginId, pluginId), eq(pluginJobs.jobKey, jobKey)))
         ...
         */
        createJobRun: async (pluginId, jobId, trigger, companyId) => {
            return db
         ...
         */
        updateJobRun: async (runId, input) => {
            return db
         ...
         */
        createWebhookDelivery: async (pluginId, webhookKey, companyId, input) => {
            return db
         ...
         */
        updateWebhookDelivery: async (deliveryId, input) => {
            return db
