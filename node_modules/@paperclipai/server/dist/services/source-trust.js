...
export const LOW_TRUST_QUARANTINED_BODY = "[Quarantined low-trust output omitted from higher-trust agent context. A trusted reviewer can inspect and promote a sanitized artifact.]";
export function isLowTrustQuarantined(sourceTrust) {
    return sourceTrust?.preset === LOW_TRUST_REVIEW_PRESET && sourceTrust.disposition === "quarantined";
}
export function redactQuarantinedBodyForHigherTrust(value) {
    if (!isLowTrustQuarantined(value.sourceTrust))
        return value;
    return {
        ...value,
...
export function sanitizeQuarantinedCommentForHigherTrust(comment) {
    if (!isLowTrustQuarantined(comment.sourceTrust))
        return comment;
    return {
        ...comment,
        body: LOW_TRUST_QUARANTINED_BODY,
