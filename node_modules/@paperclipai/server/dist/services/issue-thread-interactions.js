...
import { documents, heartbeatRuns, issueComments, issueDocuments, issueThreadInteractions, issues, } from "@paperclipai/db";
import { acceptIssueThreadInteractionSchema, askUserQuestionsPayloadSchema, askUserQuestionsResultSchema, cancelIssueThreadInteractionSchema, createIssueThreadInteractionSchema, rejectIssueThreadInteractionSchema, requestCheckboxConfirmationPayloadSchema, requestCheckboxConfirmationResultSchema, requestConfirmationPayloadSchema, requestConfirmationResultSchema, suggestTasksPayloadSchema, suggestTasksResultSchema, } from "@paperclipai/shared";
import { conflict, notFound, unprocessable } from "../errors.js";
import { issueService, runWorkspaceIsFinalized } from "./issues.js";
const ISSUE_THREAD_INTERACTION_IDEMPOTENCY_CONSTRAINT = "issue_thread_interactions_company_issue_idempotency_uq";
const REQUEST_CONFIRMATION_INTERACTION_KINDS = [
    "request_confirmation",
    "request_checkbox_confirmation",
];
function isRequestConfirmationLikeKind(kind) {
    return REQUEST_CONFIRMATION_INTERACTION_KINDS.includes(kind);
}
function isIssueThreadInteractionIdempotencyConflict(error) {
...
function isEquivalentCreateRequest(row, input, actor) {
...
async function touchIssue(db, issueId) {
    await db
        .update(issues)
...
function shouldReturnAcceptedConfirmationToCreatorAgent(args) {
    if (!isRequestConfirmationLikeKind(args.current.kind))
        return false;
    if (!args.current.createdByAgentId)
        return false;
    if (!args.actor.userId)
        return false;
    if (!args.issue.assigneeUserId)
        return false;
    if (args.issue.assigneeAgentId)
        return false;
    if (isTerminalIssueStatus(args.issue.status))
        return false;
    return true;
}
function shouldSupersedeRequestConfirmationOnUserComment(interaction) {
    return interaction.payload.supersedeOnUserComment === true;
}
function normalizeCreateInteractionInput(input) {
    switch (input.kind) {
        case "request_confirmation":
            return {
                ...input,
...
function isCommentAtOrAfterInteraction(args) {
    ...
    if (!Number.isFinite(commentCreatedAtMs) || !Number.isFinite(interactionCreatedAtMs))
        return false;
    return commentCreatedAtMs >= interactionCreatedAtMs;
}
function buildTaskCreationOrder(tasks) {
    const taskByClientKey = new Map(tasks.map((task) => [task.clientKey, task]));
    const ordered = [];
    const state = new Map();
    const visit = (clientKey) => {
...
function resolveSelectedSuggestedTasks(args) {
    const taskByClientKey = new Map(args.interaction.payload.tasks.map((task) => [task.clientKey, task]));
    const selectedClientKeys = args.selectedClientKeys ?? args.interaction.payload.tasks.map((task) => task.clientKey);
    const selectedClientKeySet = new Set();
...
async function getIssueDocumentTargetSnapshot(db, args) {
    ...
    const row = await db
        ...
        .where(and(eq(issueDocuments.companyId, args.companyId), eq(issueDocuments.issueId, targetIssueId), eq(issueDocuments.key, args.target.key)))
...
function buildIssueDocumentTargetFromSnapshot(args) {
    if (!args.snapshot?.latestRevisionId)
        return null;
    return {
        type: "issue_document",
...
function buildIssueDocumentTargetFromDocument(args) {
    if (!args.document?.latestRevisionId)
        return null;
    return {
        type: "issue_document",
...
async function assertRequestConfirmationTargetIsCurrent(db, args) {
    if (!args.target)
        return;
    if (args.target.type !== "issue_document")
        return;
...
async function expireStaleRequestConfirmationTarget(db, args) {
    if (!isRequestConfirmationLikeKind(args.row.kind) || args.row.status !== "pending")
        return null;
    const interaction = hydrateInteraction(args.row);
    ...
    const [updated] = await db
        ...
        .where(and(eq(issueThreadInteractions.id, args.row.id), eq(issueThreadInteractions.status, "pending")))
        .returning();
    if (!updated) {
...
export function issueThreadInteractionService(db) {
    async function getIdempotentInteraction(args) {
        return db
            .select()
            .from(issueThreadInteractions)
            .where(and(eq(issueThreadInteractions.companyId, args.companyId), eq(issueThreadInteractions.issueId, args.issueId), eq(issueThreadInteractions.idempotencyKey, args.idempotencyKey)))
            .then((rows) => rows[0] ?? null);
    }
    async function assertIssueWorkspaceFinalizedForAccept(args) {
        ...
        if (!executionWorkspaceId)
            return;
        const isFinalized = await runWorkspaceIsFinalized(args.db, args.issue.companyId, executionWorkspaceId, args.sourceRunId);
        if (isFinalized)
            return;
        throw conflict("Cannot accept interaction: the run that created this interaction has not finished syncing its workspace. "
            + "Retry once the local worktree has finished syncing.", { executionWorkspaceId, sourceRunId: args.sourceRunId });
    }
    ...
    async function acceptRequestConfirmation(args) {
        ...
        return db.transaction(async (tx) => {
            const [updated] = await tx
                ...
                .where(and(eq(issueThreadInteractions.id, args.current.id), eq(issueThreadInteractions.status, "pending")))
                .returning();
            if (!updated) {
                throw conflict("Interaction has already been resolved");
            }
    ...
    return {
        ...
        acceptSuggestedTasks: async (issue, interactionId, input, actor) => {
            ...
            await db.transaction(async (tx) => {
                ...
                for (const task of orderedTasks) {
                    ...
                    const { issue: createdIssue } = await issueService(tx).createChild(parentIssueId, {
                        title: task.title,
        ...
        rejectSuggestedTasks: async (issue, interactionId, input, actor, current) => {
            if (current.companyId !== issue.companyId || current.issueId !== issue.id) {
                throw notFound("Interaction not found");
            }
            if (current.kind !== "suggest_tasks") {
        ...
        expireRequestConfirmationsSupersededByComment: async (issue, comment, actor) => {
            if (!comment.authorUserId)
                return [];
            const rows = await db
                .select()
                .from(issueThreadInteractions)
                .where(and(eq(issueThreadInteractions.companyId, issue.companyId), eq(issueThreadInteractions.issueId, issue.id), inArray(issueThreadInteractions.kind, [...REQUEST_CONFIRMATION_INTERACTION_KINDS]), eq(issueThreadInteractions.status, "pending")));
            const superseded = rows.filter((row) => {
                const interaction = hydrateInteraction(row);
                return (shouldSupersedeRequestConfirmationOnUserComment(interaction)
        ...
        expireRequestConfirmationsSupersededByHistoricalComments: async (issue) => {
            const [rows, comments] = await Promise.all([
                ...
                db
                    .select()
                    .from(issueComments)
                    .where(and(eq(issueComments.companyId, issue.companyId), eq(issueComments.issueId, issue.id), isNotNull(issueComments.authorUserId)))
            ...
            for (const row of rows) {
                const interaction = hydrateInteraction(row);
                if (!shouldSupersedeRequestConfirmationOnUserComment(interaction))
                    continue;
                const supersedingComment = comments.find((comment) => isCommentAtOrAfterInteraction({
                    commentCreatedAt: comment.createdAt,
                    interactionCreatedAt: row.createdAt,
                }));
                if (!supersedingComment)
                    continue;
            ...
            for (const { comment, rowIds } of supersededByComment.values()) {
                const updatedRows = await db
                    ...
                    .where(and(inArray(issueThreadInteractions.id, rowIds), eq(issueThreadInteractions.status, "pending")))
                    .returning();
        ...
        expireStaleRequestConfirmationsForIssueDocument: async (issue, document, actor) => {
            const rows = await db
                .select()
                .from(issueThreadInteractions)
                .where(and(eq(issueThreadInteractions.companyId, issue.companyId), eq(issueThreadInteractions.issueId, issue.id), inArray(issueThreadInteractions.kind, [...REQUEST_CONFIRMATION_INTERACTION_KINDS]), eq(issueThreadInteractions.status, "pending")));
            const staleRows = rows.filter((row) => {
                ...
                if (!target || target.type !== "issue_document")
                    return false;
                ...
                if (document && target.documentId && target.documentId !== document.id)
                    return false;
                if (document && target.key !== document.key)
                    return false;
                if (!document)
                    return true;
                return (target.revisionId !== document.latestRevisionId
        ...
        cancelQuestions: async (issue, interactionId, input, actor) => {
...
//# sourceMappingURL=issue-thread-interactions.js.map