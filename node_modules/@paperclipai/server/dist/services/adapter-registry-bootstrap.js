 ...
 */
export function parseAdapterRegistryEnv(env = process.env) {
    ...
    if (inline) {
        ...
        try {
            ...
            throw new Error(`PAPERCLIP_ADAPTERS_FILE could not be read at "${filePath}": ${err.message}`);
        }
    }
    let parsed;
    try {
        ...
        throw new Error(`PAPERCLIP_ADAPTERS must be valid JSON: ${err.message}`);
    }
    const result = adapterRegistrySchema.safeParse(parsed);
    if (!result.success) {
        throw new Error(`PAPERCLIP_ADAPTERS failed validation: ${result.error.issues
 ...
 */
export function reconcileAdapterAvailability(registry) {
    if (!registry)
    ...
    if (missing.length > 0) {
        throw new Error(`PAPERCLIP_ADAPTERS declares adapter type(s) with no installed adapter: ${missing.join(", ")}`);
    }
    const enabled = [];
    const disabled = [];
...
//# sourceMappingURL=adapter-registry-bootstrap.js.map