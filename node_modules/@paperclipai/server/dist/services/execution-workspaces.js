...
async function inspectGitCloseReadiness(workspace) {
    ...
    const expectsGitInspection = workspace.providerType === "git_worktree" ||
    ...
    try {
        ...
        warnings.push(`Could not inspect git status for "${workspacePath}": ${error instanceof Error ? error.message : String(error)}`);
    }
    ...
    if (repoRoot) {
        try {
            ...
            warnings.push(`Could not read git working tree status for "${workspacePath}": ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    let aheadCount = null;
    let behindCount = null;
    let isMergedIntoBase = null;
    ...
    if (repoRoot && baseRef) {
        try {
            ...
            warnings.push(`Could not compare this workspace against ${baseRef}: ${error instanceof Error ? error.message : String(error)}`);
        }
        try {
            ...
            if (code === 1)
                ...
                warnings.push(`Could not determine whether this workspace is merged into ${baseRef}: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
    }
    return {
...
export function readExecutionWorkspaceConfig(metadata) {
...
export function mergeExecutionWorkspaceConfig(metadata, patch) {
...
function toExecutionWorkspace(row, runtimeServices = []) {
    return {
        id: row.id,
        companyId: row.companyId,
        ...
        config: readExecutionWorkspaceConfig(row.metadata ?? null),
...
function toExecutionWorkspaceSummary(row) {
    return {
        id: row.id,
        name: row.name,
        mode: row.mode,
...
function toWorkspaceOverviewPrimaryService(service) {
    if (!service)
        return null;
    return {
        id: service.id,
...
function selectPrimaryOverviewService(services) {
...
export function executionWorkspaceService(db) {
    ...
    function buildOverviewConditions(companyId, filters) {
        const conditions = [eq(executionWorkspaces.companyId, companyId)];
        if (filters.projectId)
            conditions.push(eq(executionWorkspaces.projectId, filters.projectId));
        if (filters.status && filters.status.length > 0) {
            if (filters.status.length === 1)
                conditions.push(eq(executionWorkspaces.status, filters.status[0]));
            else
                conditions.push(inArray(executionWorkspaces.status, filters.status));
        }
        else {
    ...
    return {
        listOverview: async (companyId, filters) => {
            ...
            const [totalRow, rows] = await Promise.all([
                db
                    ...
                    .innerJoin(projects, and(eq(projects.id, executionWorkspaces.projectId), eq(projects.companyId, companyId)))
                    .where(whereClause)
                ...
                db
                    ...
                    .from(executionWorkspaces)
                    .innerJoin(projects, and(eq(projects.id, executionWorkspaces.projectId), eq(projects.companyId, companyId)))
                    .leftJoin(projectWorkspaces, and(eq(projectWorkspaces.id, executionWorkspaces.projectWorkspaceId), eq(projectWorkspaces.companyId, companyId)))
                    .where(whereClause)
                    .orderBy(desc(executionWorkspaces.lastUsedAt), desc(executionWorkspaces.updatedAt), asc(executionWorkspaces.id))
                    .limit(filters.limit)
            ...
            const [runtimeServicesByWorkspaceId, linkedIssueCountRows, linkedIssueRows] = await Promise.all([
                ...
                db
                    ...
                    .from(issues)
                    .where(and(eq(issues.companyId, companyId), isNull(issues.hiddenAt), inArray(issues.executionWorkspaceId, workspaceIds)))
        ...
        `),
            ...
            const linkedIssueCountByWorkspaceId = new Map(linkedIssueCountRows
                .filter((row) => row.executionWorkspaceId)
                .map((row) => [row.executionWorkspaceId, row.count]));
            const linkedIssuesByWorkspaceId = new Map();
            ...
            const items = pageRows.map((row) => {
                ...
                return {
                    ...
                    lastUpdatedAt: maxDate(row.lastUsedAt, row.updatedAt, linkedIssues[0]?.updatedAt, primaryServiceSummary?.updatedAt),
                    projectWorkspaceId: row.projectWorkspaceId ?? null,
                    executionWorkspaceId: row.id,
                    executionWorkspaceStatus: row.status,
        ...
        listSummaries: async (companyId, filters) => {
            const conditions = buildListConditions(companyId, filters);
            const rows = await db
                .select({
                id: executionWorkspaces.id,
        ...
        getCloseReadiness: async (id) => {
            ...
            const projectWorkspace = workspace.projectWorkspaceId
                ? await db
                    ...
                    .where(and(eq(projectWorkspaces.companyId, workspace.companyId), eq(projectWorkspaces.id, workspace.projectWorkspaceId)))
                    .then((rows) => rows[0] ?? null)
                : null;
            const primaryProjectWorkspace = workspace.projectId
                ? await db
                    ...
                    .from(projectWorkspaces)
                    .where(and(eq(projectWorkspaces.companyId, workspace.companyId), eq(projectWorkspaces.projectId, workspace.projectId), eq(projectWorkspaces.isPrimary, true)))
                    .then((rows) => rows[0] ?? null)
                : null;
            ...
            const isProjectPrimaryWorkspace = workspace.projectWorkspaceId != null
            ...
            if (blockingIssues.length > 0) {
                const linkedIssueMessage = blockingIssues.length === 1
            ...
            if (runtimeServices.some((service) => service.status !== "stopped")) {
                warnings.push(runtimeServices.length === 1
            ...
            if (git?.hasDirtyTrackedFiles) {
                warnings.push(git.dirtyEntryCount === 1
            ...
            if (git?.hasUntrackedFiles) {
                warnings.push(git.untrackedEntryCount === 1
            ...
            if (git?.aheadCount && git.aheadCount > 0 && git.isMergedIntoBase === false) {
                warnings.push(git.aheadCount === 1
            ...
            if (git?.behindCount && git.behindCount > 0) {
                warnings.push(git.behindCount === 1
            ...
            if (runtimeServices.some((service) => service.status !== "stopped")) {
                plannedActions.push({
                    ...
                    description: runtimeServices.length === 1
