...
import { AGENT_DEFAULT_MAX_CONCURRENT_RUNS, getAgentWorkEligibility, isUuidLike, normalizeAgentApiKeyScope, normalizeAgentUrlKey, } from "@paperclipai/shared";
...
export function agentService(db) {
    ...
    function normalizeAgentBaseRow(row) {
        return withUrlKey({
            ...row,
            permissions: normalizeAgentPermissions(row.permissions, row.role),
        });
    }
    function toEligibilityAgent(row) {
        return {
            id: row.id,
    ...
    function normalizeAgentRows(rows, allCompanyRows = rows) {
    ...
    function normalizeAgentRow(row, allCompanyRows) {
        return normalizeAgentRows([row], allCompanyRows)[0];
    }
    async function listCompanyAgentRows(companyId) {
    ...
    async function requireGetById(id) {
    ...
    async function syncAgentSecretBindings(agent, dbClient = db) {
    ...
    async function updateAgent(id, data, options) {
        ...
        if (Object.prototype.hasOwnProperty.call(normalizedPatch, "adapterConfig") &&
            ...
            normalizedPatch.adapterConfig = await secretsSvc.normalizeAdapterConfigForPersistence(existing.companyId, normalizedPatch.adapterConfig, { adapterType: (normalizedPatch.adapterType ?? existing.adapterType) });
        }
    ...
    return {
        ...
        remove: async (id) => {
            ...
            return db.transaction(async (tx) => {
                ...
                await tx.delete(activityLog).where(or(eq(activityLog.agentId, id), sql `${activityLog.runId} in (select ${heartbeatRuns.id} from ${heartbeatRuns} where ${heartbeatRuns.agentId} = ${id})`));
        ...
        createApiKey: async (id, name, scope = { kind: "standard" }) => {
            const existing = await getById(id);
            if (!existing)
                throw notFound("Agent not found");
            if (existing.status === "pending_approval") {
