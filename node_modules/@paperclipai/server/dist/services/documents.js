...
function nextAvailableDocumentKey(sourceKey, existingKeys) {
...
export function extractLegacyPlanBody(description) {
    if (!description)
        return null;
...
function mapIssueDocumentRow(row, includeBody) {
    return {
        id: row.id,
...
export function documentService(db) {
    ...
    return {
        ...
        listIssueDocumentRevisions: async (issueId, rawKey) => {
            const key = normalizeDocumentKey(rawKey);
            return db
                .select({
        ...
        unlockIssueDocument: async (issueId, rawKey) => {
            const key = normalizeDocumentKey(rawKey);
            return db.transaction(async (tx) => {
                const existing = await tx
                    .select(issueDocumentSelect)
        ...
        deleteIssueDocument: async (issueId, rawKey) => {
            const key = normalizeDocumentKey(rawKey);
            return db.transaction(async (tx) => {
                const existing = await tx
                    .select(issueDocumentSelect)
