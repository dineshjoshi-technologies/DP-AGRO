...
import { activityLog, agentWakeupRequests, agents, approvals, assets, companies, companyMemberships, documentRevisions, documents, goals, heartbeatRuns, executionWorkspaces, issueApprovals, issueAttachments, issueInboxArchives, issueLabels, issueWatchdogs, issuePlanDecompositions, issueRecoveryActions, issueRelations, issueComments, issueDocuments, issueReadStates, issueThreadInteractions, issues, labels, projectWorkspaces, projects, workspaceOperations, } from "@paperclipai/db";
import { clampIssueRequestDepth, extractAgentMentionIds, extractProjectMentionIds, issueCommentAuthorTypeSchema, issueCommentMetadataSchema, issueCommentPresentationSchema, isUuidLike, normalizeIssueIdentifier as normalizeIssueReferenceIdentifier, } from "@paperclipai/shared";
...
import { summarizeIssueWatchdog, upsertIssueWatchdogForIssue, } from "./task-watchdogs.js";
...
export function parseStatusFilter(input) {
...
async function findAcceptedPlanDocumentInteraction(dbOrTx, input) {
    const rows = await dbOrTx
        ...
        .where(and(eq(issueThreadInteractions.companyId, input.companyId), eq(issueThreadInteractions.issueId, input.sourceIssueId), eq(issueThreadInteractions.kind, "request_confirmation"), eq(issueThreadInteractions.status, "accepted")))
 ...
 */
export async function listUnfinalizedExecutionWorkspaceIds(dbOrTx, companyId, executionWorkspaceIds) {
    ...
    const rows = await dbOrTx
        ...
        .where(and(eq(workspaceOperations.companyId, companyId), inArray(workspaceOperations.executionWorkspaceId, executionWorkspaceIds)));
...
async function listPendingFinalizeBlockerIssueIds(dbOrTx, companyId, blockerWorkspacePairs) {
    ...
    if (blockerIssueIds.length === 0 || executionWorkspaceIds.length === 0)
        return pending;
    const blockerWorkspaceKeys = new Set(blockerWorkspacePairs.map((pair) => `${pair.blockerIssueId}:${pair.executionWorkspaceId}`));
    const rows = await dbOrTx
        .select({
        ...
        .from(workspaceOperations)
        .where(and(eq(workspaceOperations.companyId, companyId), inArray(workspaceOperations.executionWorkspaceId, executionWorkspaceIds), or(inArray(workspaceOperations.issueId, blockerIssueIds), isNull(workspaceOperations.issueId))));
 ...
 */
export async function runWorkspaceIsFinalized(dbOrTx, companyId, executionWorkspaceId, runId) {
    const rows = await dbOrTx
        .select({
        phase: workspaceOperations.phase,
        status: workspaceOperations.status,
        ...
        .from(workspaceOperations)
        .where(and(eq(workspaceOperations.companyId, companyId), eq(workspaceOperations.executionWorkspaceId, executionWorkspaceId), eq(workspaceOperations.heartbeatRunId, runId)));
    let latest = null;
    for (const row of rows) {
...
async function listIssueDependencyReadinessMap(dbOrTx, companyId, issueIds) {
    ...
    const pendingFinalizeBlockerIssueIds = await listPendingFinalizeBlockerIssueIds(dbOrTx, companyId, doneBlockerWorkspacePairs);
    for (const row of blockerRows) {
...
function issueListOrderBy(companyId, { hasSearch, priorityOrder, searchOrder, sortField, sortDir, }) {
...
async function watchdogMapForIssues(dbOrTx, rows) {
    const map = new Map();
    if (rows.length === 0)
        return map;
    ...
    for (const [companyId, issueIds] of byCompany.entries()) {
        for (const issueIdChunk of chunkList([...new Set(issueIds)], ISSUE_LIST_RELATED_QUERY_CHUNK_SIZE)) {
            const watchdogRows = await dbOrTx
                .select()
                .from(issueWatchdogs)
                .where(and(eq(issueWatchdogs.companyId, companyId), inArray(issueWatchdogs.issueId, issueIdChunk), eq(issueWatchdogs.status, "active")));
...
function lowTrustBoundaryIssueCondition(companyId, boundary) {
    if (!boundary || boundary.companyId !== companyId)
        return null;
    const clauses = [];
    const issueIds = [...new Set(boundary.issueIds ?? [])];
    const projectIds = [...new Set(boundary.projectIds ?? [])];
    if (issueIds.length > 0)
        clauses.push(inArray(issues.id, issueIds));
    ...
    for (let depth = 0; frontier.length > 0 && depth < BLOCKER_ATTENTION_MAX_DEPTH; depth += 1) {
        ...
        for (const chunk of chunkList([...new Set(frontier)], ISSUE_LIST_RELATED_QUERY_CHUNK_SIZE)) {
            ...
            const childRowsPromise = dbOrTx
                ...
                .from(issues)
                .where(and(eq(issues.companyId, companyId), inArray(issues.parentId, chunk), notInArray(issues.status, BLOCKER_ATTENTION_CHILD_TERMINAL_STATUSES)));
...
function hasPlanDocumentCondition(companyId, hasPlanDocument) {
    const existsPlanDocument = sql `
    EXISTS (
      SELECT 1
      FROM ${issueDocuments}
  ...
  `;
    return hasPlanDocument ? existsPlanDocument : sql `NOT ${existsPlanDocument}`;
}
...
async function listSuccessfulRunHandoffMapForIssues(dbOrTx, companyId, issueIds) {
    const uniqueIssueIds = [...new Set(issueIds)];
    ...
    for (const issueIdChunk of chunkList(uniqueIssueIds, ISSUE_LIST_RELATED_QUERY_CHUNK_SIZE)) {
        const rows = await dbOrTx
            ...
            .from(activityLog)
            .where(and(eq(activityLog.companyId, companyId), eq(activityLog.entityType, "issue"), inArray(activityLog.entityId, issueIdChunk), inArray(activityLog.action, [...BLOCKED_INBOX_SUCCESSFUL_RUN_HANDOFF_ACTIONS])))
...
async function listIssueBlockedInboxAttentionMap(dbOrTx, companyId, issueRows) {
    ...
    const [graphIssueRows, graphRelationRows, companyAgentRows] = await Promise.all([
        dbOrTx
            .select()
            .from(issues)
            .where(and(eq(issues.companyId, companyId), isNull(issues.hiddenAt), notInArray(issues.status, [...BLOCKED_INBOX_TERMINAL_STATUSES]))),
        dbOrTx
            .select({
    ...
    const companyAgents = companyAgentRows;
    ...
    const [activeRunRows, wakeRows, scheduledRetryRows, interactionRows, approvalRows, handoffMap] = await Promise.all([
        graphIssueIds.length === 0
            ...
            : dbOrTx
                ...
                .from(heartbeatRuns)
                .where(and(eq(heartbeatRuns.companyId, companyId), inArray(heartbeatRuns.status, [...BLOCKED_INBOX_ACTIVE_RUN_STATUSES]), inArray(sql `${heartbeatRuns.contextSnapshot} ->> 'issueId'`, graphIssueIds))),
        graphIssueIds.length === 0
            ? Promise.resolve([])
            : dbOrTx
                .select({
                ...
                .from(agentWakeupRequests)
                .where(and(eq(agentWakeupRequests.companyId, companyId), inArray(agentWakeupRequests.status, [...BLOCKED_INBOX_ACTIVE_WAKE_STATUSES]), sql `${agentWakeupRequests.runId} is null`, inArray(sql `${agentWakeupRequests.payload} ->> 'issueId'`, graphIssueIds))),
        graphIssueIds.length === 0
            ? Promise.resolve([])
            : dbOrTx
                .select({
                ...
                .from(heartbeatRuns)
                .where(and(eq(heartbeatRuns.companyId, companyId), eq(heartbeatRuns.status, "scheduled_retry"), inArray(sql `${heartbeatRuns.contextSnapshot} ->> 'issueId'`, graphIssueIds))),
        graphIssueIds.length === 0
            ? Promise.resolve([])
            : dbOrTx
                .select({
                ...
                .from(issueThreadInteractions)
                .where(and(eq(issueThreadInteractions.companyId, companyId), inArray(issueThreadInteractions.status, [...BLOCKED_INBOX_PENDING_INTERACTION_STATUSES]), inArray(issueThreadInteractions.issueId, graphIssueIds))),
        graphIssueIds.length === 0
            ? Promise.resolve([])
            : dbOrTx
                .select({
                ...
                .from(issueApprovals)
                .innerJoin(approvals, eq(issueApprovals.approvalId, approvals.id))
                .where(and(eq(issueApprovals.companyId, companyId), eq(approvals.companyId, companyId), inArray(approvals.status, [...BLOCKED_INBOX_PENDING_APPROVAL_STATUSES]), inArray(issueApprovals.issueId, graphIssueIds))),
    ...
    for (const row of issueRows) {
        if (row.companyId !== companyId || BLOCKED_INBOX_TERMINAL_STATUSES.includes(row.status) || row.hiddenAt) {
            continue;
        }
...
function parseIssueAssigneeAgentFilter(assigneeAgentId) {
...
function assertValidAssigneeAgentFilter(assigneeAgentFilter) {
...
async function blockedInboxIssueConditions(dbOrTx, companyId, filters) {
    const conditions = [
    ...
    if (lowTrustCondition)
        conditions.push(lowTrustCondition);
...
async function listBlockedInboxIssues(dbOrTx, companyId, filters) {
    ...
    if (rawSearchInput) {
        ...
        for (const issueIdChunk of chunkList(issueIds, ISSUE_LIST_RELATED_QUERY_CHUNK_SIZE)) {
            const rows = await dbOrTx
                ...
                .from(issueComments)
                .where(and(eq(issueComments.companyId, companyId), inArray(issueComments.issueId, issueIdChunk), isNull(issueComments.deletedAt), sql `${issueComments.body} ILIKE ${containsPattern} ESCAPE '\\'`));
            for (const row of rows)
...
async function countBlockedInboxIssues(dbOrTx, companyId, filters) {
...
export function issueService(db) {
    ...
    async function isTerminalOrMissingHeartbeatRun(runId, dbOrTx = db) {
    ...
    async function adoptStaleCheckoutRun(input) {
        return db.transaction(async (tx) => {
            ...
            await Promise.all([
                tx.execute(sql `select ${heartbeatRuns.id} from ${heartbeatRuns} where ${heartbeatRuns.id} = ${input.expectedCheckoutRunId} for update`),
                tx.execute(sql `select ${heartbeatRuns.id} from ${heartbeatRuns} where ${heartbeatRuns.id} = ${input.actorRunId} for update`),
            ]);
    ...
    async function adoptUnownedCheckoutRun(input) {
        return db.transaction(async (tx) => {
            await tx.execute(sql `select ${heartbeatRuns.id} from ${heartbeatRuns} where ${heartbeatRuns.id} = ${input.actorRunId} for update`);
    ...
    async function clearCheckoutRunIfTerminal(issueId) {
        return db.transaction(async (tx) => {
            await tx.execute(sql `select ${issues.id} from ${issues} where ${issues.id} = ${issueId} for update`);
            const issue = await tx
            ...
            if (!issue?.checkoutRunId)
                return false;
            await tx.execute(sql `select ${heartbeatRuns.id} from ${heartbeatRuns} where ${heartbeatRuns.id} = ${issue.checkoutRunId} for update`);
            const run = await tx
                .select({ status: heartbeatRuns.status })
                .from(heartbeatRuns)
            ...
            const updated = await tx
                ...
                .where(and(eq(issues.id, issueId), eq(issues.checkoutRunId, issue.checkoutRunId), issue.executionRunId
    ...
    return {
      ...
      `;
        ...
        listWakeableBlockedDependents: async (blockerIssueId) => {
            ...
            const readinessMap = await listIssueDependencyReadinessMap(db, blockerIssue.companyId, wakeableCandidates.map((candidate) => candidate.id));
        ...
        getWakeableParentAfterChildCompletion: async (parentIssueId) => {
            ...
            const commentRows = childIdsForSummaries.length > 0
                ? await db
                    ...
                    .from(issueComments)
                    .where(and(eq(issueComments.companyId, parent.companyId), inArray(issueComments.issueId, childIdsForSummaries), isNull(issueComments.deletedAt)))
        ...
        decomposeAcceptedPlan: async (sourceIssueId, data) => {
            ...
            const initialClaim = await db.transaction(async (tx) => {
                ...
                const belongsToPlanDocument = await tx
                    ...
                    .where(and(eq(issueDocuments.companyId, sourceIssue.companyId), eq(issueDocuments.issueId, sourceIssue.id), eq(issueDocuments.key, "plan"), eq(documentRevisions.id, data.acceptedPlanRevisionId)))
                    .then((rows) => rows[0] ?? null);
                ...
                const existing = await tx
                    ...
                    .where(and(eq(issuePlanDecompositions.companyId, sourceIssue.companyId), eq(issuePlanDecompositions.sourceIssueId, sourceIssue.id), eq(issuePlanDecompositions.acceptedPlanRevisionId, data.acceptedPlanRevisionId)))
                    .then((rows) => rows[0] ?? null);
                const now = new Date();
                if (!existing) {
        ...
        listAcceptedPlanDecompositions: async (sourceIssueId) => {
            ...
            const rows = await db
                ...
                .where(and(eq(issuePlanDecompositions.companyId, sourceIssue.companyId), eq(issuePlanDecompositions.sourceIssueId, sourceIssue.id)))
        ...
        create: async (companyId, data) => {
            const { labelIds: inputLabelIds, blockedByIssueIds, inheritExecutionWorkspaceFromIssueId, watchdog, watchdogActorRunId, ...issueData } = data;
        ...
        update: async (id, data, dbOrTx = db) => {
            ...
            const shouldValidateNextAssignee = Boolean(nextAssigneeAgentId) &&
                (issueData.assigneeAgentId !== undefined || patch.status === "in_progress");
            if (shouldValidateNextAssignee) {
                await assertAssignableAgent(dbOrTx, existing.companyId, nextAssigneeAgentId, { kind: "work" });
            }
            ...
            let validatedProjectWorkspace = null;
            let validatedExecutionWorkspace = null;
            ...
            const runUpdate = async (tx) => {
                ...
                if ((issueData.status === "done" || issueData.status === "cancelled") &&
                    ...
                    if (parsedIncident?.issueId && parsedIncident.companyId === existing.companyId) {
                        await tx
                            .delete(issueRelations)
                            .where(and(eq(issueRelations.companyId, existing.companyId), eq(issueRelations.issueId, existing.id), eq(issueRelations.relatedIssueId, parsedIncident.issueId), eq(issueRelations.type, "blocks")));
                    }
                }
                return enriched;
            };
            ...
            if (checkoutRunId &&
                ...
                if (stale) {
                    ...
                    const adopted = await db
                        .update(issues)
                        .set(adoptionSet)
                        .where(and(eq(issues.id, id), inArray(issues.status, expectedStatuses), eq(issues.executionRunId, current.executionRunId), or(isNull(issues.assigneeAgentId), eq(issues.assigneeAgentId, agentId))))
                        .returning()
                        .then((rows) => rows[0] ?? null);
                    if (adopted) {
        ...
        assertCheckoutOwner: async (id, actorAgentId, actorRunId) => {
            ...
            const resolveSameRunOwnership = (candidate) => {
                if (candidate.status === "in_progress" &&
                    ...
                    return { ...candidate, adoptedFromRunId: null };
                }
                return null;
            };
            const canAdoptUnownedCheckout = (candidate) => (actorRunId
            ...
            const resolveOwnership = async (candidate) => {
                ...
                if (actorRunId &&
                    ...
                    if (staleAdoption.latest) {
                        ...
                        if (latestOwnership)
                            return { ownership: latestOwnership, latest: staleAdoption.latest };
            ...
            if (resolvedLatest.ownership)
                return resolvedLatest.ownership;
        ...
        release: async (id, actorAgentId, actorRunId) => db.transaction(async (tx) => {
        ...
        tombstoneComment: async (commentId, actor, options) => {
            const currentUserRedactionOptions = {
                enabled: (await instanceSettings.getGeneral()).censorUsernameInLogs,
            };
            return db.transaction(async (tx) => {
        ...
        addComment: async (issueId, body, actor, options, dbOrTx = db) => {
            const issue = await dbOrTx
                .select({ companyId: issues.companyId })
                .from(issues)
                .where(eq(issues.id, issueId))
        ...
        findMentionedAgents: async (companyId, body) => {
            ...
            if (explicitAgentMentionIds.length === 0)
                return [];
