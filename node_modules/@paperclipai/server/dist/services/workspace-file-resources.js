...
function normalizeWorkspaceRelativePath(input) {
    ...
    if (trimmed.includes("\0"))
        throw unprocessable("Workspace file path contains an invalid character", { code: "invalid_path" });
    if (/^file:\/\//i.test(trimmed))
        throw unprocessable("File URLs are not supported", { code: "invalid_path" });
    if (/^[a-zA-Z]:/.test(trimmed))
        throw unprocessable("Windows drive paths are not supported", { code: "invalid_path" });
    if (trimmed.startsWith("~"))
        throw unprocessable("Home-relative paths are not supported", { code: "invalid_path" });
    if (trimmed.includes("\\"))
        throw unprocessable("Workspace file paths must use forward slashes", { code: "invalid_path" });
    if (path.posix.isAbsolute(trimmed))
        throw unprocessable("Workspace file path must be relative", { code: "invalid_path" });
...
function denyReasonForPathSegments(segments) {
    const lowerSegments = segments.map((segment) => segment.toLowerCase());
    if (lowerSegments.some((segment) => DENIED_SEGMENTS.has(segment)))
        return "denied_path_segment";
    ...
    if (["id_rsa", "id_ed25519", ".npmrc", ".pypirc", ".netrc", "kubeconfig"].includes(fileName))
        return "denied_secret";
    if (lowerSegments.includes(".aws") || lowerSegments.includes(".ssh"))
        return "denied_secret";
    if (lowerSegments.length >= 2 && lowerSegments.at(-2) === ".docker" && fileName === "config.json")
        return "denied_secret";
    if (lowerSegments.length >= 2 && lowerSegments.at(-2) === ".kube" && fileName === "config")
        return "denied_secret";
    return null;
}
function throwIfDenied(segments) {
...
function shouldPruneSegments(segments) {
    return denyReasonForPathSegments(segments) != null;
}
function contentTypeForPath(filePath) {
    const ext = path.extname(filePath).toLowerCase();
    if (IMAGE_CONTENT_TYPES.has(ext))
        return IMAGE_CONTENT_TYPES.get(ext) ?? null;
    if (VIDEO_CONTENT_TYPES.has(ext))
        return VIDEO_CONTENT_TYPES.get(ext) ?? null;
    if (ext === ".pdf")
...
function previewKindForKnownContentType(contentType) {
    if (!contentType)
        return null;
...
async function sniffUnknownPreviewKind(realPath) {
...
function remoteResource(candidate, relativePath) {
    return {
...
function candidateFromExecutionWorkspace(row) {
...
function candidateFromProjectWorkspace(row, project) {
...
async function statLocalCandidate(candidate, normalized) {
...
async function statLocalDirectory(candidate, normalized) {
    if (!candidate.rootPath) {
        throw unprocessable("Workspace is not locally readable", { code: "remote_workspace" });
    }
    throwIfDenied(normalized.segments);
...
async function readStableFile(realPath, maxBytes) {
    const handle = await fs.open(realPath, "r");
    try {
        ...
        if (before.size > maxBytes)
            throw unprocessable("Workspace file is too large to preview", { code: "too_large" });
...
function matchesSearch(relativePath, normalizedQuery) {
...
async function listLocalFileCandidate(input) {
    ...
    if (shouldPruneSegments(normalized.segments))
        return null;
    if (!matchesSearch(normalized.relativePath, input.normalizedQuery))
        return null;
    ...
    try {
        ...
        if (stat.isSymbolicLink() || !stat.isFile())
            return null;
        targetReal = await fs.realpath(targetLexical);
        if (!isInsideRoot(input.rootReal, targetReal))
            return null;
        const realRelative = relativePathFromReal(input.rootReal, targetReal);
        if (shouldPruneSegments(realRelative.split("/").filter(Boolean)))
            return null;
    }
    catch {
        return null;
...
async function enumerateWorkspaceFiles(input) {
    ...
    while (dirs.length > 0) {
        ...
        for (const entry of entries) {
            ...
            if (shouldPruneSegments(segments))
                continue;
        ...
        if (hitScanCap || (truncated && input.mode !== "recent"))
            break;
    }
    if (input.mode === "recent") {
...
async function listChangedWorkspaceFiles(input) {
    let stdout;
    try {
        const result = await execFileAsync("git", ["-C", input.rootReal, "status", "--porcelain=v1", "-z", "--untracked-files=all"], { maxBuffer: GIT_STATUS_MAX_BUFFER_BYTES });
        stdout = result.stdout;
    }
    catch {
        return { unavailableReason: "changed_unavailable" };
    }
...
export function workspaceFileResourceService(db) {
    ...
    async function targetProjectWorkspaceCandidate(issue, target) {
    ...
    async function listCandidates(issue, selector, target = {}) {
        ...
        if ((selector === "auto" || selector === "execution") && issue.projectId) {
            const executionIds = [issue.executionWorkspaceId].filter((id) => Boolean(id));
            let executionRows = [];
            if (executionIds.length > 0) {
                executionRows = await db.select().from(executionWorkspaces).where(and(eq(executionWorkspaces.companyId, issue.companyId), inArray(executionWorkspaces.id, executionIds)));
            }
            const sourceIssueIds = [issue.id, issue.parentId].filter((id) => Boolean(id));
            if (sourceIssueIds.length > 0) {
                const activeRows = await db.select().from(executionWorkspaces).where(and(eq(executionWorkspaces.companyId, issue.companyId), eq(executionWorkspaces.projectId, issue.projectId), inArray(executionWorkspaces.sourceIssueId, sourceIssueIds), eq(executionWorkspaces.status, "active"), isNull(executionWorkspaces.closedAt))).orderBy(desc(executionWorkspaces.lastUsedAt)).limit(2);
        ...
        if ((selector === "auto" || selector === "project") && issue.projectId) {
            if (issue.projectWorkspaceId) {
                const rows = await db.select().from(projectWorkspaces).where(and(eq(projectWorkspaces.companyId, issue.companyId), eq(projectWorkspaces.projectId, issue.projectId), eq(projectWorkspaces.id, issue.projectWorkspaceId))).limit(1);
                if (rows[0])
                    candidates.push(candidateFromProjectWorkspace(rows[0]));
            }
            const primaryRows = await db.select().from(projectWorkspaces).where(and(eq(projectWorkspaces.companyId, issue.companyId), eq(projectWorkspaces.projectId, issue.projectId), eq(projectWorkspaces.isPrimary, true))).limit(1);
    ...
    async function sameCompanyProjectWorkspaceCandidates(issue, excludedWorkspaceIds) {
        const rows = await db.select({
            ...
            .where(and(eq(projectWorkspaces.companyId, issue.companyId), eq(projects.companyId, issue.companyId)))
            .orderBy(desc(projectWorkspaces.isPrimary), desc(projectWorkspaces.updatedAt));
        const candidates = [];
        const seen = new Set(excludedWorkspaceIds);
        for (const row of rows) {
            if (seen.has(row.workspace.id))
                continue;
    ...
    async function discoverUniqueProjectWorkspaceMatch(issue, excludedWorkspaceIds, check) {
        ...
        if (matches.length === 1)
            return { state: "one", value: matches[0] };
        return { state: "none" };
    }
    function throwAmbiguousWorkspacePath(count) {
        throw new HttpError(409, "Workspace path matched multiple project workspaces", {
            code: "ambiguous_workspace_path",
            matchCount: count,
        });
    }
    async function resolve(issueId, input, opts = {}) {
        ...
        for (const candidate of candidates) {
            if (candidate.remote) {
                if (explicitTarget || selector !== "auto")
                    return remoteResource(candidate, normalized.relativePath);
                continue;
            }
            try {
        ...
        if (lastNotFound && !explicitTarget && selector === "auto") {
            const discovered = await discoverUniqueProjectWorkspaceMatch(issue, new Set(candidates.map((candidate) => candidate.workspaceId)), async (candidate) => isDirectoryRequest
                ? (await statLocalDirectory(candidate, normalized)).resource
                : (await statLocalCandidate(candidate, normalized)).resource);
            if (discovered.state === "one")
                return discovered.value;
            if (discovered.state === "ambiguous")
                throwAmbiguousWorkspacePath(discovered.count);
        }
        if (lastNotFound)
            throw lastNotFound;
        throw unprocessable("No local-readable workspace is available for this issue", { code: "no_local_workspace" });
    }
    async function list(issueId, input = {}, opts = {}) {
        const issue = opts.issue ?? await getIssue(issueId);
        const selector = input.workspace ?? "auto";
        const explicitTarget = Boolean(input.projectId || input.workspaceId);
        ...
        const limit = Math.min(WORKSPACE_FILE_LIST_MAX_LIMIT, Math.max(1, Math.floor(input.limit ?? WORKSPACE_FILE_LIST_DEFAULT_LIMIT)));
        ...
        if (lastNotFound && normalizedPath && !explicitTarget && selector === "auto") {
            const discovered = await discoverUniqueProjectWorkspaceMatch(issue, new Set(candidates.map((candidate) => candidate.workspaceId)), async (candidate) => {
                const resolvedDirectory = await statLocalDirectory(candidate, normalizedPath);
                return {
                    candidate,
                    rootReal: resolvedDirectory.rootReal,
    ...
    async function readContent(issueId, input, opts = {}) {
        const issue = opts.issue ?? await getIssue(issueId);
        const selector = input.workspace ?? "auto";
        const explicitTarget = Boolean(input.projectId || input.workspaceId);
        const normalized = normalizeWorkspaceRelativePath(input.path);
        ...
        if (lastNotFound && !explicitTarget && selector === "auto") {
            const discovered = await discoverUniqueProjectWorkspaceMatch(issue, new Set(candidates.map((candidate) => candidate.workspaceId)), async (candidate) => statLocalCandidate(candidate, normalized));
            if (discovered.state === "ambiguous")
                throwAmbiguousWorkspacePath(discovered.count);
            if (discovered.state === "one") {
    ...
    async function prepareDownload(issueId, input, opts = {}) {
        const issue = opts.issue ?? await getIssue(issueId);
        const selector = input.workspace ?? "auto";
        const explicitTarget = Boolean(input.projectId || input.workspaceId);
        const normalized = normalizeWorkspaceRelativePath(input.path);
...
//# sourceMappingURL=workspace-file-resources.js.map