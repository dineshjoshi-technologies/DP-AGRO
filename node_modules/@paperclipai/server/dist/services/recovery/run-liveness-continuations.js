...
export function readContinuationAttempt(value) {
...
export async function findExistingRunLivenessContinuationWake(db, input) {
    return db
        .select({ id: agentWakeupRequests.id, status: agentWakeupRequests.status })
        .from(agentWakeupRequests)
        .where(and(eq(agentWakeupRequests.companyId, input.companyId), eq(agentWakeupRequests.idempotencyKey, input.idempotencyKey), inArray(agentWakeupRequests.status, IDEMPOTENT_WAKE_STATUSES)))
        .limit(1)
        .then((rows) => rows[0] ?? null);
}
export function decideRunLivenessContinuation(input) {
    const { run, issue, agent, livenessState, livenessReason, nextAction, budgetBlocked, idempotentWakeExists, } = input;
    ...
    if (!agent)
        return { kind: "skip", reason: "agent not found" };
...
//# sourceMappingURL=run-liveness-continuations.js.map