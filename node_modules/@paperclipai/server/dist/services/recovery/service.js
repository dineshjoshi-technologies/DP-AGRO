...
import { agents, agentWakeupRequests, approvals, activityLog, companies, heartbeatRunEvents, heartbeatRunWatchdogDecisions, heartbeatRuns, issueAttachments, issueComments, issueApprovals, issueRecoveryActions, issueRelations, issueThreadInteractions, issues, } from "@paperclipai/db";
...
export const STRANDED_RECENT_PROGRESS_EXEMPTION_MS = Math.max(60_000, Number(process.env.STRANDED_RECENT_PROGRESS_EXEMPTION_MS) || 30 * 60 * 1000);
...
export function classifyContinuationFailure(latestRun) {
...
export function recoveryService(db, deps) {
    ...
    async function hasPendingWakeInteraction(companyId, issueId) {
        return db
            ...
            .where(and(eq(issueThreadInteractions.companyId, companyId), eq(issueThreadInteractions.issueId, issueId), eq(issueThreadInteractions.status, "pending"), inArray(issueThreadInteractions.continuationPolicy, ["wake_assignee", "wake_assignee_on_accept"])))
            .limit(1)
    ...
    async function hasRecentVisibleProgress(companyId, issueId, assigneeAgentId, windowMs) {
        const since = new Date(Date.now() - windowMs);
        const [comment, attachment] = await Promise.all([
            db
                .select({ id: issueComments.id })
                .from(issueComments)
                .where(and(eq(issueComments.companyId, companyId), eq(issueComments.issueId, issueId), eq(issueComments.authorAgentId, assigneeAgentId), gt(issueComments.createdAt, since)))
                .limit(1)
                .then((rows) => rows[0] ?? null),
            db
                .select({ id: issueAttachments.id })
                .from(issueAttachments)
                .where(and(eq(issueAttachments.companyId, companyId), eq(issueAttachments.issueId, issueId), gt(issueAttachments.createdAt, since)))
                .limit(1)
                .then((rows) => rows[0] ?? null),
        ]);
    ...
    async function findClosedStaleRunEvaluation(companyId, runId) {
        const [row] = await db
            .select({ id: issues.id, identifier: issues.identifier, status: issues.status })
            .from(issues)
            .where(and(eq(issues.companyId, companyId), eq(issues.originKind, STALE_ACTIVE_RUN_EVALUATION_ORIGIN_KIND), eq(issues.originId, runId), isNull(issues.hiddenAt), eq(issues.status, "done")))
    ...
    async function hasDismissedFalsePositiveDecision(companyId, runId) {
        const [row] = await db
            .select({ id: heartbeatRunWatchdogDecisions.id })
            .from(heartbeatRunWatchdogDecisions)
            .where(and(eq(heartbeatRunWatchdogDecisions.companyId, companyId), eq(heartbeatRunWatchdogDecisions.runId, runId), eq(heartbeatRunWatchdogDecisions.decision, "dismissed_false_positive")))
            .limit(1);
        return row != null;
    }
    ...
    async function foldSourceResolvedStaleRun(input) {
        ...
        const finalizedRun = await db.transaction(async (tx) => {
            ...
            await tx
                ...
                .where(and(eq(issues.id, input.sourceIssue.id), eq(issues.companyId, input.run.companyId), eq(issues.executionRunId, input.run.id)));
            return updatedRun;
        });
        if (!finalizedRun)
            return { kind: "skipped" };
    ...
    async function ensureSourceIssueCommentedForStaleEvaluation(input) {
        ...
        const [priorEscalation] = await db
            ...
            .from(activityLog)
            .where(and(eq(activityLog.companyId, input.sourceIssue.companyId), eq(activityLog.action, "heartbeat.output_stale_escalated"), eq(activityLog.entityType, "issue"), eq(activityLog.entityId, input.sourceIssue.id), sql `${activityLog.details} ->> 'evaluationIssueId' = ${input.evaluationIssue.id}`))
            .limit(1);
        if (priorEscalation)
            return false;
    ...
    async function createOrUpdateStaleRunEvaluation(input) {
        ...
        if (closedEvaluation) {
            const autoDismissed = await db.transaction(async (tx) => {
                await tx.execute(sql `SELECT pg_advisory_xact_lock(hashtextextended(${`watchdog_dismiss:${input.run.companyId}:${input.run.id}`}, 0))`);
                const hasAnyDecision = await tx
                    .select({ id: heartbeatRunWatchdogDecisions.id })
                    .from(heartbeatRunWatchdogDecisions)
                    .where(and(eq(heartbeatRunWatchdogDecisions.companyId, input.run.companyId), eq(heartbeatRunWatchdogDecisions.runId, input.run.id)))
                    .limit(1)
    ...
    async function existingUnresolvedBlockerIssues(companyId, issueId) {
        return db
    ...
    async function resolveContinuationWaitingOnReview(issue) {
        const existingBlockers = await existingUnresolvedBlockerIssues(issue.companyId, issue.id);
        const openChildren = await db
            .select({ id: issues.id, identifier: issues.identifier })
            .from(issues)
            .where(and(eq(issues.companyId, issue.companyId), eq(issues.parentId, issue.id), isNull(issues.hiddenAt), notInArray(issues.status, ["done", "cancelled"])));
        ...
        await issuesSvc.addComment(issue.id, `This task is waiting on ${waitingOn} to finish. ` +
            ...
            "Paperclip turned that into a normal dependency wait instead of flagging it as stuck.)", {}, { authorType: "system" });
        await logActivity(db, {
    ...
    async function escalateStrandedAssignedIssue(input) {
        ...
        const shouldPostEscalationComment = recoveryAction.attemptCount === 1 ||
            ...
            if (issue.status === "todo") {
                ...
                if (didAutomaticRecoveryFail(latestRun, "assignment_recovery")) {
                    ...
                    const updated = await escalateStrandedAssignedIssue({
                        ...
                        previousStatus: "todo",
                        latestRun,
                        comment: "Paperclip automatically retried dispatch for this assigned `todo` issue after a lost wake/run, " +
    ...
    async function collectIssueGraphLivenessFindings() {
        ...
        const openRecoveryIssues = recoveryIssueRows.flatMap((row) => {
            if (row.originKind === RECOVERY_ORIGIN_KINDS.issueGraphLivenessEscalation) {
                ...
                if (!parsed || parsed.companyId !== row.companyId)
                    return [];
                return [
                    {
    ...
    async function sweepStaleIssueLocks() {
        ...
        const isCleanable = (runId) => {
            if (!runId)
                return true;
            const status = runStatusById.get(runId);
            if (!status)
                return true; // missing run row → no real claim
        ...
        for (const issue of candidates) {
            ...
            const updated = await db
                ...
                .where(and(eq(issues.id, issue.id), issue.checkoutRunId
                ? eq(issues.checkoutRunId, issue.checkoutRunId)
                : isNull(issues.checkoutRunId), issue.executionRunId
        ...
        if (result.cleared > 0) {
            logger.warn({ cleared: result.cleared, issueIds: result.issueIds }, "swept stale issue lock columns");
        }
        return result;
    }
    return {
