...
function readTaskWatchdogContext(contextSnapshot) {
    ...
    if (!taskWatchdog && context?.taskWatchdog !== true)
        return null;
    return {
...
export async function resolveTaskWatchdogMutationScope(db, actor) {
    if (actor.type !== "agent")
        return { kind: "none" };
    ...
    const watchdog = await db
        ...
        .from(issueWatchdogs)
        .where(and(eq(issueWatchdogs.companyId, run.companyId), eq(issueWatchdogs.issueId, taskWatchdog.watchedIssueId), eq(issueWatchdogs.watchdogAgentId, agentId), eq(issueWatchdogs.status, "active")))
        .then((rows) => rows[0] ?? null);
    if (!watchdog) {
        return {
            kind: "invalid",
...
export async function issueIsInTaskWatchdogSubtree(db, companyId, issueId, watchedIssueId) {
    ...
    for (let depth = 0; currentId && depth < MAX_WATCHDOG_SCOPE_ANCESTRY_DEPTH; depth += 1) {
        ...
        if (parent.originKind === TASK_WATCHDOG_ORIGIN_KIND)
            return false;
        if (currentId === watchedIssueId)
            return true;
        currentId = parent.parentId ?? null;
    }
    return false;
}
export async function taskWatchdogScopeAllowsIssueMutation(db, scope, issue, opts = {}) {
