import { activityLog, agentTaskSessions as agentTaskSessionsTable, agents as agentsTable, budgetIncidents, costEvents, heartbeatRuns, invites, issues as issuesTable, pluginLogs, principalPermissionGrants, projects as projectsTable, } from "@paperclipai/db";
...
export function buildHostServices(db, pluginId, pluginKey, eventBus, notifyWorker, options = {}) {
    ...
    const getStoredLocalFolderConfig = async (companyId, folderKey) => {
    ...
    const inspectStoredLocalFolder = async (companyId, folderKey) => inspectPluginLocalFolder({
        folderKey,
    ...
    const collectIssueSubtreeIds = async (companyId, rootIssueId) => {
    ...
    const getIssueRunSummaries = async (companyId, issueIds, options = {}) => {
    ...
    const getIssueCostSummary = async (companyId, issueIds, billingCode) => {
    ...
    const getOpenBudgetIncidents = async (companyId) => {
        const rows = await db
            .select({
    ...
    const loadPluginMember = async (companyId, memberId) => {
        const member = await access.getMemberById(companyId, memberId);
        if (!member)
            return null;
        const grants = await access.listPrincipalGrants(companyId, member.principalType, member.principalId);
        return {
            ...member,
        ...
        authorization: {
            ...
            async setGrants(params) {
                ...
                if (params.principalType === "agent") {
                    ...
                    const membership = await access.getMembership(companyId, params.principalType, params.principalId);
                    if (!membership)
                        throw new Error("Principal is not a member of this company");
                }
                await access.setPrincipalGrants(companyId, params.principalType, params.principalId, params.grants.map((grant) => ({
                    permissionKey: grant.permissionKey,
                ...
                return access
                    .listPrincipalGrants(companyId, params.principalType, params.principalId)
