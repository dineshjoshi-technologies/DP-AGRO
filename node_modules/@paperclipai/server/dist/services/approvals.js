...
export function approvalService(db) {
    ...
    function redactApprovalComment(comment, censorUsernameInLogs) {
        return {
            ...comment,
    ...
    return {
        ...
        getById: (id) => db
            ...
            .where(eq(approvals.id, id))
            .then((rows) => rows[0] ?? null),
        findOpenHireApprovalForAgent: async (companyId, agentId) => {
            const rows = await db
                .select()
                .from(approvals)
                .where(and(eq(approvals.companyId, companyId), eq(approvals.type, "hire_agent"), inArray(approvals.status, resolvableStatuses), sql `${approvals.payload} ->> 'agentId' = ${agentId}`));
            return rows[0] ?? null;
        },
