...
import { agentWakeupRequests, heartbeatRuns, issueComments, issueTreeHoldMembers, issueTreeHolds, issues, } from "@paperclipai/db";
import { ISSUE_STATUSES, } from "@paperclipai/shared";
import { conflict, notFound, unprocessable } from "../errors.js";
const TERMINAL_ISSUE_STATUSES = new Set(["done", "cancelled"]);
const ACTIVE_RUN_STATUSES = ["queued", "running"];
const DEFAULT_RELEASE_POLICY = { strategy: "manual" };
const MAX_PAUSE_HOLD_ANCESTOR_DEPTH = 100;
export const ISSUE_TREE_CONTROL_INTERACTION_WAKE_REASONS = new Set([
    "issue_commented",
    "issue_reopened_via_comment",
    "issue_comment_mentioned",
]);
const ISSUE_TREE_CONTROL_INTERACTION_WAKE_SOURCES = {
...
function readNonEmptyStringFromRecord(record, key) {
    if (!record || typeof record !== "object")
        return null;
    const value = record[key];
    return typeof value === "string" && value.trim().length > 0 ? value.trim() : null;
}
function readInteractionWakeCommentId(record) {
    if (!record || typeof record !== "object")
        return null;
    const value = record.wakeCommentIds;
...
function hasVerifiedInteractionSource(wakeReason, contextSnapshot) {
...
function actorMatchesComment(actor, comment) {
    if (!actor.requestedByActorType)
        return false;
    if (actor.requestedByActorType === "system")
        return true;
    if (!actor.requestedByActorId)
        return false;
    if (actor.requestedByActorType === "agent")
        return comment.authorAgentId === actor.requestedByActorId;
    if (actor.requestedByActorType === "user")
        return comment.authorUserId === actor.requestedByActorId;
    return false;
}
async function hasVerifiedInteractionWakeRequest(dbOrTx, input) {
    ...
    if (input.agentId)
        predicates.push(eq(agentWakeupRequests.agentId, input.agentId));
    if (input.runId && input.wakeupRequestId) {
        const requestScope = or(eq(agentWakeupRequests.runId, input.runId), eq(agentWakeupRequests.id, input.wakeupRequestId));
        if (requestScope)
            predicates.push(requestScope);
    }
    else if (input.runId) {
...
export async function isVerifiedIssueTreeControlInteractionWake(dbOrTx, input) {
    ...
    if (!wakeReason || !ISSUE_TREE_CONTROL_INTERACTION_WAKE_REASONS.has(wakeReason))
        return false;
    if (!contextSnapshot || !hasVerifiedInteractionSource(wakeReason, contextSnapshot))
        return false;
    ...
    const comment = await dbOrTx
        ...
        .from(issueComments)
        .where(and(eq(issueComments.companyId, input.companyId), eq(issueComments.issueId, input.issueId), eq(issueComments.id, commentId)))
    ...
    if (actorMatchesComment(directActor, comment))
        return true;
...
function normalizeReleasePolicy(releasePolicy) {
    return releasePolicy ?? DEFAULT_RELEASE_POLICY;
}
function coerceIssueStatus(status) {
    return ISSUE_STATUSES.includes(status) ? status : "backlog";
}
function isTerminalIssue(status) {
    return TERMINAL_ISSUE_STATUSES.has(coerceIssueStatus(status));
}
function toPreviewRun(row) {
    return {
        id: row.id,
        issueId: row.issueId,
        agentId: row.agentId,
...
function toHold(row, members) {
    return {
        ...
        status: row.status,
        reason: row.reason,
        releasePolicy: row.releasePolicy ?? null,
        createdByActorType: row.createdByActorType,
        createdByAgentId: row.createdByAgentId,
        createdByUserId: row.createdByUserId,
        createdByRunId: row.createdByRunId,
        releasedAt: row.releasedAt,
        releasedByActorType: row.releasedByActorType,
...
function issueSkipReason(input) {
    ...
    if (input.mode === "restore") {
        ...
        if (!input.activeCancelSnapshot?.member)
            return "not_cancelled_by_tree_control";
...
function buildAffectedAgents(issuesToPreview) {
...
function restoreStatusFromCancelSnapshot(status) {
...
export function issueTreeControlService(db) {
    async function listTreeIssues(companyId, rootIssueId) {
        const root = await db
            .select()
            .from(issues)
    ...
    async function activeRunsForTree(companyId, treeIssues) {
        ...
        const issueIdFromContext = sql `${heartbeatRuns.contextSnapshot} ->> 'issueId'`;
        ...
        const rows = await db
            ...
            .from(heartbeatRuns)
            .where(and(eq(heartbeatRuns.companyId, companyId), inArray(heartbeatRuns.status, [...ACTIVE_RUN_STATUSES]), uniqueRunIds.length > 0
            ? or(inArray(heartbeatRuns.id, uniqueRunIds), inArray(issueIdFromContext, issueIds))
            : inArray(issueIdFromContext, issueIds)));
        const issueIdByExecutionRunId = new Map(treeIssues
            .filter((issue) => issue.executionRunId)
            .map((issue) => [issue.executionRunId, issue.id]));
        return rows
            .map((run) => {
    ...
    async function activeHoldsByIssueId(companyId, issueIds) {
        ...
        const rows = await db
            ...
            .where(and(eq(issueTreeHoldMembers.companyId, companyId), eq(issueTreeHolds.status, "active"), inArray(issueTreeHoldMembers.issueId, issueIds)))
        ...
        for (const row of rows) {
            ...
            if (row.mode === "pause")
                current.pause.push(row.holdId);
            byIssueId.set(row.issueId, current);
        }
        return byIssueId;
    }
    async function activeCancelSnapshotsByIssueId(companyId, rootIssueId) {
        ...
        for (const hold of [...activeCancelHolds].reverse()) {
            for (const member of hold.members ?? []) {
                ...
                if (!current.holdIds.includes(hold.id))
                    current.holdIds.push(hold.id);
                if (!current.member && !member.skipped)
    ...
    async function activePauseHoldsForIssueIds(companyId, issueIds) {
        if (issueIds.length === 0)
            return [];
        return db
            .select()
            .from(issueTreeHolds)
            .where(and(eq(issueTreeHolds.companyId, companyId), eq(issueTreeHolds.status, "active"), eq(issueTreeHolds.mode, "pause"), inArray(issueTreeHolds.rootIssueId, issueIds)))
            .orderBy(asc(issueTreeHolds.createdAt), asc(issueTreeHolds.id));
    }
    async function getActivePauseHoldGate(companyId, issueId) {
        const activePauseHolds = await db
            ...
            .from(issueTreeHolds)
            .where(and(eq(issueTreeHolds.companyId, companyId), eq(issueTreeHolds.status, "active"), eq(issueTreeHolds.mode, "pause")))
            .orderBy(asc(issueTreeHolds.createdAt), asc(issueTreeHolds.id));
        ...
        while (currentIssueId
            ...
            if (hold) {
                return {
                    ...
                    mode: "pause",
                    reason: hold.reason,
                    releasePolicy: hold.releasePolicy ?? null,
                };
            }
    ...
    async function preview(companyId, rootIssueId, input) {
        ...
        for (const run of activeRunRows) {
            if (!runsByIssueId.has(run.issueId))
                runsByIssueId.set(run.issueId, run);
        }
    ...
    async function createHold(companyId, rootIssueId, input) {
        ...
        if (input.mode === "resume") {
            ...
            if (resumedPauseHoldIds.length > 0) {
                await Promise.all(activePauseHolds.map((pauseHold) => releaseHold(companyId, pauseHold.rootIssueId, pauseHold.id, {
    ...
    async function cancelIssueStatusesForHold(companyId, rootIssueId, holdId) {
        ...
        const updated = await db
            ...
            .where(and(eq(issues.companyId, companyId), inArray(issues.id, issueIds), notInArray(issues.status, ["done", "cancelled"])))
            .returning({
            id: issues.id,
    ...
    async function restoreIssueStatusesForHold(companyId, rootIssueId, restoreHoldId, input) {
        ...
        const cancelSnapshotByIssueId = new Map();
        for (const hold of [...activeCancelHolds].reverse()) {
            for (const member of hold.members ?? []) {
        ...
        const restoreIssueIds = [...new Set((restoreHold.members ?? [])
                .filter((member) => !member.skipped)
                .map((member) => member.issueId))];
        const restoreStatusByIssueId = new Map();
        for (const issueId of restoreIssueIds) {
            ...
            if (restoredStatus)
                restoreStatusByIssueId.set(issueId, restoredStatus);
        }
        const issueIdsByStatus = new Map();
        ...
        const updatedIssues = await db.transaction(async (tx) => {
            ...
            for (const [status, issueIdsForStatus] of issueIdsByStatus) {
                ...
                const rows = await tx
                    ...
                    .where(and(eq(issues.companyId, companyId), inArray(issues.id, issueIdsForStatus), eq(issues.status, "cancelled")))
                    .returning({
                    id: issues.id,
                    status: issues.status,
                    assigneeAgentId: issues.assigneeAgentId,
    ...
    async function getHold(companyId, holdId) {
        const hold = await db
            .select()
            .from(issueTreeHolds)
    ...
    async function listHolds(companyId, rootIssueId, input) {
        const whereClauses = [
            eq(issueTreeHolds.companyId, companyId),
            eq(issueTreeHolds.rootIssueId, rootIssueId),
        ];
        if (input?.status)
            whereClauses.push(eq(issueTreeHolds.status, input.status));
        if (input?.mode)
            whereClauses.push(eq(issueTreeHolds.mode, input.mode));
        ...
        const members = await db
            .select()
            .from(issueTreeHoldMembers)
            .where(and(eq(issueTreeHoldMembers.companyId, companyId), inArray(issueTreeHoldMembers.holdId, holdIds)))
            .orderBy(asc(issueTreeHoldMembers.depth), asc(issueTreeHoldMembers.createdAt), asc(issueTreeHoldMembers.issueId));
        const membersByHoldId = new Map();
    ...
    async function releaseHold(companyId, rootIssueId, holdId, input) {
        ...
        const [updated] = await db
            ...
            releasePolicy: input.releasePolicy
                ? normalizeReleasePolicy(input.releasePolicy)
    ...
    async function cancelUnclaimedWakeupsForTree(companyId, rootIssueId, reason) {
        const treeIssues = await listTreeIssues(companyId, rootIssueId);
        const issueIds = treeIssues.map((issue) => issue.id);
        if (issueIds.length === 0)
            return [];
        ...
        return db
            ...
            .where(and(eq(agentWakeupRequests.companyId, companyId), inArray(agentWakeupRequests.status, ["queued", "deferred_issue_execution"]), isNull(agentWakeupRequests.runId), inArray(sql `${agentWakeupRequests.payload} ->> 'issueId'`, issueIds)))
            .returning({
...
//# sourceMappingURL=issue-tree-control.js.map