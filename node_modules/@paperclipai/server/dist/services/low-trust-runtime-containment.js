...
import { LOW_TRUST_ISSUE_ANCESTRY_MAX_DEPTH, isIssueWithinLowTrustBoundary, } from "./trust-preset-resolver.js";
export const LOW_TRUST_RUNTIME_MANAGEMENT_TOOL_CLASS = "runtime.manage";
export function isLowTrustRuntimeManagementAllowed(resolution) {
...
async function issueIdIsDescendantOf(db, issueId, rootIssueId, companyId) {
    let cursor = issueId;
...
async function workspaceIssueWithinLowTrustBoundary(input) {
    if (isIssueWithinLowTrustBoundary(input.boundary, input.issue))
        return true;
    if (!input.db || !input.issue.id || !input.boundary.rootIssueId)
        return false;
...
export async function assertLowTrustWorkspaceIsolation(input) {
    ...
    if (input.resolution.kind !== "low_trust_review")
        return;
...
export function assertLowTrustRuntimeServicesAllowed(input) {
    ...
    if (input.resolution.kind !== "low_trust_review")
        return;
    if (input.runtimeServiceCount === 0)
        return;
    if (isLowTrustRuntimeManagementAllowed(input.resolution))
        return;
...
//# sourceMappingURL=low-trust-runtime-containment.js.map