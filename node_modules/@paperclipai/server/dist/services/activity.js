...
import { activityLog, agents, documentRevisions, environmentLeases, environments, heartbeatRunEvents, heartbeatRuns, issueComments, issueDocuments, issues, issueWorkProducts, workspaceOperations, } from "@paperclipai/db";
...
export function activityService(db) {
    const scheduledLivenessBackfills = new Set();
    const issueIdAsText = sql `${issues.id}::text`;
    const summarizedUsageJson = sql `
    case
  ...
  `.as("usageJson");
    const summarizedResultJson = sql `
    case
  ...
  `.as("resultJson");
    ...
    async function backfillMissingRunLivenessForIssue(companyId, issueId) {
        const runs = await db
            .select({
            id: heartbeatRuns.id,
            ...
            .from(heartbeatRuns)
            .where(and(eq(heartbeatRuns.companyId, companyId), isNull(heartbeatRuns.livenessState), sql `${heartbeatRuns.status} not in ('queued', 'running')`, or(sql `${heartbeatRuns.contextSnapshot} ->> 'issueId' = ${issueId}`, sql `exists (
              select 1
              from ${activityLog}
        ...
        for (const run of runs) {
            ...
            const continuationAttempt = readNumber(context?.continuationAttempt) ??
            ...
            const [commentStats] = await db
                ...
                .from(issueComments)
                .where(and(eq(issueComments.companyId, companyId), eq(issueComments.issueId, issueId), eq(issueComments.createdByRunId, run.id)));
            const [documentStats] = await db
                ...
                .where(and(eq(documentRevisions.companyId, companyId), eq(documentRevisions.createdByRunId, run.id), eq(issueDocuments.companyId, companyId), eq(issueDocuments.issueId, issueId), sql `${issueDocuments.key} != ${ISSUE_CONTINUATION_SUMMARY_DOCUMENT_KEY}`));
            const [workProductStats] = await db
                ...
                .where(and(eq(issueWorkProducts.companyId, companyId), eq(issueWorkProducts.issueId, issueId), eq(issueWorkProducts.createdByRunId, run.id)));
    ...
    function scheduleRunLivenessBackfill(companyId, issueId) {
        const key = `${companyId}:${issueId}`;
        if (scheduledLivenessBackfills.has(key))
            return;
    ...
    return {
        ...
        forIssue: (issueId) => db
            ...
            .orderBy(desc(activityLog.createdAt)),
        runsForIssue: async (companyId, issueId) => {
            ...
            const exhaustionRows = await db
                ...
                .from(heartbeatRunEvents)
                .where(and(inArray(heartbeatRunEvents.runId, runIds), eq(heartbeatRunEvents.eventType, "lifecycle"), sql `${heartbeatRunEvents.message} like 'Bounded retry exhausted%'`))
                .orderBy(asc(heartbeatRunEvents.runId), desc(heartbeatRunEvents.id));
            const retryExhaustedReasonByRunId = new Map();
            for (const row of exhaustionRows) {
                if (!row.message || retryExhaustedReasonByRunId.has(row.runId))
                    continue;
            ...
            const leaseRows = await db
                ...
                .where(and(eq(environmentLeases.companyId, companyId), inArray(environmentLeases.heartbeatRunId, runIds)))
                .orderBy(desc(environmentLeases.lastUsedAt), desc(environmentLeases.createdAt));
            const leaseByRunId = new Map();
            ...
            return runs.map((run) => {
                ...
                const workspacePath = typeof leaseMetadata?.remoteCwd === "string" && leaseMetadata.remoteCwd.trim().length > 0
        ...
        issuesForRun: async (runId) => {
            ...
            const fromActivity = await db
                ...
                .where(and(eq(activityLog.companyId, run.companyId), eq(activityLog.runId, runId), eq(activityLog.entityType, "issue"), isNull(issues.hiddenAt)))
                .orderBy(issueIdAsText);
            const context = run.contextSnapshot;
            const contextIssueId = context && typeof context === "object" && typeof context.issueId === "string"
                ? context.issueId
                : null;
            if (!contextIssueId)
                return fromActivity;
