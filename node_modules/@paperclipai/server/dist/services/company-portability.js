...
import { AGENT_DEFAULT_MAX_CONCURRENT_RUNS, ISSUE_PRIORITIES, ISSUE_STATUSES, PROJECT_ICON_NAMES, PROJECT_STATUSES, ROUTINE_CATCH_UP_POLICIES, ROUTINE_CONCURRENCY_POLICIES, ROUTINE_STATUSES, ROUTINE_TRIGGER_KINDS, ROUTINE_TRIGGER_SIGNING_MODES, deriveProjectUrlKey, envConfigSchema, issueCommentAuthorTypeSchema, issueCommentMetadataSchema, issueCommentPresentationSchema, normalizeAgentUrlKey, } from "@paperclipai/shared";
...
import { PORTABLE_CATALOG_PROVENANCE_STRING_KEYS, readCatalogStringList, readPortableCatalogProvenance, } from "./catalog-provenance.js";
...
function collectAgentSafeImportPolicyErrors(manifest, include) {
    const errors = [];
    if (include.projects) {
...
function buildPortableCatalogProvenance(skill) {
    ...
    if (auditCodes)
        provenance.auditCodes = auditCodes;
...
function normalizeProjectIconName(value) {
    return value && PROJECT_ICON_NAMES.includes(value) ? value : null;
}
...
function derivePortableCommentAuthorType(value) {
...
function readPortableIssueComments(value, warnings, sourceLabel) {
...
function applyImportAdapterRunDefaults(adapterType, adapterConfig) {
...
function filterExportFiles(files, selectedFilesInput, paperclipExtensionPath) {
...
function findPaperclipExtensionPath(files) {
    if (typeof files[".paperclip.yaml"] === "string")
        return ".paperclip.yaml";
    if (typeof files[".paperclip.yml"] === "string")
        return ".paperclip.yml";
...
function envInputScopedKey(input) {
    if (input.agentSlug)
        return `agent:${input.agentSlug}:${input.key}`;
    if (input.projectSlug)
        return `project:${input.projectSlug}:${input.key}`;
    return input.key;
}
function envInputValue(input, values) {
    if (!values)
        return null;
...
function importSecretLabel(input) {
...
function importSecretKey(input, suffix) {
    const scope = input.agentSlug
        ? `agent-${input.agentSlug}`
        : input.projectSlug
            ? `project-${input.projectSlug}`
...
function writeManifestEnvBinding(manifest, input, binding) {
    if (input.agentSlug) {
...
function buildManifestFromPackageFiles(files, opts) {
    ...
    const manifest = {
        ...
        company: {
            ...
            attachmentMaxBytes: typeof paperclipCompany.attachmentMaxBytes === "number" && Number.isFinite(paperclipCompany.attachmentMaxBytes)
...
export function companyPortabilityService(db, storage) {
    ...
    function assertKnownImportAdapterType(type) {
    ...
    async function assertImportAdapterConfigConstraints(adapterType, adapterConfig) {
    ...
    async function prepareImportedAgentAdapter(companyId, adapterType, adapterConfig, desiredSkills, mode) {
        ...
        const nextAdapterConfig = writePaperclipSkillSyncPreference(applyImportAdapterRunDefaults(effectiveAdapterType, adapterConfig), desiredSkills);
        ...
        const normalizedAdapterConfig = await secrets.normalizeAdapterConfigForPersistence(companyId, nextAdapterConfig, { strictMode: strictSecretsMode, adapterType: effectiveAdapterType });
    ...
    async function materializeImportEnvInputValues(companyId, manifest, envInputs, secretValues, actorUserId, createdSecretIds = []) {
    ...
    function resolveImportedAssigneeAgentId(assigneeSlug, importedSlugToAgentId, existingSlugToAgentId, agentStatusById, warnings, subjectLabel) {
        if (!assigneeSlug)
            return null;
        const assigneeAgentId = importedSlugToAgentId.get(assigneeSlug)
        ...
        if (assigneeStatus === "pending_approval" || assigneeStatus === "terminated") {
            warnings.push(`${subjectLabel} assignee ${assigneeSlug} is ${assigneeStatus}; imported work was left unassigned.`);
            return null;
        }
    ...
    async function exportBundle(companyId, input) {
        ...
        const paperclipAgentsOut = {};
        const paperclipProjectsOut = {};
        const paperclipTasksOut = {};
        const unportableTaskWorkspaceRefs = new Map();
        const paperclipRoutinesOut = {};
        ...
        if (include.agents) {
            for (const agent of agentRows) {
                ...
                const desiredSkills = readPaperclipSkillSyncPreference(agent.adapterConfig ?? {}).desiredSkills;
        ...
        const paperclipAgents = Object.fromEntries(Object.entries(paperclipAgentsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0));
        const paperclipProjects = Object.fromEntries(Object.entries(paperclipProjectsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0));
        const paperclipTasks = Object.fromEntries(Object.entries(paperclipTasksOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0));
        const paperclipRoutines = Object.fromEntries(Object.entries(paperclipRoutinesOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0));
    ...
    async function buildPreview(input, options) {
        ...
        if (mode === "agent_safe" && include.projects && targetCompanyId) {
            for (const project of manifest.projects) {
                if (!project.env)
                    continue;
                try {
                    await secrets.normalizeEnvBindingsForPersistence(targetCompanyId, project.env, {
        ...
        for (const manifestAgent of selectedAgents) {
            if (manifestAgent.reportsToExistingAgentId
                && !existingAgentIds.has(manifestAgent.reportsToExistingAgentId)) {
                warnings.push(`Agent ${manifestAgent.slug} references existing manager id ${manifestAgent.reportsToExistingAgentId}, but that agent is not present in the target company.`);
            }
            if (manifestAgent.reportsToExistingAgentSlug
                && !existingSlugToAgent.has(manifestAgent.reportsToExistingAgentSlug)) {
                warnings.push(`Agent ${manifestAgent.slug} references existing manager slug ${manifestAgent.reportsToExistingAgentSlug}, but that agent is not present in the target company.`);
            }
    ...
    async function importBundle(input, actorUserId, options) {
        ...
        if (input.target.mode === "new_company") {
            ...
            if (mode === "agent_safe" && options?.sourceCompanyId) {
                ...
                await access.ensureRoleDefaultGrants(created.id, ownerPrincipalId, "owner", actorUserId ?? null);
            }
        ...
        if (!targetCompany)
            throw notFound("Target company not found");
        const importedAgentEnvSlugs = new Set(plan.preview.plan.agentPlans
            .filter((entry) => entry.action !== "skip")
            .map((entry) => entry.slug));
        const importedProjectEnvSlugs = new Set(plan.preview.plan.projectPlans
            .filter((entry) => entry.action !== "skip")
            .map((entry) => entry.slug));
        ...
        const createdImportSecretIds = [];
        try {
            await materializeImportEnvInputValues(targetCompany.id, sourceManifest, importEnvInputs, input.secretValues, actorUserId, createdImportSecretIds);
            if (include.company) {
            ...
            const preImportExistingSlugToAgentId = new Map();
            const preImportExistingAgentIds = new Set();
            ...
            if (include.agents) {
                for (const planAgent of plan.preview.plan.agentPlans) {
                    ...
                    const normalizedAdapter = await prepareImportedAgentAdapter(targetCompany.id, adapterOverride?.adapterType ?? manifestAgent.adapterType, baseAdapterConfig, desiredSkills, mode);
                    const patch = {
                        name: planAgent.plannedName,
                    ...
                    if (planAgent.action === "update" && planAgent.existingAgentId) {
                        ...
                        await secrets.syncEnvBindingsForTarget?.(targetCompany.id, { targetType: "agent", targetId: updated.id }, isPlainRecord(updated.adapterConfig) ? updated.adapterConfig.env : undefined);
                    ...
                    await secrets.syncEnvBindingsForTarget?.(targetCompany.id, { targetType: "agent", targetId: created.id }, isPlainRecord(created.adapterConfig) ? created.adapterConfig.env : undefined);
                ...
                for (const manifestAgent of plan.selectedAgents) {
                    ...
                    if (!managerSlug && !existingManagerId)
                        continue;
                    const managerId = existingManagerId
                        ?? (managerSlug
            ...
            if (include.projects) {
                for (const planProject of plan.preview.plan.projectPlans) {
                    ...
                    const normalizedProjectEnv = manifestProject.env
                        ? await secrets.normalizeEnvBindingsForPersistence(targetCompany.id, manifestProject.env, {
                            strictMode: strictSecretsMode,
                    ...
                    if (!projectId)
                        continue;
                    await secrets.syncEnvBindingsForTarget?.(targetCompany.id, { targetType: "project", targetId: projectId }, normalizedProjectEnv ?? {});
            ...
            if (include.issues) {
                ...
                for (const manifestIssue of sourceManifest.issues) {
                    ...
                    const assigneeAgentId = resolveImportedAssigneeAgentId(manifestIssue.assigneeAgentSlug, importedSlugToAgentId, existingSlugToAgentId, agentStatusById, warnings, `Task ${manifestIssue.slug}`);
                    ...
                    let issueStatus = manifestIssue.status && ISSUE_STATUSES.includes(manifestIssue.status)
                        ? manifestIssue.status
                        : "backlog";
