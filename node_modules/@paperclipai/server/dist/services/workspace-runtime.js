...
function parseRemoteTrackingRef(ref) {
...
async function refreshRemoteTrackingBaseRef(repoRoot, baseRef) {
...
async function resolveBaseRefSha(repoRoot, baseRef) {
    return await runGit(["rev-parse", "--verify", `${baseRef}^{commit}`], repoRoot).catch(() => null);
}
function readRecordedBaseRefSha(metadata) {
...
export async function inspectExecutionWorkspaceBaseDrift(input) {
    ...
    if (branchBaseRefSha !== currentBaseRefSha) {
        ...
        warnings.push(`Execution workspace branch ${input.branchName ? `"${input.branchName}"` : "HEAD"} is behind ${baseRef} by ${behindText}: ${recordedText}, current base ${formatShortSha(currentBaseRefSha)}. Refresh or rebase the workspace before relying on recent base-branch fixes.`);
    }
...
async function resolveAuthoritativeBaseRef(repoRoot, configuredBaseRef) {
    const warnings = [];
    ...
    if (await localBranchExists(repoRoot, configured)) {
        ...
        if (await remoteExists(repoRoot, "origin")) {
            warnings.push(`Configured base ref "${configured}" is a local branch with no matching origin/${configured}; basing the execution workspace on the local ref, which may include unpushed commits.`);
        }
        return { baseRef: configured, warnings, refreshed: false };
    }
    return { baseRef: configured, warnings, refreshed: false };
...
async function refreshUnstartedWorktreeToBase(input) {
    ...
    const commitsPastBaseRaw = await runGit(["rev-list", "--count", `${input.currentBaseRefSha}..HEAD`], input.worktreePath).catch(() => null);
...
export async function realizeExecutionWorkspace(input) {
    ...
    const { baseRef, warnings: baseRefResolutionWarnings, refreshed: baseRefAlreadyRefreshed, } = await resolveAuthoritativeBaseRef(repoRoot, configuredBaseRef);
...
function isPaperclipDevRuntimeService(input) {
    ...
    return (serviceName === "paperclip-dev"
        || serviceName === "paperclip-dev-once"
        || (command.includes("dev:once") && command.includes("tailscale-auth")));
}
function resolveRuntimeServiceHealthUrl(url, input) {
    if (!url || !isPaperclipDevRuntimeService(input ?? {}))
        return url;
    try {
...
async function isRuntimeServiceUrlHealthy(url, input) {
    if (!url)
        return true;
