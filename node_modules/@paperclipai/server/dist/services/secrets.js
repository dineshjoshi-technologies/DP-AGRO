...
import { agents, companySecretBindings, companySecretProviderConfigs, companySecrets, companySecretVersions, environments, heartbeatRuns, issues, projects, routines, secretAccessEvents, } from "@paperclipai/db";
import { createSecretProviderConfigSchema, deriveProjectUrlKey, envBindingSchema, isUuidLike, normalizeAgentUrlKey, secretProviderConfigPayloadSchema, secretProviderConfigDiscoveryPreviewSchema, updateSecretProviderConfigSchema, } from "@paperclipai/shared";
...
export function secretService(db) {
    ...
    async function assertBindingContext(companyId, secretId, context) {
        ...
        if (Array.isArray(context.allowedBindingIds) &&
            ...
            throw unprocessable("Secret binding is outside the active low-trust boundary", { code: "binding_not_allowed" });
        }
        return binding;
    }
    ...
    async function assertSecretInCompany(companyId, secretId, source = db) {
    ...
    async function resolveSecretValueInternal(companyId, secretId, version, options) {
    ...
    async function resolveSecretValueForEphemeralAccess(companyId, secretId, version, context) {
        ...
        if ((context.actorType !== "agent" && context.actorType !== "user") ||
            !context.actorId?.trim()) {
            throw forbidden("Ephemeral secret resolution requires an authenticated actor");
        }
        const actor = context.actorType === "agent"
            ? {
                type: "agent",
                agentId: context.actorId,
                companyId,
                source: context.actorSource === "agent_jwt" ? "agent_jwt" : "agent_key",
            }
            : {
                type: "board",
    ...
    async function listAdapterSchemaSecretFieldKeys(adapterType) {
        if (!adapterType)
            return [];
    ...
    async function normalizeSchemaSecretFieldForPersistence(companyId, input) {
    ...
    async function createManagedLocalSecret(companyId, input, actor) {
    ...
    return {
        ...
        previewProviderConfigDiscovery: async (companyId, input) => {
        ...
        syncSecretRefsForTarget: async (companyId, target, refs, options) => {
            ...
            await db.transaction(async (tx) => {
                if (options?.replaceAll) {
                    ...
                    for (const pathPrefix of pathPrefixes) {
                        await tx
                            .delete(companySecretBindings)
                            .where(and(eq(companySecretBindings.companyId, companyId), eq(companySecretBindings.targetType, target.targetType), eq(companySecretBindings.targetId, target.targetId), or(eq(companySecretBindings.configPath, pathPrefix), like(companySecretBindings.configPath, `${pathPrefix}.%`))));
                    }
                }
                else {
                    await tx
        ...
        listBindingCompanyIdsForTarget: async (target) => db
            .select({ companyId: companySecretBindings.companyId })
            .from(companySecretBindings)
            .where(and(eq(companySecretBindings.targetType, target.targetType), eq(companySecretBindings.targetId, target.targetId)))
        ...
        syncEnvBindingsForTarget: async (companyId, target, envValue, options) => {
            ...
            const writeBindings = async (targetDb) => {
                ...
                if (refs.length === 0)
                    return;
                await targetDb.insert(companySecretBindings).values(refs.map((ref) => ({
                    companyId,
                    secretId: ref.secretId,
                    targetType: target.targetType,
                    targetId: target.targetId,
        ...
        collectMissingRuntimeBindings: async (companyId, envValue, context) => {
            const record = asRecord(envValue);
            if (!record)
                return [];
