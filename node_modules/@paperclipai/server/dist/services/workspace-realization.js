...
function readWorkspaceRealizationRequest(value) {
    ...
    if (!localPath || !companyId || !environmentId || !heartbeatRunId || !adapterType)
        return null;
    return {
        version: 1,
        adapterType,
        ...
        source: {
            kind: source.kind === "task_session" || source.kind === "agent_home"
                ? source.kind
                : "project_primary",
            localPath,
...
export function buildWorkspaceRealizationRecord(input) {
    ...
    const transport = input.environment.driver === "ssh" || input.environment.driver === "sandbox" || input.environment.driver === "plugin"
        ? input.environment.driver
        : "local";
    const remotePath = readString(providerMetadata.remoteCwd) ??
...
//# sourceMappingURL=workspace-realization.js.map