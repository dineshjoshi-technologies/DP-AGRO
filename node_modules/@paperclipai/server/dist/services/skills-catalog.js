...
let cachedCatalogPaths = null;
let cachedCatalogPathsError = null;
...
export function isCatalogManifestUnavailableError(error) {
    return error instanceof CatalogManifestUnavailableError;
}
function manifestUnavailableMessage(manifestPath) {
...
function loadCatalogManifest(manifestPath) {
    try {
...
function resolveCatalogSourcePath(source, relativePath) {
    const sourceRoot = sourceRootPath(source);
    return sourceRoot ? `${sourceRoot}/${relativePath}` : relativePath;
}
async function fetchCatalogSourceFile(skill, relativePath) {
    ...
    if (source.type !== "github") {
        throw unprocessable(`Unsupported catalog source type: ${source.type}`);
    }
...
async function readCatalogFileBytes(skill, relativePath) {
...
export function listCatalogSkills(query = {}) {
...
export function listCatalogSkillsOrEmpty(query = {}) {
    try {
...
export function resolveCatalogSkillReference(reference) {
    ...
    if (exact)
        return { skill: exact, ambiguous: false };
    const slugMatches = catalogSkills.filter((skill) => skill.slug === trimmed);
    if (slugMatches.length === 1)
        return { skill: slugMatches[0], ambiguous: false };
    if (slugMatches.length > 1)
        return { skill: null, ambiguous: true };
    return { skill: null, ambiguous: false };
}
export function getCatalogSkillOrThrow(reference) {
...
export async function readCatalogSkillFile(reference, relativePath = "SKILL.md") {
...
export async function copyCatalogSkillFile(reference, relativePath, targetPath) {
    const skill = getCatalogSkillOrThrow(reference);
    const normalizedPath = normalizePortablePath(relativePath || "SKILL.md");
    const fileEntry = skill.files.find((entry) => entry.path === normalizedPath);
    if (!fileEntry) {
