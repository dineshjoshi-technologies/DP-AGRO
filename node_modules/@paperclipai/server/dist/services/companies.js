...
import { companies, companyLogos, assets, agents, agentApiKeys, agentRuntimeState, agentTaskSessions, agentWakeupRequests, issues, issueComments, projects, goals, heartbeatRuns, heartbeatRunEvents, costEvents, financeEvents, issueReadStates, approvalComments, approvals, activityLog, companySecrets, joinRequests, invites, principalPermissionGrants, companyMemberships, companySkills, documents, } from "@paperclipai/db";
import { notFound, unprocessable } from "../errors.js";
import { environmentService } from "./environments.js";
import { heartbeatService } from "./heartbeat.js";
import { logActivity } from "./activity-log.js";
const SYSTEM_COMPANY_ACTOR = {
    actorType: "system",
    actorId: "system",
    agentId: null,
    runId: null,
...
export function companyService(db) {
    ...
    async function applyArchiveCascadeInTx(tx, id) {
        const pausedAgentRows = await tx
            ...
            .where(and(eq(agents.companyId, id), notInArray(agents.status, ["paused", "terminated", "pending_approval"])))
        ...
        await tx
            ...
            .where(and(eq(agentWakeupRequests.companyId, id), inArray(agentWakeupRequests.status, ["queued", "deferred_issue_execution", "claimed"]), isNull(agentWakeupRequests.runId)));
        return { agentsPaused: pausedAgentRows.length, activeRunIds };
    }
    async function finalizeArchive(id, actor, cascade) {
    ...
    async function getMonthlySpendByCompanyIds(companyIds, database = db) {
    ...
    async function hydrateCompanySpend(rows, database = db) {
    ...
    function getCompanyQuery(database) {
        return database
    ...
    return {
        ...
        create: async (data) => {
            ...
            const row = await getCompanyQuery(db)
                ...
                .then((rows) => rows[0] ?? null);
            if (!row)
                throw notFound("Company not found after creation");
            const [hydrated] = await hydrateCompanySpend([row], db);
            return enrichCompany(hydrated);
        },
        update: async (id, data, actor = SYSTEM_COMPANY_ACTOR) => {
            const result = await db.transaction(async (tx) => {
                ...
                if (willReactivate) {
                    const restoredRows = await tx
                        ...
                        .where(and(eq(agents.companyId, id), eq(agents.status, "paused"), eq(agents.pauseReason, "company_archived")))
                        .returning({ id: agents.id });
        ...
        archive: async (id, actor = SYSTEM_COMPANY_ACTOR) => {
            const result = await db.transaction(async (tx) => {
