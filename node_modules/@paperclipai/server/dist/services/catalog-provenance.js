...
function asCatalogString(value) {
    if (typeof value !== "string")
        return null;
    const trimmed = value.trim();
    return trimmed.length > 0 ? trimmed : null;
}
export function readCatalogStringList(value) {
    if (!Array.isArray(value))
        return null;
    const entries = value.map((entry) => asCatalogString(entry)).filter((entry) => Boolean(entry));
    return entries.length === value.length ? entries : null;
}
function isCatalogRecord(value) {
    return typeof value === "object" && value !== null && !Array.isArray(value);
}
export function readPortableCatalogProvenance(metadata, canonicalKey = null) {
    ...
    if (!canonicalKey && catalogSkillKey)
        normalized.skillKey = catalogSkillKey;
    ...
    if (sourceRef && !normalized.originHash)
        normalized.originHash = sourceRef;
    const auditCodes = readCatalogStringList(catalog.auditCodes);
    if (auditCodes)
        normalized.auditCodes = auditCodes;
    return {
        sourceRef,
        metadata: normalized,
    };
