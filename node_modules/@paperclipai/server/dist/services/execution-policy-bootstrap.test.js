...
const { parseExecutionPolicyBootstrapEnv, applyExecutionPolicyBootstrap, } = await import("./execution-policy-bootstrap.js");
...
describe("parseExecutionPolicyBootstrapEnv", () => {
    ...
    it("returns null when execution mode is explicitly any", () => {
        expect(parseExecutionPolicyBootstrapEnv(env({ PAPERCLIP_EXECUTION_MODE: "any" }))).toBeNull();
    });
    ...
    it("throws on an unknown execution mode", () => {
        expect(() => parseExecutionPolicyBootstrapEnv(env({ PAPERCLIP_EXECUTION_MODE: "vm" }))).toThrow(/PAPERCLIP_EXECUTION_MODE/);
    });
    ...
    it("throws when PAPERCLIP_K8S_RPC_TIMEOUT_MS is not a positive integer", () => {
        expect(() => parseExecutionPolicyBootstrapEnv(env({ PAPERCLIP_EXECUTION_MODE: "kubernetes", PAPERCLIP_K8S_RPC_TIMEOUT_MS: "0" }))).toThrow(/PAPERCLIP_K8S_RPC_TIMEOUT_MS/);
        expect(() => parseExecutionPolicyBootstrapEnv(env({ PAPERCLIP_EXECUTION_MODE: "kubernetes", PAPERCLIP_K8S_RPC_TIMEOUT_MS: "abc" }))).toThrow(/PAPERCLIP_K8S_RPC_TIMEOUT_MS/);
    });
});
describe("applyExecutionPolicyBootstrap", () => {
    ...
    it("throws when at least one company fails, after attempting every company", async () => {
        listCompanyIds.mockResolvedValue(["c1", "c2", "c3"]);
        ensureKubernetesEnvironment.mockImplementation(async (companyId) => {
            if (companyId === "c2")
                throw new Error("operator config missing");
            return { id: `env-${companyId}` };
        });
        await expect(applyExecutionPolicyBootstrap(fakeDb, bootstrap)).rejects.toThrow(/execution-policy bootstrap: 1 of 3 companies failed.*c2/);
        // It keeps going past the failure (attempts all three companies).
        expect(ensureKubernetesEnvironment).toHaveBeenCalledTimes(3);
    });
});
//# sourceMappingURL=execution-policy-bootstrap.test.js.map