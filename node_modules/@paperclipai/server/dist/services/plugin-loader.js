...
export function buildPluginWorkerEnv(input) {
    ...
    if (!canRegisterEnvironmentDrivers)
        return env;
...
function buildLocalPluginBuildCommand(pkgJson) {
    const packageName = pkgJson["name"];
    if (typeof packageName !== "string" || packageName.trim().length === 0)
        return null;
    return `pnpm --filter ${packageName} build`;
}
function readPackageDependencyNames(pkgJson, field) {
    const deps = pkgJson[field];
    if (deps === null || typeof deps !== "object" || Array.isArray(deps))
        return [];
    return Object.keys(deps);
}
function resolvePackageInstallPath(packageRoot, packageName) {
...
export function isRepoBundledPluginPath(packageRoot, options = {}) {
...
export function isStandaloneBundledPluginPath(packageRoot, options = {}) {
    const repoRoot = options.repoRoot ?? REPO_ROOT;
    return isPathWithin(path.join(repoRoot, "packages", "plugins", "sandbox-providers"), packageRoot);
}
export function resolveDeclaredPluginEntrypoints(packageRoot, pkgJson) {
    ...
    for (const key of ["manifest", "worker", "ui"]) {
        const relativePath = paperclipPlugin[key];
...
export function listMissingDeclaredPluginEntrypoints(packageRoot, pkgJson) {
    return resolveDeclaredPluginEntrypoints(packageRoot, pkgJson).filter((entrypoint) => !existsSync(entrypoint.absolutePath));
}
function listMissingStandaloneBundledPluginRuntimeDependencies(packageRoot, pkgJson) {
    ...
    return [...dependencyNames].filter((packageName) => !existsSync(resolvePackageInstallPath(packageRoot, packageName)));
}
function formatLocalPluginManualBuildHint(packageRoot, pkgJson, options = {}) {
    if (!isRepoBundledPluginPath(packageRoot, { repoRoot: options.repoRoot }))
        return "";
    const manualBuildCommand = buildLocalPluginRecoveryCommand(packageRoot, pkgJson, { repoRoot: options.repoRoot });
    if (!manualBuildCommand)
        return "";
...
function buildStandaloneBundledPluginInstallArgs(packageRoot) {
...
function buildStandaloneBundledPluginInstallCommand(packageRoot) {
    return `pnpm ${buildStandaloneBundledPluginInstallArgs(packageRoot).join(" ")}`;
}
function buildLocalPluginRecoveryCommand(packageRoot, pkgJson, options = {}) {
...
function buildLocalPluginBuildCommands(packageRoot, pkgJson, options = {}) {
    if (isStandaloneBundledPluginPath(packageRoot, { repoRoot: options.repoRoot })) {
        const commands = [];
        const shouldInstallStandaloneRuntime = options.needsStandaloneRuntimeBootstrap === true
...
export async function ensureLocalPluginBuilt(packageRoot, pkgJson, options = {}) {
    const processEnv = options.processEnv ?? process.env;
    if (processEnv["PAPERCLIP_DISABLE_PLUGIN_AUTOBUILD"] === "1")
        return;
    if (!isRepoBundledPluginPath(packageRoot, { repoRoot: options.repoRoot }))
        return;
    ...
    const missingStandaloneRuntimeDeps = isStandaloneBundledPluginPath(packageRoot, { repoRoot: options.repoRoot })
        ...
        : [];
    if (missingEntrypoints.length === 0 && missingStandaloneRuntimeDeps.length === 0)
        return;
    const packageName = pkgJson["name"];
    const manualBuildCommand = buildLocalPluginRecoveryCommand(packageRoot, pkgJson, { repoRoot: options.repoRoot });
    if (typeof packageName !== "string" || packageName.trim().length === 0 || !manualBuildCommand)
        return;
    ...
    try {
        for (const command of buildCommands) {
            await runExecFileAsync(command.file, command.args, { cwd: command.cwd, timeout: LOCAL_PLUGIN_AUTOBUILD_TIMEOUT_MS });
        }
    }
    catch (error) {
        const stderr = typeof error.stderr === "string"
            ? error.stderr.trim()
            : "";
        const timeoutMessage = error.killed === true
            ? ` after timing out at ${LOCAL_PLUGIN_AUTOBUILD_TIMEOUT_MS}ms`
            : "";
        const stderrMessage = stderr.length > 0 ? ` stderr: ${stderr}` : "";
        throw new Error(`Failed to auto-build bundled local plugin ${packageName}${timeoutMessage}. ` +
    ...
    if (stillMissingEntrypoints.length > 0 || stillMissingStandaloneRuntimeDeps.length > 0) {
        ...
        throw new Error(`Bundled local plugin ${packageName} is still missing ${missingDetails.join("; ")} after auto-build. ` +
    ...
    async function fetchAndValidate(installOptions) {
        ...
        if (!manifestPath || !existsSync(manifestPath)) {
            const manualBuildHint = localPath
                ...
                : "";
            throw new Error(`Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a Paperclip plugin (no manifest found).${manualBuildHint}`);
        }
