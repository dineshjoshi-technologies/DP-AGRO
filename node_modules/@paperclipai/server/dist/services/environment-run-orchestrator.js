...
// Service factory
// ---------------------------------------------------------------------------
export function environmentRunOrchestrator(db, options = {}) {
    ...
    async function resolveEnvironment(input) {
        ...
        const environment = environmentId === input.localEnvironmentId
    ...
    async function acquireLease(input) {
        try {
            ...
            throw new EnvironmentRunError("lease_acquire_failed", `Failed to acquire lease for environment "${input.environment.name}" (${input.environment.driver}): ${err instanceof Error ? err.message : String(err)}`, {
    ...
    async function resolveTransport(input) {
        try {
            ...
            throw new EnvironmentRunError("transport_resolution_failed", `Failed to resolve execution transport for "${input.environment.name}": ${err instanceof Error ? err.message : String(err)}`, {
                environmentId: input.environment.id,
                driver: input.environment.driver,
                cause: err,
            });
    ...
    async function realizeForRun(input) {
        const { environment, adapterType, companyId, issueId, heartbeatRunId, executionWorkspace, effectiveExecutionWorkspaceMode, } = input;
        ...
        let workspaceRealization = {};
        let realizedWorkspaceCwd = null;
        if (environment.driver === "local" ||
            environment.driver === "ssh" ||
            environment.driver === "sandbox") {
            try {
                const remoteCwd = typeof lease.metadata?.remoteCwd === "string" && lease.metadata.remoteCwd.trim().length > 0
                ...
                throw new EnvironmentRunError("workspace_realization_failed", `Failed to realize workspace for environment "${environment.name}" (${environment.driver}): ${err instanceof Error ? err.message : String(err)}`, {
                    environmentId: environment.id,
                    driver: environment.driver,
                    cause: err,
                });
        ...
        const realizedCwd = realizedWorkspaceCwd ??
        ...
        if (provisionCommand && environment.driver !== "local") {
            try {
                ...
                throw new EnvironmentRunError("workspace_realization_failed", `Failed to provision workspace for environment "${environment.name}" (${environment.driver}): ${err instanceof Error ? err.message : String(err)}`, {
                    environmentId: environment.id,
                    driver: environment.driver,
                    cause: err,
                });
        ...
        try {
            executionTarget = await resolveEnvironmentExecutionTarget({
                ...
                environment,
                leaseId: lease.id,
                leaseMetadata: lease.metadata ?? null,
                lease,
                environmentRuntime,
            });
        }
        catch (err) {
            throw new EnvironmentRunError("transport_resolution_failed", `Failed to resolve execution target for "${environment.name}": ${err instanceof Error ? err.message : String(err)}`, {
                environmentId: environment.id,
                driver: environment.driver,
                cause: err,
            });
...
//# sourceMappingURL=environment-run-orchestrator.js.map