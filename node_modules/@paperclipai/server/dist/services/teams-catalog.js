...
async function statCatalogManifest() {
    ...
    throw new Error(`Teams catalog manifest not found. Checked: ${catalogPackageRootCandidates.map((root) => path.join(root, "generated/catalog.json")).join(", ")}. Run pnpm --filter @paperclipai/teams-catalog build:manifest.`);
}
async function getCatalogManifest() {
    const { stats, manifestPath } = await statCatalogManifest();
    if (cachedCatalogManifest
...
export async function listCatalogTeams(query = {}) {
 ...
 */
export function readCatalogTeamProvenance(metadata) {
...
export async function resolveCatalogTeamReference(reference) {
    ...
    if (exact)
        return { team: exact, ambiguous: false };
    const slugMatches = teams.filter((team) => team.slug === trimmed);
    if (slugMatches.length === 1)
        return { team: slugMatches[0], ambiguous: false };
    if (slugMatches.length > 1)
        return { team: null, ambiguous: true };
    return { team: null, ambiguous: false };
}
export async function getCatalogTeamOrThrow(reference) {
...
function catalogTeamRoot(team) {
    return path.resolve(catalogPackageRoot, team.path);
}
function resolveCatalogTeamFile(team, relativePath) {
...
export async function readCatalogTeamFile(reference, relativePath = "TEAM.md") {
...
function renderStringArrayYaml(key, values) {
    if (values.length === 0)
        return [];
    return [
        `${key}:`,
...
function renderYamlFile(value) {
    return `${renderYamlBlock(value).join("\n")}\n`;
}
function renderSyntheticCompanyMarkdown(team) {
    const lines = [
        "---",
...
async function catalogProvenance(team) {
    const manifest = await getCatalogManifest();
    return {
        catalogId: team.id,
        catalogKey: team.key,
...
async function renderCatalogProvenanceYaml(team, targetManager) {
...
function mergePlainRecords(base, override) {
    const merged = { ...base };
...
function renderSimpleMarkdown(frontmatter, body) {
    const lines = ["---"];
    for (const [key, value] of Object.entries(frontmatter)) {
        if (value === undefined)
            continue;
        if (Array.isArray(value)) {
            lines.push(...renderStringArrayYaml(key, value.filter((entry) => typeof entry === "string")));
            continue;
        }
...
function collectCatalogSkillKeyMap(team) {
    const map = new Map();
    for (const requirement of team.requiredSkills) {
        if (requirement.type !== "catalog" || !requirement.catalogSkillKey)
            continue;
        map.set(requirement.ref, requirement.catalogSkillKey);
        if (requirement.catalogSkillId)
            map.set(requirement.catalogSkillId, requirement.catalogSkillKey);
        map.set(requirement.catalogSkillKey, requirement.catalogSkillKey);
        const slug = requirement.catalogSkillKey.split("/").at(-1);
        if (slug)
            map.set(slug, requirement.catalogSkillKey);
    }
    return map;
}
function rewriteAgentCatalogSkillRefs(team, files) {
    ...
    for (const agentPath of Object.keys(files).filter((filePath) => filePath.endsWith("/AGENTS.md") || filePath === "AGENTS.md")) {
        ...
        const skills = Array.isArray(parsed.frontmatter.skills)
            ? parsed.frontmatter.skills.filter((entry) => typeof entry === "string")
            : [];
        if (skills.length === 0)
            continue;
        const rewritten = skills.map((skill) => keyMap.get(skill) ?? skill);
        if (rewritten.every((skill, index) => skill === skills[index]))
            continue;
        files[agentPath] = renderSimpleMarkdown({ ...parsed.frontmatter, skills: rewritten }, parsed.body);
    }
}
function preparation(requirement, action, reason = null) {
    return {
...
function isPinnedSourceRef(value) {
    return Boolean(value && /^[0-9a-f]{40}$/i.test(value.trim()));
}
export function collectCatalogTeamSkillPreparations(team, sourcePolicy = {}) {
    const preparations = [];
    const warnings = [];
    const errors = [];
    for (const requirement of team.requiredSkills) {
...
async function readCatalogTeamSourceFiles(team) {
    const files = {
 ...
 */
function withSafeCatalogAdapterDefaults(agentSlugs, callerOverrides, defaultAdapterType) {
    const merged = { ...(callerOverrides ?? {}) };
...
function buildPortabilityInput(companyId, source, options) {
...
export function teamsCatalogService(db) {
    ...
    async function prepareCatalogTeamSource(companyId, catalogRef, options = {}) {
        ...
        const existingExtension = typeof files[".paperclip.yaml"] === "string"
    ...
    async function logCatalogEvent(action, companyId, team, actor, details) {
        if (!actor)
            return;
    ...
    async function previewCatalogTeamImport(companyId, catalogRef, options = {}) {
    ...
    async function prepareSkillInstalls(companyId, prepared) {
        const warnings = [];
    ...
    async function installCatalogTeam(companyId, catalogRef, options = {}) {
        const prepared = await prepareCatalogTeamSource(companyId, catalogRef, options);
        ...
        const importInput = {
            ...
            adapterOverrides: withSafeCatalogAdapterDefaults(prepared.team.agentSlugs, options.adapterOverrides, defaultAdapterType),
        ...
        const defaultedAdapterSlugs = prepared.team.agentSlugs.filter((slug) => !options.adapterOverrides?.[slug]);
        const warnings = [
            ...prepared.warnings,
        ...
        const result = await portability.importBundle(importInput, options.actor?.userId ?? (options.actor?.actorType === "user" ? options.actor.actorId : null), {
            mode: "agent_safe",
            sourceCompanyId: companyId,
        });
     ...
     * [PAP-10238 §3.2 + §5]).
     */
    async function listInstalledCatalogTeams(companyId) {
        ...
        for (const agent of companyAgents) {
            const provenance = readCatalogTeamProvenance(agent.metadata);
            if (!provenance)
                continue;
            ...
            if (!entry) {
                entry = {
                    catalogId: provenance.catalogId,
                    catalogKey: provenance.catalogKey,
                    originHashes: new Set(),
                    agentCount: 0,
                };
                byCatalogId.set(provenance.catalogId, entry);
            }
            entry.agentCount += 1;
            if (!entry.catalogKey && provenance.catalogKey)
                entry.catalogKey = provenance.catalogKey;
            if (provenance.originHash)
                entry.originHashes.add(provenance.originHash);
        }
...
//# sourceMappingURL=teams-catalog.js.map