...
import { authUsers, boardApiKeys, cliAuthChallenges, companies, companyMemberships, instanceUserRoles, } from "@paperclipai/db";
...
export function boardApiKeyExpiresAt(nowMs = Date.now()) {
    return new Date(nowMs + BOARD_API_KEY_TTL_MS);
}
export function cliAuthChallengeExpiresAt(nowMs = Date.now()) {
    return new Date(nowMs + CLI_AUTH_CHALLENGE_TTL_MS);
}
function challengeStatusForRow(row) {
    if (row.cancelledAt)
        return "cancelled";
    if (row.expiresAt.getTime() <= Date.now())
        return "expired";
    if (row.approvedAt && row.boardApiKeyId)
        return "approved";
    return "pending";
}
export function boardAuthService(db) {
    async function resolveBoardAccess(userId) {
    ...
    async function findBoardApiKeyByToken(token) {
    ...
    async function listBoardApiKeys(userId, opts = {}) {
        const conditions = [eq(boardApiKeys.userId, userId)];
        if (!opts.includeInactive) {
            const activeExpirationCondition = or(isNull(boardApiKeys.expiresAt), gt(boardApiKeys.expiresAt, new Date()));
            conditions.push(isNull(boardApiKeys.revokedAt));
            if (activeExpirationCondition)
                conditions.push(activeExpirationCondition);
        }
        return db
            .select({
            id: boardApiKeys.id,
    ...
    async function getBoardApiKeyForUser(keyId, userId) {
        return db
            .select({
            id: boardApiKeys.id,
    ...
    async function createCliAuthChallenge(input) {
        ...
        const pendingKeyName = input.requestedAccess === "instance_admin_required"
    ...
    async function getCliAuthChallenge(id) {
        return db
            .select()
            .from(cliAuthChallenges)
    ...
    async function getCliAuthChallengeBySecret(id, token) {
    ...
    async function describeCliAuthChallenge(id, token) {
    ...
    async function approveCliAuthChallenge(id, token, userId) {
        const access = await resolveBoardAccess(userId);
        return db.transaction(async (tx) => {
            await tx.execute(sql `select ${cliAuthChallenges.id} from ${cliAuthChallenges} where ${cliAuthChallenges.id} = ${id} for update`);
            ...
            return { status: "approved", challenge: updated };
        });
    }
    async function cancelCliAuthChallenge(id, token) {
        const challenge = await getCliAuthChallengeBySecret(id, token);
        if (!challenge)
            throw notFound("CLI auth challenge not found");
        const status = challengeStatusForRow(challenge);
    ...
    async function assertCurrentBoardKey(keyId, userId) {
        if (!keyId || !userId)
            throw conflict("Board API key context is required");
        const key = await db
            .select()
            .from(boardApiKeys)
            .where(and(eq(boardApiKeys.id, keyId), eq(boardApiKeys.userId, userId)))
...
//# sourceMappingURL=board-auth.js.map