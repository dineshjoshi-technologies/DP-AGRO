...
function defaultTimeoutSecForAdapter(adapterType) {
    return adapterType === "openclaw_gateway" ? 120 : 0;
}
export function normalizeMaxTurnStopReason(value) {
    return value === "max_turns_exhausted" || value === "turn_limit_exhausted"
        ? "max_turns_exhausted"
        : null;
}
export function resolveHeartbeatRunTimeoutPolicy(adapterType, adapterConfig) {
...
export function inferHeartbeatRunStopReason(input) {
    ...
    if (maxTurnStopReason)
        return maxTurnStopReason;
    ...
    if (input.outcome === "cancelled") {
        ...
        if (message.includes("pause") || message.includes("paused"))
            return "paused";
        return "cancelled";
    }
...
export function mergeHeartbeatRunStopMetadata(resultJson, metadata) {
...
//# sourceMappingURL=heartbeat-stop-metadata.js.map