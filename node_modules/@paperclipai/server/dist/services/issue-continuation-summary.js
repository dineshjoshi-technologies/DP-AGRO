...
const WAITING_FOR_REVIEW_OR_APPROVAL_RE = /\bwait(?:ing)? for\b.{0,160}\b(?:review(?:er)?(?: feedback)?|approval|board|human|user|operator)\b/i;
...
function readResultSummary(resultJson) {
    if (!resultJson || typeof resultJson !== "object" || Array.isArray(resultJson))
        return null;
    return (asNonEmptyString(resultJson.summary) ??
...
function extractPathCandidates(...texts) {
    const seen = new Set();
...
function inferNextAction(issue, run, previousNextAction) {
    ...
    if (run.status === "cancelled")
        return "Confirm the cancellation reason before starting another run.";
...
function extractPreviousNextAction(previousBody) {
...
export function extractContinuationSummaryNextAction(body) {
    return extractPreviousNextAction(body);
}
export function continuationSummaryParksExecutor(body) {
...
export function buildContinuationSummaryMarkdown(input) {
    ...
    const body = [
        ...
        "## Blockers / Decisions",
        "",
        bulletList(run.error
            ? [`Latest run ended with \`${run.status}\`; inspect the error before continuing.`]
            : ["No new blocker was recorded by the latest run."], "No blockers or decisions captured."),
        "",
        "## Next Action",
        "",
        `- ${nextAction}`,
...
export async function getIssueContinuationSummaryDocument(db, issueId) {
    const row = await db
        .select({
...
//# sourceMappingURL=issue-continuation-summary.js.map