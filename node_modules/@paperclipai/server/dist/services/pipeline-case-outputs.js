...
import { assets, companies, documents, documentRevisions, heartbeatRuns, issueAttachments, issueDocuments, issues, issueWorkProducts, pipelineCaseIssueLinks, pipelineCases, } from "@paperclipai/db";
import { SYSTEM_ISSUE_DOCUMENT_KEYS, } from "@paperclipai/shared";
...
function contentPath(attachmentId) {
    return `/api/attachments/${attachmentId}/content`;
}
function downloadPath(attachmentId) {
...
function sourceIssuePath(companyPrefix, identifier, issueId) {
    return `/${companyPrefix}/issues/${identifier ?? issueId}`;
}
function sourceDocumentPath(companyPrefix, identifier, issueId, key) {
    return `${sourceIssuePath(companyPrefix, identifier, issueId)}#document-${encodeURIComponent(key)}`;
}
function truncateContextExcerpt(value, maxLength = CONTEXT_OUTPUT_EXCERPT_MAX_LENGTH) {
    if (!value)
        return { excerpt: null, excerptTruncated: false };
    if (maxLength <= 0) {
...
function sanitizeOutputContextSummary(summary) {
    ...
    const items = summary.items.slice(0, boundedLimit).map((item) => {
        const excerpt = truncateContextExcerpt(item.excerpt, Math.min(CONTEXT_OUTPUT_EXCERPT_MAX_LENGTH, remainingExcerptChars));
        if (excerpt.excerpt) {
...
function deliverableDocumentRank(item) {
...
function outputSortGroup(item) {
    const deliverableRank = deliverableDocumentRank(item);
    if (deliverableRank !== null)
        return deliverableRank;
...
function contextFetchHint(item) {
    if (item.kind === "document") {
...
export function summarizePipelineCaseOutputsForContext(outputs, limit = CONTEXT_OUTPUT_ITEM_LIMIT) {
    ...
    let remainingExcerptChars = CONTEXT_OUTPUT_EXCERPT_TOTAL_MAX_LENGTH;
    const items = boundedItems.map((item) => {
        const excerpt = truncateContextExcerpt(item.preview, Math.min(CONTEXT_OUTPUT_EXCERPT_MAX_LENGTH, remainingExcerptChars));
        if (excerpt.excerpt) {
            remainingExcerptChars -= excerpt.excerpt.length;
        }
        const key = item.kind === "document"
            ? item.documentKey
...
export function formatPipelineCaseOutputContextMarkdown(summary) {
    if (!summary)
        return null;
...
export function pipelineCaseOutputsService(db) {
    return {
        listCaseOutputs: async (companyId, caseId) => {
            ...
            const sourceRows = await db
                ...
                .where(and(eq(pipelineCaseIssueLinks.companyId, companyId), eq(pipelineCaseIssueLinks.caseId, caseId), isNull(pipelineCaseIssueLinks.retiredAt), eq(issues.companyId, companyId), isNull(issues.hiddenAt), isNull(issues.cancelledAt), ne(issues.status, "cancelled")))
            ...
            if (sourceIssueIds.length > 0) {
                ...
                const documentRows = await db
                    ...
                    .leftJoin(latestRevision, and(eq(latestRevision.id, documents.latestRevisionId), eq(latestRevision.companyId, documents.companyId)))
                    .where(and(eq(issueDocuments.companyId, companyId), inArray(issueDocuments.issueId, sourceIssueIds), notInArray(issueDocuments.key, [...SYSTEM_ISSUE_DOCUMENT_KEYS])));
                ...
                const workProductRows = await db
                    ...
                    .leftJoin(workProductRun, and(eq(workProductRun.id, issueWorkProducts.createdByRunId), eq(workProductRun.companyId, issueWorkProducts.companyId)))
                    .where(and(eq(issueWorkProducts.companyId, companyId), inArray(issueWorkProducts.issueId, sourceIssueIds)));
                ...
                const attachmentRows = await db
                    ...
                    .where(and(eq(issueAttachments.companyId, companyId), inArray(issueAttachments.issueId, sourceIssueIds)));
...
//# sourceMappingURL=pipeline-case-outputs.js.map