...
import { COMPANY_SEARCH_MAX_LIMIT, COMPANY_SEARCH_MAX_OFFSET, COMPANY_SEARCH_MAX_TOKENS, COMPANY_ARTIFACTS_MAX_LIMIT, COMPANY_ARTIFACTS_MAX_QUERY_LENGTH, } from "@paperclipai/shared";
...
function tokenizeQuery(normalizedQuery) {
    ...
    for (const match of matches) {
        ...
        if (!tokens.includes(token))
            tokens.push(token);
        if (tokens.length >= COMPANY_SEARCH_MAX_TOKENS)
            break;
    }
    return tokens;
}
function fuzzyEligibleTokens(tokens) {
...
function tokenMatchExpression(textExpression, tokenArray) {
    return sql `
    EXISTS (
      SELECT 1
...
function findFirstMatchIndex(value, terms) {
...
function createSnippet(field, label, source, terms) {
...
function routePrefix(issuePrefix) {
    return issuePrefix?.trim() || "company";
}
function issueHref(prefix, issue, suffix = "") {
    return `/${prefix}/issues/${encodeURIComponent(issue.identifier ?? issue.id)}${suffix}`;
}
function matchTerms(normalizedQuery, tokens) {
    return [normalizedQuery, ...tokens].filter((term, index, terms) => term.length > 0 && terms.indexOf(term) === index);
}
function makeCounts(results) {
    const counts = { issue: 0, artifact: 0, agent: 0, project: 0 };
    for (const result of results)
        counts[result.type] += 1;
    return counts;
}
function scopeIncludesIssues(scope) {
    return scope === "all" || scope === "issues" || scope === "comments" || scope === "documents";
}
function scopeIncludesAgents(scope) {
    return scope === "all" || scope === "agents";
}
function scopeIncludesArtifacts(scope) {
    return scope === "all" || scope === "artifacts";
}
function scopeIncludesProjects(scope) {
    return scope === "all" || scope === "projects";
}
function issueSearchCondition(scope, input) {
    if (scope === "comments")
        return input.commentMatch;
    if (scope === "documents")
        return input.documentMatch;
    if (scope === "issues")
        return sql `(${input.issueTextMatch} OR ${input.fuzzyMatch})`;
    return sql `(${input.issueTextMatch} OR ${input.commentMatch} OR ${input.documentMatch} OR ${input.fuzzyMatch})`;
}
function selectPrimarySnippets(row, normalizedQuery, tokens) {
    ...
    return candidates.filter((snippet) => Boolean(snippet)).slice(0, 2);
}
function issueResult(row, prefix, normalizedQuery, tokens) {
    ...
    const previewImageUrl = extractFirstImageUrl(row.description) ??
...
function scoreSimpleRow(row, normalizedQuery, tokens) {
    ...
    if (row.title.toLowerCase().startsWith(normalizedQuery))
        score += 80;
    return score;
}
function artifactResult(artifact, normalizedQuery, tokens) {
    const terms = matchTerms(normalizedQuery, tokens);
...
function simpleTextCondition(fields, containsPattern, tokenArray) {
    const phraseConditions = fields.map((field) => sql `lower(coalesce(${field}, '')) LIKE ${containsPattern} ESCAPE '\\'`);
    const tokenConditions = fields.map((field) => tokenMatchExpression(field, tokenArray));
    return sql `(${sql.join([...phraseConditions, ...tokenConditions], sql ` OR `)})`;
}
export function companySearchBranchFetchLimit(limit, offset = 0) {
...
export function companySearchService(db) {
    return {
        search: async (companyId, query) => {
            ...
            const emptyCounts = { issue: 0, artifact: 0, agent: 0, project: 0 };
            ...
            const titlePhraseMatch = sql `lower(${issues.title}) LIKE ${containsPattern} ESCAPE '\\'`;
            const titleStartsWith = sql `lower(${issues.title}) LIKE ${startsWithPattern} ESCAPE '\\'`;
            const identifierPhraseMatch = sql `lower(coalesce(${issues.identifier}, '')) LIKE ${containsPattern} ESCAPE '\\'`;
            const identifierStartsWith = sql `lower(coalesce(${issues.identifier}, '')) LIKE ${startsWithPattern} ESCAPE '\\'`;
            const descriptionPhraseMatch = sql `lower(coalesce(${issues.description}, '')) LIKE ${containsPattern} ESCAPE '\\'`;
      ...
      `;
            ...
            const fuzzyMaxEditsExpr = sql.raw(`CASE
        ...
        `
            ...
            const fuzzyIdentifierMatch = fuzzyEnabled
                ...
                : noMatchSql();
            const fuzzyMatch = sql `(${fuzzyTokenTitleMatch} OR ${fuzzyIdentifierMatch})`;
            const tokenCoverage = sql `
        (
          SELECT count(*)::int
          FROM unnest(${tokenArray}) AS search_token(value)
      ...
      `;
            ...
            `,
                    ...
                    .from(issues)
                    .where(and(eq(issues.companyId, companyId), isNull(issues.hiddenAt), issueSearchCondition(scope, { issueTextMatch, commentMatch, documentMatch, fuzzyMatch })))
            ...
            const artifactRows = scopeIncludesArtifacts(scope)
                ...
                : [];
            const results = [
                ...issueRows.map((row) => issueResult(row, prefix, normalizedQuery, tokens)),
