...
import { agents, companySecretBindings, companySecretVersions, companySecrets, documentRevisions, documents, executionWorkspaces, goals, heartbeatRuns, issueInboxArchives, issueReadStates, issues, pluginManagedResources, plugins, projects, routineRevisions, routineRuns, routineDocuments, routines, routineTriggers, } from "@paperclipai/db";
import { WORKSPACE_BRANCH_ROUTINE_VARIABLE, getBuiltinRoutineVariableValues, extractRoutineVariableNames, interpolateRoutineTemplate, isValidRoutineDateString, pluginOperationIssueOriginKind, stringifyRoutineVariableValue, syncRoutineVariablesWithTemplate, } from "@paperclipai/shared";
...
const ROUTINE_DESCRIPTION_DOCUMENT_KEY = "description";
...
const zonedMinuteFormatterCache = new Map();
function getZonedMinuteFormatter(timeZone) {
...
function parseDateVariableValue(name, raw) {
    if (typeof raw !== "string") {
...
function createRoutineEnvFingerprint(env) {
...
export function routineService(db, deps = {}) {
    ...
    async function getRoutineDescriptionDocument(routineId, executor = db) {
        const row = await executor
            ...
            .where(and(eq(routineDocuments.routineId, routineId), eq(routineDocuments.key, ROUTINE_DESCRIPTION_DOCUMENT_KEY)))
            .then((rows) => rows[0] ?? null);
        return row ? mapRoutineDescriptionDocument(row) : null;
    }
    async function upsertRoutineDescriptionDocument(executor, routine, actor, options = {}) {
        if (executor === db) {
            return db.transaction(async (tx) => (upsertRoutineDescriptionDocument(tx, routine, actor, options)));
        }
        const now = new Date();
    ...
    return {
        ...
        getDescriptionDocument: async (routineId) => getRoutineDescriptionDocument(routineId),
        create: async (companyId, input, actor) => {
            ...
            const createdRoutine = await db.transaction(async (tx) => {
                ...
                if (env) {
                    await secretsSvc.syncEnvBindingsForTarget(companyId, { targetType: "routine", targetId: routine.id }, env, { db: tx });
                }
                return routine;
            });
        ...
        update: async (id, patch, actor) => {
            ...
            const updatedRoutine = await db.transaction(async (tx) => {
                ...
                if (locked.latestRevisionId) {
                    ...
                    if (latestRevision && snapshotsMatch(nextSnapshot, latestRevision.snapshot)) {
                        if (patch.env !== undefined) {
                            await secretsSvc.syncEnvBindingsForTarget(locked.companyId, { targetType: "routine", targetId: locked.id }, candidate.env, { db: tx });
                        }
                        return locked;
                    }
                }
                ...
                if (patch.env !== undefined) {
                    await secretsSvc.syncEnvBindingsForTarget(routine.companyId, { targetType: "routine", targetId: routine.id }, routine.env, { db: tx });
                }
                return routine;
            });
        ...
        deleteTrigger: async (id, actor = {}) => {
            ...
            if (result.deleted && existing.secretId) {
                try {
                    ...
                    logger.warn({ err, routineId: existing.routineId, triggerId: existing.id, secretId: existing.secretId }, "failed to remove routine trigger webhook secret after trigger deletion");
                }
            }
            return result;
        },
        ...
        restoreRevision: async (routineId, revisionId, actor) => {
            ...
            const result = await db.transaction(async (tx) => {
                ...
                await secretsSvc.syncEnvBindingsForTarget(locked.companyId, { targetType: "routine", targetId: locked.id }, routineSnapshot.env, { db: tx });
                return {
        ...
        runPipelineStageEntryRoutine: async (id, input, actor) => {
            const routine = await getRoutineById(id);
            if (!routine)
                throw notFound("Routine not found");
            if (routine.status === "archived")
