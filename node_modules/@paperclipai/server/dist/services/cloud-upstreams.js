...
import { agents, cloudUpstreamConnections, cloudUpstreamRuns, companies, goals, issueComments, issues, projects, routines, } from "@paperclipai/db";
...
export function cloudUpstreamService(db, options = {}) {
    ...
    return {
        ...
        preview: async (connectionId, companyId) => {
        ...
        createRun: async (input) => {
            ...
            const created = await db.transaction(async (tx) => {
                await tx.execute(sql `select ${cloudUpstreamConnections.id} from ${cloudUpstreamConnections} where ${cloudUpstreamConnections.id} = ${connection.id} and ${cloudUpstreamConnections.companyId} = ${connection.companyId} for update`);
        ...
        readRun: async (connectionId, runId, companyId) => {
            const row = await getRunRow(connectionId, runId, companyId);
            return runFromRow(row);
        },
        cancelRun: async (connectionId, runId, companyId) => {
            const row = await getRunRow(connectionId, runId, companyId);
    ...
    async function getConnectionRow(connectionId, companyId) {
        const row = await db
            .select()
            .from(cloudUpstreamConnections)
            .where(companyId
    ...
    async function getRunRow(connectionId, runId, companyId) {
        const row = await db
            .select()
            .from(cloudUpstreamRuns)
            .where(and(eq(cloudUpstreamRuns.id, runId), eq(cloudUpstreamRuns.connectionId, connectionId), eq(cloudUpstreamRuns.companyId, companyId)))
            .then((rows) => rows[0]);
        if (!row)
            throw notFound("Cloud upstream run was not found");
        return row;
    }
    async function assertNoRunningRun(connectionId, companyId, database) {
        const [running] = await database
            .select({ id: cloudUpstreamRuns.id })
            .from(cloudUpstreamRuns)
            .where(and(eq(cloudUpstreamRuns.connectionId, connectionId), eq(cloudUpstreamRuns.companyId, companyId), eq(cloudUpstreamRuns.status, "running")))
            .limit(1);
        if (running) {
...
function buildEntitiesFromPortableExport(localCompanyId, sourceInstanceId, exported) {
    const companyKey = {
        sourceInstanceId,
...
async function proofHeaders(connection, method, pathAndSearch) {
    ...
    return {
        ...
        "X-Paperclip-Upstream-Proof-Signature": sign(null, Buffer.from(payload, "utf8"), privateKeyPem).toString("base64url"),
    };
}
