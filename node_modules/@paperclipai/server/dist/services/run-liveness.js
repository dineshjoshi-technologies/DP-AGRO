...
const PLANNING_ONLY_RE = /\b(?:i(?:'ll| will| am going to|'m going to)|let me|i need to|next(?:,| i will| i'll)?|my next step is|the next step is)\s+(?:first\s+)?(?:inspect|check|review|look|investigate|analy[sz]e|open|read|start|begin|work on|implement|fix|test|update|create|add)\b/i;
const NEXT_STEPS_RE = /^\s*(?:next steps?|plan)\s*:/im;
const BLOCKER_RE = /\b(?:blocked|can't proceed|cannot proceed|unable to proceed|waiting on|need(?:s|ed)? .{0,80}\b(?:approval|access|credential|credentials|secret|api key|token|input|clarification)|requires? .{0,80}\b(?:approval|access|credential|credentials|secret|api key|token|input|clarification))\b/i;
const NEGATED_BLOCKER_RE = /\b(?:not blocked|no blocker|no blockers|unblocked)\b/i;
const APPROVAL_REQUIRED_RE = /\b(?:approval required|requires? .{0,80}\bapproval|need(?:s|ed)? .{0,80}\bapproval|waiting on .{0,80}\bapproval|pending approval|board approval|human approval|user approval|operator approval)\b/i;
const EXTERNAL_BLOCKER_RE = /\b(?:can't proceed|cannot proceed|unable to proceed|waiting on|blocked by|blocked on|need(?:s|ed)?|requires?) .{0,120}\b(?:access|credential|credentials|secret|secrets|api key|token|password|login|account|permission|permissions|input|clarification)\b/i;
const MANAGER_REVIEW_RE = /\b(?:manager review|human review|manual review|security review|escalate|production deploy|deploy(?:ing)? to production|deploy(?:ing)? to prod|prod deploy|production access|rotate .{0,40}\b(?:secret|key|token)|delete .{0,40}\bproduction|security-sensitive|credentialed operation|budget-sensitive|cost approval|spend approval)\b/i;
const RUNNABLE_RE = /\b(?:(?:run|rerun|execute)\s+(?:pnpm|npm|yarn|bun|vitest|jest|pytest|cargo|go test|curl|tests?|typecheck|build|lint|package|verification)|(?:inspect|check|review|look|investigate|analy[sz]e|open|read|start|begin|continue|implement|fix|test|update|create|add|write|verify|validate|report)\b)/i;
const PLAN_TASK_TITLE_RE = /\b(?:plan|planning|analysis|investigation|research|report|proposal|design doc|write-?up)\b/i;
const PLAN_TASK_DESCRIPTION_RE = /\b(?:create|write|produce|draft|update|revise|prepare)\s+(?:a\s+|the\s+)?(?:plan|analysis|investigation|research report|report|proposal|design doc|write-?up)\b/i;
...
function normalizeContinuationAttempt(value) {
    return typeof value === "number" && Number.isFinite(value) ? Math.max(0, Math.floor(value)) : 0;
}
...
function resultFinalText(resultJson) {
    if (!resultJson)
        return "";
    return [
...
function resultRawText(resultJson) {
    if (!resultJson)
        return "";
    return [
...
function highSignalSources(input) {
    return [
...
export function looksLikePlanningOnly(input) {
...
export function isPlanningOrDocumentTask(issue) {
    if (!issue)
        return false;
    if (PLAN_TASK_TITLE_RE.test(issue.title))
        return true;
...
export function hasConcreteActionEvidence(evidence) {
    ...
    return (normalized.issueCommentsCreated +
...
function evidenceReason(evidence) {
    const parts = [];
    if (evidence.issueCommentsCreated > 0)
        parts.push(`${evidence.issueCommentsCreated} issue comment(s)`);
    if (evidence.documentRevisionsCreated > 0)
        parts.push(`${evidence.documentRevisionsCreated} document revision(s)`);
    if (evidence.workProductsCreated > 0)
        parts.push(`${evidence.workProductsCreated} work product(s)`);
    if (evidence.workspaceOperationsCreated > 0)
        parts.push(`${evidence.workspaceOperationsCreated} workspace operation(s)`);
    if (evidence.activityEventsCreated > 0)
        parts.push(`${evidence.activityEventsCreated} activity event(s)`);
...
function isNoisyTranscriptLine(line) {
    const trimmed = line.trim();
    if (!trimmed)
        return true;
    return (/^(?:command|status|exit_code|tool|tool_call|tool_result|stdout|stderr|event|payload|session|cwd|ref_id)\s*:/i.test(trimmed) ||
...
function stripNoisyTranscriptLines(text) {
    return text
        .split(/\r?\n/)
        .map((line) => line.trim())
...
function nextNonNoiseLine(lines, startIndex) {
    for (let i = startIndex + 1; i < lines.length; i += 1) {
        const line = stripMarkdownListPrefix(lines[i] ?? "");
        if (!line || isNoisyTranscriptLine(line))
            continue;
        return line;
    }
    return null;
}
function extractNextActionFromText(text) {
    const lines = text.split(/\r?\n/).map((entry) => entry.trim());
    for (let i = 0; i < lines.length; i += 1) {
        const rawLine = lines[i] ?? "";
        if (!rawLine || isNoisyTranscriptLine(rawLine))
            continue;
        ...
        if (PLANNING_ONLY_RE.test(line))
            return line;
    }
    return null;
}
function extractNextAction(input) {
    ...
    const candidates = [
        ...
        ...rawSources(input),
    ].filter((value) => Boolean(readText(value)));
...
export function classifyRunActionability(input) {
    const text = actionabilityText(input);
    if (!text)
        return "unknown";
    ...
    if (APPROVAL_REQUIRED_RE.test(text))
    ...
    if (MANAGER_REVIEW_RE.test(text))
        return "manager_review";
    if (RUNNABLE_RE.test(text))
        return "runnable";
    return "unknown";
}
export function classifyRunLiveness(input) {
    ...
    const output = (state, reason, nextAction = null) => ({
        livenessState: state,
...
//# sourceMappingURL=run-liveness.js.map