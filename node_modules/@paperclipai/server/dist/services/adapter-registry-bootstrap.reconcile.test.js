...
vi.mock("./adapter-plugin-store.js", () => ({
    setAdapterDisabled: (type, disabled) => setAdapterDisabled(type, disabled),
}));
...
describe("reconcileAdapterAvailability", () => {
    ...
    it("throws when a declared adapter has no installed implementation", () => {
        setAdapterDisabled.mockReset();
        expect(() => reconcileAdapterAvailability([{ adapterType: "ghost_adapter", enabled: true }])).toThrow(/no installed adapter: ghost_adapter/);
    });
});
//# sourceMappingURL=adapter-registry-bootstrap.reconcile.test.js.map