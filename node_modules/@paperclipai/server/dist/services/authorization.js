...
import { agents, companyMemberships, heartbeatRuns, instanceUserRoles, issueComments, issues, principalPermissionGrants, projects, } from "@paperclipai/db";
import { LOW_TRUST_REVIEW_PRESET, extractAgentMentionIds } from "@paperclipai/shared";
import { LOW_TRUST_ISSUE_ANCESTRY_MAX_DEPTH, isIssueWithinLowTrustBoundary, resolveCoreTrustPreset, } from "./trust-preset-resolver.js";
import { logger } from "../middleware/logger.js";
function companyIdForResource(resource) {
    return resource.companyId;
}
function permissionForAction(action) {
    if (action === "agent_config:read" || action === "agent_config:update")
    ...
    if (action === "issue:comment" || action === "issue:mutate")
        return null;
    return action;
}
...
function readPolicyObject(container, key) {
...
function evaluateAuthorizationPolicyForAssignment(policy, label) {
    if (!policy || objectIsEmpty(policy))
        return { kind: "none" };
    ...
    const requiresApproval = readBoolean(protectedAgent?.requiresApproval) === true ||
...
function agentIsInSubtree(agentsById, rootAgentId, targetAgentId) {
    if (rootAgentId === targetAgentId)
        return true;
    let cursor = targetAgentId;
    for (let depth = 0; cursor && depth < 50; depth += 1) {
        const current = agentsById.get(cursor);
        if (!current)
            return false;
        if (current.reportsTo === rootAgentId)
            return true;
        cursor = current.reportsTo;
    }
    return false;
}
async function loadCompanyAgentHierarchy(db, companyId) {
    const rows = await db
...
async function isAgentInSubtree(db, companyId, rootAgentId, targetAgentId) {
    return agentIsInSubtree(await loadCompanyAgentHierarchy(db, companyId), rootAgentId, targetAgentId);
}
async function scopeAllows(db, companyId, grantScope, requestedScope, options = {}) {
    if (!grantScope || Object.keys(grantScope).length === 0)
        return !options.requireStructuredScope;
    if (!requestedScope)
        return false;
    const targetAssigneeAgentId = typeof requestedScope.assigneeAgentId === "string"
    ...
    if (projectIds.length > 0) {
        constrained = true;
        if (!scopeIncludesId(projectIds, requestedProjectId))
            return false;
    }
    ...
    if (targetAgentIds.length > 0) {
        constrained = true;
        if (!scopeIncludesId(targetAgentIds, targetAssigneeAgentId))
            return false;
    }
...
export function authorizationService(db) {
    ...
    async function getActiveMembership(companyId, principalType, principalId) {
        return db
            .select()
            .from(companyMemberships)
            .where(and(eq(companyMemberships.companyId, companyId), eq(companyMemberships.principalType, principalType), eq(companyMemberships.principalId, principalId), eq(companyMemberships.status, "active")))
            .then((rows) => rows[0] ?? null);
    }
    async function findGrant(companyId, principalType, principalId, permissionKey) {
        return db
            .select()
    ...
    async function decidePrincipalGrant(input) {
        ...
        if (!(await scopeAllows(db, input.companyId, grant.scope, input.scope, {
    ...
    async function loadRunPolicy(runId, companyId, agentId) {
        if (!runId)
            return null;
        const row = await db
            .select({
            ...
            .then((rows) => rows[0] ?? null);
        if (!row || row.companyId !== companyId || row.agentId !== agentId)
            return null;
    ...
    async function loadProjectAuthorizationPolicy(companyId, projectId) {
        const row = await db
    ...
    async function loadIssueAuthorizationPolicy(companyId, issueId) {
        const row = await db
    ...
    async function loadResourceContext(resource) {
        ...
        const projectId = resource.type === "issue"
    ...
    async function issueIdIsDescendantOf(issueId, rootIssueId, companyId) {
    ...
    `);
        ...
        return Boolean(first &&
            typeof first === "object" &&
            first.is_descendant === true);
    }
    async function issueResourceWithinLowTrustBoundary(boundary, resource) {
        ...
        if (isIssueWithinLowTrustBoundary(boundary, candidate))
            return true;
        ...
        if (!parent)
            return false;
        if (isIssueWithinLowTrustBoundary(boundary, {
    ...
    async function projectWithinLowTrustBoundary(boundary, projectId) {
        if (!projectId)
            return false;
        if (boundary.projectIds?.includes(projectId))
            return true;
    ...
    function agentWithinLowTrustBoundary(boundary, actorAgentId, targetAgentId) {
        if (!targetAgentId)
            return false;
    ...
    async function decideLowTrustAccess(input) {
        if (input.resolution.kind === "standard")
            return null;
        ...
        const lowTrustDeny = (explanation) => deny({
            action: input.action,
            reason: "deny_low_trust_boundary",
            explanation,
        });
        const lowTrustAllow = (explanation) => allow({
            action: input.action,
        ...
        if (input.action === "company_scope:read" ||
            ...
            return lowTrustDeny(`${LOW_TRUST_REVIEW_PRESET} agents cannot use company-wide or privileged ${input.action} APIs by default.`);
        }
        ...
        if (input.action === "project:read") {
            const projectId = input.resource.type === "issue"
        ...
        if (input.action === "tasks:assign") {
            ...
            if (input.resource.assigneeAgentId &&
    ...
    function taskBridgeScopeIds(scope, singularKey, pluralKey) {
        return [
            ...(typeof scope[singularKey] === "string" ? [scope[singularKey]] : []),
            ...(Array.isArray(scope[pluralKey]) ? scope[pluralKey] : []),
        ].filter((value) => typeof value === "string" && value.length > 0);
    }
    async function parentIssueMatchesTaskBridgeBoundary(parentIssueId, companyId, allowedParentIssueIds) {
        if (!parentIssueId || allowedParentIssueIds.length === 0)
            return false;
        if (allowedParentIssueIds.includes(parentIssueId))
            return true;
        for (const rootIssueId of allowedParentIssueIds) {
            if (await issueIdIsDescendantOf(parentIssueId, rootIssueId, companyId))
                return true;
        }
        return false;
    }
    async function issueMatchesTaskBridgeCreateBoundary(scope, resource) {
        ...
        if (resource.projectId && allowedProjectIds.includes(resource.projectId))
            return true;
    ...
    async function issueMatchesTaskBridgeWriteBoundary(input) {
        ...
        if (assigneeAgentId === input.actorAgentId)
            return true;
    ...
    async function decideTaskBridgeAccess(input) {
        const denyBridge = (explanation) => deny({
            action: input.action,
            reason: "deny_scope",
            explanation,
        });
        const allowBridge = (explanation) => allow({
            action: input.action,
            reason: "allow_explicit_grant",
            explanation,
        });
    ...
    async function assignmentTargetIsInCompany(resource) {
    ...
    async function assignmentPolicyEffect(resource) {
        if (resource.type !== "issue")
            return { kind: "none" };
        const checks = [];
        if (resource.assigneeAgentId) {
            checks.push(loadAgent(resource.assigneeAgentId).then((agent) => evaluateAuthorizationPolicyForAssignment(readPolicyObject(agent?.permissions, "authorizationPolicy"), "Target agent")));
        }
        if (resource.projectId) {
            checks.push(loadProjectAuthorizationPolicy(resource.companyId, resource.projectId).then((policy) => evaluateAuthorizationPolicyForAssignment(policy, "Target project")));
        }
        if (resource.issueId) {
            checks.push(loadIssueAuthorizationPolicy(resource.companyId, resource.issueId).then((policy) => evaluateAuthorizationPolicyForAssignment(policy, "Target issue")));
        }
        if (resource.parentIssueId && resource.parentIssueId !== resource.issueId) {
            checks.push(loadIssueAuthorizationPolicy(resource.companyId, resource.parentIssueId).then((policy) => evaluateAuthorizationPolicyForAssignment(policy, "Parent issue")));
        }
        if (checks.length === 0)
            return { kind: "none" };
        const effects = await Promise.all(checks);
        return (effects.find((effect) => effect.kind === "unknown") ??
            effects.find((effect) => effect.kind === "requires_approval") ??
            effects.find((effect) => effect.kind === "restricted") ??
            { kind: "none" });
    }
    async function isManagerOf(companyId, managerAgentId, assigneeAgentId) {
    ...
    function commentAuthorCanGrantIssueMention(input) {
        if (input.authorAgentId) {
            if (input.authorAgentId === input.mentionedAgentId)
                return false;
    ...
    async function agentHasMentionGrantOnIssue(input) {
        const rows = await db
            ...
            .from(issueComments)
            .where(and(eq(issueComments.companyId, input.companyId), eq(issueComments.issueId, input.issueId), isNull(issueComments.deletedAt), sql `${issueComments.body} LIKE ${"%agent://" + input.actorAgentId + "%"}`));
        ...
        const activeAuthorUserIds = new Set(authorUserIds.length === 0
            ? []
            : await db
                .select({ principalId: companyMemberships.principalId })
                .from(companyMemberships)
                .where(and(eq(companyMemberships.companyId, input.companyId), eq(companyMemberships.principalType, "user"), eq(companyMemberships.status, "active"), inArray(companyMemberships.principalId, authorUserIds)))
    ...
    function allowIssueMentionGrant(action) {
        return allow({
            action,
    ...
    async function decide(input) {
        ...
        async function decideWithTaskAssignmentGrants(principalType, principalId) {
            ...
            if (broadDecision.allowed || broadDecision.reason === "deny_missing_membership")
            ...
            if (scopedDecision.allowed || broadDecision.reason === "deny_missing_grant")
                return scopedDecision;
            return broadDecision;
        }
        async function denyForAssignmentPolicyIfNeeded(policyEffect) {
            if (policyEffect.kind === "none" || policyEffect.kind === "restricted")
                return null;
            return deny({
                action: input.action,
                reason: "deny_policy_restricted",
        ...
        function denyRestrictedAssignmentPolicy(policyEffect) {
            return deny({
                action: input.action,
                reason: "deny_policy_restricted",
        ...
        if (input.actor.type === "board") {
            let taskAssignmentPolicyEffect = null;
            ...
            if (!permissionKey) {
                if (input.action === "agent:read" ||
                    ...
                    const requiresNonViewer = input.action === "runtime:manage" || input.action === "secrets:read";
        ...
        if (input.actor.source === "agent_key" && input.actor.keyScope?.kind === "task_bridge") {
            ...
            if (taskBridgeDecision)
                return taskBridgeDecision;
        }
