...
async function estimateLocalDirSize(input) {
    ...
    const walk = async (dir, relative) => {
        ...
        for (const entry of entries) {
            ...
            if (isExcluded(entryRelative, entry.name))
                continue;
...
async function probeRemoteDirSize(input) {
    try {
        const result = await runSshScript(input.spec, `du -sk ${shellQuote(input.remoteDir)} 2>/dev/null | cut -f1`, { timeoutMs: 15_000, maxBuffer: 16 * 1024 });
...
function createTransferProgress(input) {
    ...
    const totalReady = input.totalBytes != null && typeof input.totalBytes.then === "function"
        ? input.totalBytes.then(applyTotal, () => applyTotal(null))
        : (applyTotal(input.totalBytes), Promise.resolve());
    let transferred = 0;
