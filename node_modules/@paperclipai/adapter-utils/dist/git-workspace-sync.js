...
export const GIT_ARCHIVE_EXCLUDES = [".git", ".git/*"];
function shellQuote(value) {
    return `'${value.replace(/'/g, `'\"'\"'`)}'`;
}
export async function runLocalGit(localDir, args, options = {}) {
    return await new Promise((resolve, reject) => {
        execFile("git", ["-C", localDir, ...args], {
...
export async function readGitWorkspaceSnapshot(localDir) {
    try {
        ...
        const splitNul = (value) => value.split("\0").map((entry) => entry.trim()).filter(Boolean);
        return {
...
export async function withShallowGitWorkspaceClone(input, fn) {
    ...
    try {
        ...
        await runLocalGit(cloneDir, input.snapshot.branchName
...
export function createImportedGitRef(scope = "remote") {
    return `refs/paperclip/git-sync/imported/${scope}/${randomUUID()}`;
}
export function createRemoteGitExportRef(scope = "remote") {
...
export async function integrateImportedGitHead(input) {
    ...
    for (let attempt = 0; attempt < 5; attempt += 1) {
        ...
        try {
            ...
            throw new Error(`Failed to merge concurrent remote git histories for ${currentHead.slice(0, 12)} and ${input.importedHead.slice(0, 12)}: ${reason}`);
        }
...
export async function resetLocalGitIndexToHead(input) {
    ...
    if (!input.checkWorkingTreeClean)
        return;
...
//# sourceMappingURL=git-workspace-sync.js.map