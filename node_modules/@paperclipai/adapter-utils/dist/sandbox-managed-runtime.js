...
import { buildRemoteGitDeltaBundleScript, createImportedGitRef, createRemoteGitExportRef, deleteLocalGitRef, fetchGitBundleIntoLocalRef, GIT_ARCHIVE_EXCLUDES, integrateImportedGitHead, readGitWorkspaceSnapshot, resetLocalGitIndexToHead, withShallowGitWorkspaceClone, } from "./git-workspace-sync.js";
...
async function copyWorkspaceEntry(sourceRoot, targetRoot, relative) {
...
async function copySelectedWorkspaceEntries(input) {
    ...
    for (const relative of input.relativePaths) {
        if (shouldExcludePath(relative, input.exclude))
            continue;
...
async function emitRuntimeStatus(sink, phase, message) {
    if (!sink)
        return;
    await Promise.resolve(sink({ phase, message })).catch(() => undefined);
}
function mergeExcludes(...groups) {
    return [...new Set(groups.flatMap((group) => group ?? []))];
}
function preserveFindArgs(entries) {
...
async function removeDeletedPathsInSandbox(input) {
    ...
    await input.client.run(`sh -c ${shellQuote(`cd ${shellQuote(input.remoteDir)} && rm -rf -- ${quotedPaths}`)}`, { timeoutMs: input.spec.timeoutMs });
}
...
function makeTransferProgress(sink, phase, direction, label) {
    if (!sink)
...
export async function prepareSandboxManagedRuntime(input) {
    ...
    const workspaceArchiveExclude = mergeExcludes(SANDBOX_WORKSPACE_HEAVY_DIR_EXCLUDES, [...GIT_ARCHIVE_EXCLUDES], input.workspaceExclude, gitIgnoredExcludes);
    const restoreExclude = mergeExcludes(SANDBOX_WORKSPACE_HEAVY_DIR_EXCLUDES, [...GIT_ARCHIVE_EXCLUDES], [".paperclip-runtime"], input.preserveAbsentOnRestore, input.workspaceExclude, gitIgnoredExcludes);
    ...
    await withTempDir("paperclip-sandbox-sync-", async (tempDir) => {
        ...
        if (gitSnapshot) {
            ...
            await withShallowGitWorkspaceClone({
                ...
                await input.client.run(`sh -c ${shellQuote(`mkdir -p ${shellQuote(workspaceRemoteDir)} && ` +
                    ...
                    `rm -f ${shellQuote(remoteGitTar)}`)}`, { timeoutMs: input.spec.timeoutMs });
            });
        }
        ...
        await input.client.writeFile(remoteWorkspaceTar, toArrayBuffer(workspaceTarBytes), workspaceUpload.options);
        ...
        const extractWorkspaceTarCommand = gitSnapshot
            ...
            : `mkdir -p ${shellQuote(workspaceRemoteDir)} && ` +
                ...
                `tar -xf ${shellQuote(remoteWorkspaceTar)} -C ${shellQuote(workspaceRemoteDir)} && ` +
                `rm -f ${shellQuote(remoteWorkspaceTar)}`;
        await input.client.run(`sh -c ${shellQuote(extractWorkspaceTarCommand)}`, { timeoutMs: input.spec.timeoutMs });
        if (gitSnapshot) {
    ...
    return {
        ...
        restoreWorkspace: async (onProgress) => {
            ...
            await withTempDir("paperclip-sandbox-restore-", async (tempDir) => {
                let importedRef = null;
                let importedHead = null;
                ...
                try {
                    if (gitSnapshot) {
                        ...
                        await input.client.run(`sh -c ${shellQuote(buildRemoteGitDeltaBundleScript({
                            remoteDir: workspaceRemoteDir,
                    ...
                    await input.client.run(`sh -c ${shellQuote(`mkdir -p ${shellQuote(runtimeRootDir)} && ` +
                        ...
                        `${tarExcludeFlags(restoreExclude)} .`)}`, { timeoutMs: input.spec.timeoutMs });
