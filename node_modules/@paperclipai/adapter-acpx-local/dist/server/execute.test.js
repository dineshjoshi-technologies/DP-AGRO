...
async function onlyChildDir(parent) {
    const entries = await fs.readdir(parent);
    expect(entries).toHaveLength(1);
    return path.join(parent, entries[0]);
}
async function createSkill(root, name, body = `---\nrequired: false\n---\n# ${name}\n`) {
...
async function runExecutor(config, options = {}) {
...
describe("acpx_local runtime skill isolation", () => {
    ...
    it.skipIf(process.platform === "win32")("replaces stale managed Codex auth files with source symlinks", async () => {
        ...
        const managedCodexHome = path.join(paperclipHome, "instances", paperclipInstanceId, "companies", "company-1", "codex-home");
    ...
    it("cleans aged credential wrapper scripts across ACPX agent changes", async () => {
        ...
        await Promise.all((await fs.readdir(wrappersDir))
    ...
    it("keeps distinct wrapper env files for concurrent runs with different credentials", async () => {
        ...
        const envFiles = await Promise.all(envFileNames.map(async (name) => fs.readFile(path.join(stateDir, "wrappers", name), "utf8")));
    ...
    it("enriches acpx.error diagnostics and child stderr when ensureSession rejects", async () => {
        ...
        const execute = createAcpxLocalExecutor({
            createRuntime: () => ({
                ensureSession: async () => {
                    throw new FakeAcpRuntimeError("session/new failed: backend rejected initialize", new Error("upstream timeout"));
                },
                startTurn: () => ({
                    events: (async function* () { })(),
                    result: Promise.resolve({ status: "completed", stopReason: "end_turn" }),
    ...
    it("writes wrapper that redirects child stderr to a per-run log file", async () => {
        ...
        expect(result.exitCode).toBe(0);
        const verboseFlags = runtimeOptions.map((options) => options.verbose);
    ...
    it("passes Paperclip env through the ACP agent wrapper instead of process.env", async () => {
        let observedApiKeyDuringStream;
        const execute = createAcpxLocalExecutor({
            createRuntime: () => ({
                ensureSession: async () => ({
                    backendSessionId: "backend-session",
    ...
    it("writes a Paperclip-managed .claude/settings.local.json for the claude agent so it can reach the Paperclip API", async () => {
        ...
        const { meta } = await runExecutor({ agent: "claude", stateDir, cwd }, { context: { paperclipWorkspace: { cwd, agentHome: path.join(root, "agent-home") } } });
        ...
        const additionalDirectories = written.permissions?.additionalDirectories;
        ...
        const note = meta[0]?.commandNotes?.find((entry) => entry.includes("Paperclip-managed Claude settings"));
    ...
    it("merges Paperclip allowlist into an existing .claude/settings.local.json without losing user entries", async () => {
        ...
        await fs.writeFile(path.join(cwd, ".claude", "settings.local.json"), JSON.stringify({
        ...
        const written = JSON.parse(await fs.readFile(path.join(cwd, ".claude", "settings.local.json"), "utf8"));
    ...
    it("overrides a user-supplied dontAsk defaultMode so ACPX can route Bash through canUseTool", async () => {
        ...
        const cwd = path.join(root, "worktree");
        await fs.mkdir(path.join(cwd, ".claude"), { recursive: true });
        await fs.writeFile(path.join(cwd, ".claude", "settings.local.json"), JSON.stringify({ permissions: { defaultMode: "dontAsk" } }, null, 2), "utf8");
        const { meta } = await runExecutor({ agent: "claude", stateDir, cwd }, { context: { paperclipWorkspace: { cwd } } });
        const written = JSON.parse(await fs.readFile(path.join(cwd, ".claude", "settings.local.json"), "utf8"));
        expect(written.permissions?.defaultMode).toBe("default");
        const overrideNote = meta[0]?.commandNotes?.find((entry) => entry.includes("overrode user dontAsk"));
    ...
    it("opts the claude agent into ACPX runtime verbose logs but leaves codex/custom agents quiet", async () => {
        ...
        const cwd = path.join(root, "worktree");
        await fs.mkdir(cwd, { recursive: true });
        const verboseByAgent = {};
        for (const agent of ["claude", "codex", "custom"]) {
            const runtimeOptions = [];
            const execute = createAcpxLocalExecutor({
                createRuntime: (options) => {
                    runtimeOptions.push(options);
            ...
            expect(result.exitCode).toBe(0);
            verboseByAgent[agent] = runtimeOptions[0]?.verbose;
        }
    ...
    it("does not touch .claude/settings.local.json for the codex agent", async () => {
        ...
        const cwd = path.join(root, "worktree");
        await fs.mkdir(cwd, { recursive: true });
        await runExecutor({ agent: "codex", stateDir, cwd }, { context: { paperclipWorkspace: { cwd } } });
