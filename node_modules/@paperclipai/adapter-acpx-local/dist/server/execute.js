...
import { DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE, applyPaperclipWorkspaceEnv, asNumber, asString, buildInvocationEnvForLogs, buildPaperclipEnv, ensureAbsoluteDirectory, ensurePathInEnv, joinPromptSections, materializePaperclipSkillCopy, parseObject, readPaperclipRuntimeSkillEntries, readPaperclipIssueWorkModeFromContext, renderPaperclipWakePrompt, renderTemplate, resolvePaperclipInstanceRootForAdapter, resolvePaperclipDesiredSkillNames, rewriteWorkspaceCwdEnvVarsForExecution, shapePaperclipWorkspaceEnvForExecution, stringifyPaperclipWakePayload, } from "@paperclipai/adapter-utils/server-utils";
...
async function writePaperclipClaudeSettings(input) {
    ...
    const existingPerms = existing.permissions && typeof existing.permissions === "object" && !Array.isArray(existing.permissions)
        ? existing.permissions
        : {};
    const existingAllow = Array.isArray(existingPerms.allow)
        ? existingPerms.allow.filter((value) => typeof value === "string")
        : [];
    const existingAdditionalDirectories = Array.isArray(existingPerms.additionalDirectories)
        ? existingPerms.additionalDirectories.filter((value) => typeof value === "string")
        : [];
    ...
    const existingDefaultMode = typeof existingPerms.defaultMode === "string" ? existingPerms.defaultMode : "";
    const defaultMode = existingDefaultMode && existingDefaultMode !== "dontAsk" ? existingDefaultMode : "default";
    ...
    const next = { ...existing, permissions: nextPermissions };
...
async function buildRuntime(input) {
    ...
    let paperclipClaudeSettings = null;
    if (acpxAgent === "claude") {
        ...
        skillCommandNotes.push(`Wrote Paperclip-managed Claude settings to ${paperclipClaudeSettings.filePath} (defaultMode=${paperclipClaudeSettings.defaultMode}${paperclipClaudeSettings.overrodeDontAsk ? "; overrode user dontAsk" : ""}, +${paperclipClaudeSettings.additionalDirectories.length} read root(s), +${paperclipClaudeSettings.allow.length} allow rule(s)).`);
    }
...
async function applySessionConfigOptions(input) {
    ...
    for (const option of options) {
        ...
        await input.onLog("stdout", `[paperclip] Applied ACPX ${input.prepared.acpxAgent} config ${option.key}=${option.value}\n`);
    }
}
...
function describeErrorDiagnostics(err) {
    const errorName = err instanceof Error ? err.name || err.constructor.name : typeof err;
    ...
    const causeMessage = cause instanceof Error
        ...
        : typeof cause === "string"
            ? cause
            : null;
    const retryable = err && typeof err === "object" && typeof err.retryable === "boolean"
        ? err.retryable
        : null;
...
function classifyError(err, phase) {
    ...
    const phaseCode = (() => {
        ...
        if (acpCode === "ACP_TURN_FAILED")
            return "acpx_turn_failed";
        if (acpCode === "ACP_BACKEND_MISSING")
            return "acpx_backend_missing";
        if (acpCode === "ACP_BACKEND_UNAVAILABLE")
            return "acpx_backend_unavailable";
        if (phase === "ensure_session")
            return "acpx_session_init_failed";
        if (phase === "configure_session")
            return "acpx_session_config_failed";
        if (phase === "turn")
            return "acpx_turn_failed";
        return null;
    })();
...
async function emitAcpxFailure(input) {
    ...
    if (childStderrTail) {
        await ctx.onLog("stderr", `[paperclip] ACPX child stderr tail (${phase}):\n${childStderrTail}\n`);
    }
    await emitAcpxLog(ctx, {
        type: "acpx.error",
        message,
...
export function createAcpxLocalExecutor(deps = {}) {
    ...
    return async function executeAcpxLocal(ctx) {
        ...
        try {
            ...
            const preEmitMessage = messageOverride ?? (err instanceof Error ? err.message : String(err));
            if (cancel)
                await cancel(preEmitMessage).catch(() => { });
            await runtime.close({
                handle: sessionHandle,
