...
import { agents, companies, createDb, documents, issues, pipelineAutomationExecutions, pipelineCaseBlockers, pipelineCaseEvents, pipelineCaseIssueLinks, pipelineCases, pipelineDocuments, pipelineStages, pipelineTransitions, pipelines, routines, } from "./index.js";
import { getEmbeddedPostgresTestSupport, startEmbeddedPostgresTestDatabase, } from "./test-embedded-postgres.js";
const embeddedPostgresSupport = await getEmbeddedPostgresTestSupport();
const describeEmbeddedPostgres = embeddedPostgresSupport.supported ? describe : describe.skip;
if (!embeddedPostgresSupport.supported) {
    console.warn(`Skipping embedded Postgres pipeline schema tests on this host: ${embeddedPostgresSupport.reason ?? "unsupported environment"}`);
}
function expectConstraintError(action) {
...
describeEmbeddedPostgres("pipeline schema", () => {
    ...
    it("persists one row per pipeline table and enforces unique keys and required checks", async () => {
        ...
        await expectConstraintError(() => db.insert(pipelines).values({ companyId: company.id, key: "content", name: "Duplicate" }));
        ...
        await expectConstraintError(() => db.insert(pipelineStages).values({
            pipelineId: pipeline.id,
            key: "intake",
        ...
        await expectConstraintError(() => db.insert(pipelineTransitions).values({
            pipelineId: pipeline.id,
            fromStageId: intakeStage.id,
            toStageId: reviewStage.id,
        }));
        ...
        await expectConstraintError(() => db.insert(pipelineCases).values({
            companyId: company.id,
            pipelineId: pipeline.id,
            stageId: intakeStage.id,
            caseKey: "release-1",
        ...
        await expectConstraintError(() => db.insert(pipelineCaseEvents).values({
            companyId: company.id,
            caseId: rootCase.id,
            type: "updated",
            actorType: "agent",
        ...
        await expectConstraintError(() => db.insert(pipelineCaseIssueLinks).values({
            companyId: company.id,
            caseId: rootCase.id,
            issueId: issue.id,
            role: "work",
        ...
        await expectConstraintError(() => db.insert(pipelineCaseBlockers).values({
            companyId: company.id,
            caseId: childCase.id,
            blockedByCaseId: rootCase.id,
        }));
        ...
        await expectConstraintError(() => db.insert(pipelineDocuments).values({
            companyId: company.id,
            pipelineId: pipeline.id,
            documentId: document.id,
        ...
        await expectConstraintError(() => db.insert(pipelineAutomationExecutions).values({
            companyId: company.id,
            caseId: rootCase.id,
            automationId: "draft-on-enter",
            triggeringEventId: event.id,
...
//# sourceMappingURL=pipelines-schema.test.js.map