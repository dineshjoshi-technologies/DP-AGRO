...
export const routineRevisions = pgTable("routine_revisions", {
    id: uuid("id").primaryKey().defaultRandom(),
    companyId: uuid("company_id").notNull().references(() => companies.id, { onDelete: "cascade" }),
    ...
    restoredFromRevisionId: uuid("restored_from_revision_id").references(() => routineRevisions.id, { onDelete: "set null" }),
    createdByAgentId: uuid("created_by_agent_id").references(() => agents.id, { onDelete: "set null" }),
    createdByUserId: text("created_by_user_id"),
    ...
    companyRoutineCreatedIdx: index("routine_revisions_company_routine_created_idx").on(table.companyId, table.routineId, table.createdAt),
}));
