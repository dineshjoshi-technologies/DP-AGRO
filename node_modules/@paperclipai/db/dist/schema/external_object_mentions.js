...
export const externalObjectMentions = pgTable("external_object_mentions", {
    ...
    canonicalIdentity: jsonb("canonical_identity").$type(),
    objectId: uuid("object_id").references(() => externalObjects.id, { onDelete: "set null" }),
    providerKey: text("provider_key"),
    detectorKey: text("detector_key"),
    objectType: text("object_type"),
    confidence: text("confidence").$type().notNull().default("exact"),
    createdByPluginId: uuid("created_by_plugin_id").references(() => plugins.id, { onDelete: "set null" }),
    createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
    updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
}, (table) => ({
    companySourceIssueIdx: index("external_object_mentions_company_source_issue_idx").on(table.companyId, table.sourceIssueId),
    companyObjectIdx: index("external_object_mentions_company_object_idx").on(table.companyId, table.objectId),
    companyProviderIdx: index("external_object_mentions_company_provider_idx").on(table.companyId, table.providerKey, table.objectType),
    companySourceMentionWithRecordUq: uniqueIndex("external_object_mentions_company_source_record_uq").on(table.companyId, table.sourceIssueId, table.sourceKind, table.sourceRecordId, table.documentKey, table.propertyKey, table.canonicalIdentityHash).where(sql `${table.sourceRecordId} is not null and ${table.canonicalIdentityHash} is not null`),
    companySourceMentionWithoutRecordUq: uniqueIndex("external_object_mentions_company_source_null_record_uq").on(table.companyId, table.sourceIssueId, table.sourceKind, table.documentKey, table.propertyKey, table.canonicalIdentityHash).where(sql `${table.sourceRecordId} is null and ${table.canonicalIdentityHash} is not null`),
}));
//# sourceMappingURL=external_object_mentions.js.map