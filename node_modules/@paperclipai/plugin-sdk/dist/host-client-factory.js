...
export class InvocationScopeDeniedError extends Error {
    ...
    constructor(pluginId, method, message) {
...
export function createHostClientHandlers(options) {
    ...
    const noCompanyScope = { kind: "none" };
    ...
    function requestedCompanyScope(method, params) {
        if (method === "companies.list")
            return { kind: "all" };
        if (!isRecord(params))
            return noCompanyScope;
        const companyId = readNonEmptyString(params.companyId);
        if (companyId)
            return { kind: "single", companyId };
        ...
        if (method === "events.subscribe" && isRecord(params.filter)) {
            ...
            if (filterCompanyId)
                return { kind: "single", companyId: filterCompanyId };
        }
        return noCompanyScope;
    }
    function requireInvocationCompanyScope(method, params, context) {
        const requested = requestedCompanyScope(method, params);
        if (requested.kind === "none")
            return;
        if (context?.invalidInvocationScope) {
            throw new InvocationScopeDeniedError(pluginId, method, "the worker referenced a missing, expired, or unknown invocation scope");
        }
        ...
        if (requested.kind === "all") {
            if (method === "companies.list")
                return;
            throw new InvocationScopeDeniedError(pluginId, method, `the current invocation is scoped to company "${allowedCompanyId}"`);
        }
        if (requested.companyId !== allowedCompanyId) {
            throw new InvocationScopeDeniedError(pluginId, method, `requested company "${requested.companyId}" but the current invocation is scoped to company "${allowedCompanyId}"`);
        }
    }
    /**
    ...
    return {
        ...
        "companies.list": gated("companies.list", async (params, context) => {
            ...
            if (!allowedCompanyId)
                return rows;
            return rows.filter((company) => isRecord(company) && company.id === allowedCompanyId);
        }),
