...
export function buildPipelineMentionHref(pipelineId, stageKey) {
    const trimmedPipelineId = pipelineId.trim();
    const normalizedStageKey = stageKey?.trim();
    if (!normalizedStageKey)
        return `${PIPELINE_MENTION_SCHEME}${trimmedPipelineId}`;
    return `${PIPELINE_MENTION_SCHEME}${trimmedPipelineId}?stage=${encodeURIComponent(normalizedStageKey)}`;
}
export function parsePipelineMentionHref(href) {
    if (!href.startsWith(PIPELINE_MENTION_SCHEME))
        return null;
    let url;
    try {
        url = new URL(href);
...
export function extractPipelineMentions(markdown) {
    if (!markdown)
        return [];
    const seen = new Set();
    const mentions = [];
