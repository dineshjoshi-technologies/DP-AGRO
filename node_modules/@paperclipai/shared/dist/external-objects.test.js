...
import { buildExternalObjectMentionSourceKey, buildExternalObjectScopedIdentityKey, canonicalizeExternalObjectUrl, extractExternalObjectCanonicalUrls, findExternalObjectUrlMatches, } from "./external-objects-server.js";
...
describe("external object references", () => {
    it("extracts external urls without changing internal issue reference behavior", () => {
        expect(findExternalObjectUrlMatches("See PAP-1, /issues/PAP-2, https://paperclip.ing/PAP/issues/PAP-3, and https://github.com/acme/app/pull/4.")).toEqual([{ index: 70, length: 34, matchedText: "https://github.com/acme/app/pull/4" }]);
    });
    ...
    it("dedupes extracted canonical urls by canonical identity", () => {
        expect(extractExternalObjectCanonicalUrls("https://github.com/acme/app/pull/1?token=a and https://github.com/acme/app/pull/1#discussion").map((entry) => entry.sanitizedCanonicalUrl)).toEqual(["https://github.com/acme/app/pull/1"]);
    });
    it("includes company id in scoped object identity keys", () => {
        ...
        expect(buildExternalObjectScopedIdentityKey({ companyId: "company-a", ...base })).not.toBe(buildExternalObjectScopedIdentityKey({ companyId: "company-b", ...base }));
    });
    ...
    it("formats stable source labels", () => {
        ...
        expect(formatExternalObjectMentionSourceLabel({ sourceKind: "document", documentKey: "plan" })).toBe("Document: plan");
        expect(formatExternalObjectMentionSourceLabel({ sourceKind: "property", propertyKey: "pr" })).toBe("Property: pr");
    });
...
//# sourceMappingURL=external-objects.test.js.map