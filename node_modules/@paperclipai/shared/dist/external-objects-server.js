...
export function findExternalObjectUrlMatches(markdown) {
    if (!markdown)
        return [];
    ...
    while ((match = re.exec(scrubbed)) !== null) {
        ...
        if (!matchedText || parseIssueReferenceHref(matchedText))
            continue;
        matches.push({
            index: match.index,
...
export function canonicalizeExternalObjectUrl(value, options = {}) {
    let url;
    try {
...
export function extractExternalObjectCanonicalUrls(markdown, options = {}) {
    const seen = new Set();
    const ordered = [];
    for (const match of findExternalObjectUrlMatches(markdown)) {
        ...
        if (!canonical || seen.has(canonical.canonicalIdentityHash))
            continue;
...
export function buildExternalObjectMentionSourceKey(source) {
    return [
        source.companyId,
...
//# sourceMappingURL=external-objects-server.js.map