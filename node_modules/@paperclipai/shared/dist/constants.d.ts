...
export declare const AGENT_ADAPTER_TYPES: readonly ["process", "http", "acpx_local", "claude_local", "codex_local", "cursor_cloud", "gemini_local", "hermes_gateway", "hermes_local", "opencode_local", "pi_local", "cursor", "openclaw_gateway"];
 ...
 */
export declare const PROJECT_ICON_NAMES: readonly ["folder", "rocket", "code", "terminal", "database", "globe", "package", "boxes", "box", "layers", "briefcase", "compass", "target", "flame", "zap", "star", "bug", "wrench", "hammer", "lightbulb", "sparkles", "shield", "lock", "search", "cog", "brain", "cpu", "git-branch", "file-code", "puzzle", "gem", "atom", "heart", "mail", "message-square", "crown", "radar", "telescope", "hexagon"];
...
export declare const ISSUE_WORK_MODES: readonly ["standard", "ask", "planning"];
...
export declare const ISSUE_THREAD_INTERACTION_KINDS: readonly ["suggest_tasks", "ask_user_questions", "request_confirmation", "request_checkbox_confirmation"];
export type IssueThreadInteractionKind = (typeof ISSUE_THREAD_INTERACTION_KINDS)[number];
export declare const REQUEST_CHECKBOX_CONFIRMATION_OPTION_LIMIT = 200;
...
export declare const TASK_WATCHDOG_PRODUCT_BUG_ORIGIN_KIND = "task_watchdog_product_bug";
export declare const ISSUE_ORIGIN_KINDS: readonly ["manual", "routine_execution", "stale_active_run_evaluation", "harness_liveness_escalation", "issue_productivity_review", "stranded_issue_recovery", "task_watchdog", "task_watchdog_product_bug"];
export type BuiltInIssueOriginKind = (typeof ISSUE_ORIGIN_KINDS)[number];
export type PluginIssueOriginKind = `plugin:${string}`;
export type IssueOriginKind = BuiltInIssueOriginKind | PluginIssueOriginKind;
export declare const ISSUE_WATCHDOG_DISCOVERY_KINDS: readonly ["product_bug", "platform_bug"];
...
export declare const ISSUE_RECOVERY_ACTION_KINDS: readonly ["missing_disposition", "stranded_assigned_issue", "workspace_validation", "configuration_validation", "active_run_watchdog", "issue_graph_liveness"];
...
export declare const PIPELINE_CASE_BODY_DOCUMENT_KEY: "pipeline-case-body";
export declare const SYSTEM_ISSUE_DOCUMENT_KEYS: readonly ["continuation-summary", "pipeline-case-body"];
...
export declare const DOCUMENT_ANNOTATION_THREAD_STATUSES: readonly ["open", "resolved"];
export type DocumentAnnotationThreadStatus = (typeof DOCUMENT_ANNOTATION_THREAD_STATUSES)[number];
export declare const DOCUMENT_ANNOTATION_ANCHOR_STATES: readonly ["active", "stale", "orphaned"];
export type DocumentAnnotationAnchorState = (typeof DOCUMENT_ANNOTATION_ANCHOR_STATES)[number];
export declare const DOCUMENT_ANNOTATION_ANCHOR_CONFIDENCES: readonly ["exact", "duplicate", "fuzzy", "ambiguous", "missing"];
export type DocumentAnnotationAnchorConfidence = (typeof DOCUMENT_ANNOTATION_ANCHOR_CONFIDENCES)[number];
export declare const EXTERNAL_OBJECT_STATUS_CATEGORIES: readonly ["unknown", "open", "waiting", "running", "succeeded", "failed", "blocked", "closed", "archived", "auth_required", "unreachable"];
export type ExternalObjectStatusCategory = (typeof EXTERNAL_OBJECT_STATUS_CATEGORIES)[number];
export declare const EXTERNAL_OBJECT_STATUS_TONES: readonly ["neutral", "info", "success", "warning", "danger", "muted"];
export type ExternalObjectStatusTone = (typeof EXTERNAL_OBJECT_STATUS_TONES)[number];
export declare const EXTERNAL_OBJECT_LIVENESS_STATES: readonly ["unknown", "fresh", "stale", "auth_required", "unreachable"];
export type ExternalObjectLivenessState = (typeof EXTERNAL_OBJECT_LIVENESS_STATES)[number];
export declare const EXTERNAL_OBJECT_MENTION_SOURCE_KINDS: readonly ["title", "description", "comment", "document", "property", "plugin"];
export type ExternalObjectMentionSourceKind = (typeof EXTERNAL_OBJECT_MENTION_SOURCE_KINDS)[number];
export declare const EXTERNAL_OBJECT_MENTION_CONFIDENCES: readonly ["exact", "likely", "possible"];
...
export declare const ENVIRONMENT_LEASE_STATUSES: readonly ["active", "released", "expired", "failed", "retained", "pending_cleanup"];
...
export declare const ROUTINE_VARIABLE_TYPES: readonly ["text", "textarea", "number", "boolean", "select", "date"];
...
export declare const PAUSE_REASONS: readonly ["manual", "budget", "system", "company_archived"];
...
export declare const LIVE_EVENT_TYPES: readonly ["heartbeat.run.queued", "heartbeat.run.status", "heartbeat.run.progress", "heartbeat.run.event", "heartbeat.run.log", "agent.status", "activity.logged", "external_object.updated", "plugin.ui.updated", "plugin.worker.crashed", "plugin.worker.restarted"];
...
export declare const PERMISSION_KEYS: readonly ["agents:create", "skills:create", "environments:manage", "users:invite", "users:manage_permissions", "tasks:assign", "tasks:assign_scope", "tasks:manage_active_checkouts", "pipelines:write", "joins:approve"];
 ...
 */
export declare const PLUGIN_CAPABILITIES: readonly ["companies.read", "projects.read", "project.workspaces.read", "execution.workspaces.read", "issues.read", "issue.relations.read", "issue.subtree.read", "issue.comments.read", "issue.documents.read", "agents.read", "goals.read", "goals.create", "goals.update", "activity.read", "costs.read", "issues.orchestration.read", "access.members.read", "access.invites.read", "authorization.grants.read", "authorization.policies.read", "authorization.audit.read", "database.namespace.read", "issues.create", "issues.update", "issue.relations.write", "issues.checkout", "issues.wakeup", "issue.comments.create", "issue.interactions.create", "issue.documents.write", "projects.managed", "routines.managed", "skills.managed", "agents.pause", "agents.resume", "agents.invoke", "agents.managed", "access.members.write", "access.invites.write", "authorization.grants.write", "authorization.policies.write", "agent.sessions.create", "agent.sessions.list", "agent.sessions.send", "agent.sessions.close", "activity.log.write", "metrics.write", "telemetry.track", "database.namespace.migrate", "database.namespace.write", "external.objects.detect", "external.objects.read", "external.objects.write", "external.objects.refresh", "plugin.state.read", "plugin.state.write", "events.subscribe", "events.emit", "jobs.schedule", "webhooks.receive", "api.routes.register", "http.outbound", "secrets.read-ref", "environment.drivers.register", "local.folders", "agent.tools.register", "instance.settings.register", "ui.sidebar.register", "ui.page.register", "ui.detailTab.register", "ui.dashboardWidget.register", "ui.commentAnnotation.register", "ui.action.register"];
...
export declare const WORKSPACE_OVERVIEW_DEFAULT_LIMIT = 50;
export declare const WORKSPACE_OVERVIEW_MAX_LIMIT = 100;
export declare const WORKSPACE_OVERVIEW_LINKED_ISSUE_LIMIT = 4;
/**
 ...
 */
export declare const PLUGIN_RESERVED_COMPANY_ROUTE_SEGMENTS: readonly ["dashboard", "onboarding", "companies", "company", "settings", "plugins", "org", "agents", "projects", "issues", "goals", "approvals", "costs", "activity", "inbox", "workspaces", "design-guide", "tests"];
 ...
 */
export declare const PLUGIN_RESERVED_COMPANY_SETTINGS_ROUTE_SEGMENTS: readonly ["general", "environments", "access", "members", "invites", "secrets", "instance"];
