...
export function resolveProjectionRange(projection, normalizedStart, normalizedEnd) {
...
export function createDocumentAnchorSelector(projection, range, options = {}) {
...
export function selectorToAnchorSnapshot(selector) {
    return {
...
export function anchorSnapshotToSelector(anchor) {
    return {
        quote: {
...
export function verifyDocumentAnchorSelector(input) {
    const projection = projectMarkdownToText(input.markdown);
    const range = resolveProjectionRange(projection, input.selector.position.normalizedStart, input.selector.position.normalizedEnd);
    if (!range) {
    ...
    if (range.markdownStart !== input.selector.position.markdownStart
...
export function remapDocumentAnchor(input) {
...
function stripBlockSyntax(line, absoluteOffset) {
    const blockMatch = line.match(/^\s{0,3}(?:(#{1,6})\s+|(?:[-+*]|\d+[.)])\s+|>\s?)/);
    if (!blockMatch)
        return { text: line, sourceOffset: absoluteOffset };
    return { text: line.slice(blockMatch[0].length), sourceOffset: absoluteOffset + blockMatch[0].length };
}
function addInlineMarkdownText(builder, text, sourceOffset) {
    for (let index = 0; index < text.length; index += 1) {
        ...
        if (isMarkdownFormattingChar(char, text, index))
            continue;
        builder.addChar(char, absolute, absolute + 1);
    }
}
function isMarkdownFormattingChar(char, text, index) {
    if (char === "*" || char === "_" || char === "~")
        return true;
...
function findFuzzyCandidate(projection, previousAnchor, contextLength) {
...
function buildAnchorSnapshot(projection, normalizedStart, normalizedEnd, contextLength) {
...
function prefixOverlapScore(expectedPrefix, actualPrefix) {
    ...
    for (let size = Math.min(expected.length, actual.length); size > 0; size -= 1) {
        if (expected.slice(0, size) === actual.slice(0, size))
            return size / expected.length;
    }
    return 0;
}
function suffixOverlapScore(expectedPrefix, actualPrefix) {
    const expected = normalizeAnchorText(expectedPrefix);
    const actual = normalizeAnchorText(actualPrefix);
    if (!expected)
        return 0.5;
    for (let size = Math.min(expected.length, actual.length); size > 0; size -= 1) {
        if (expected.slice(-size) === actual.slice(-size))
            return size / expected.length;
    }
    return 0;
}
...
//# sourceMappingURL=document-anchors.js.map